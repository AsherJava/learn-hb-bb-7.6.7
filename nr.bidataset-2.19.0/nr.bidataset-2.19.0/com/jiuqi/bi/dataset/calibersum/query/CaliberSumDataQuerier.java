...
public class CaliberSumDataQuerier {
    private DimensionValueSet srcMasterKeys;
    private CaliberSumContext context;
    private DataModelDefinitionsCache dataDefinitionsCache;
    ...
    private List<CaliberSumDSField> expFields = new ArrayList<CaliberSumDSField>();
    private List<CaliberSumDSField> destExpFields = new ArrayList<CaliberSumDSField>();
    private int unitColumnIndex;
    private CaliberSumResultSet result;

    public CaliberSumDataQuerier(DimensionValueSet srcMasterKeys, CaliberSumContext context, CaliberSumDSModel dsModel, DataModelService dataModelService, INvwaDataAccessProvider accessProvider) throws ParseException {
        this.srcMasterKeys = srcMasterKeys;
        this.context = context;
        this.dataDefinitionsCache = context.getExecutorContext().getCache().getDataModelDefinitionsCache();
    ...
    private void buildQuery() {
        for (int dsIndex = 0; dsIndex < this.dsModel.getFields().size(); ++dsIndex) {
            ...
            if (columnModel == null) {
                if (caliberSumDSField.getEvalExpression() == null) continue;
                if (caliberSumDSField.isDestEval()) {
                    this.destExpFields.add(caliberSumDSField);
                    continue;
                }
                this.expFields.add(caliberSumDSField);
                continue;
            }
            ...
            if (queryModel == null) {
                ...
                TableModelRunInfo tableInfo = this.dataDefinitionsCache.getTableInfoByCode(caliberSumDSField.getTableModelCode());
    ...
    private void tryEval() {
        if (this.expFields.isEmpty()) {
            return;
        }
        try {
            ArrayList<Integer> fieldIndexes = new ArrayList<Integer>(this.expFields.size());
            ArrayList<String> exps = new ArrayList<String>(this.expFields.size());
            for (CaliberSumDSField field : this.expFields) {
                fieldIndexes.add(field.getIndex());
                exps.add(field.getEvalExpression());
            }
            ...
            Map results = evaluator.evalBatch(exps, this.context.getExecutorContext(), this.srcMasterKeys);
            for (Map.Entry entry : results.entrySet()) {
                List<CaliberSumDSRow> destRows;
                String unitKey = (String)entry.getKey();
                Object[] dataRow = (Object[])entry.getValue();
                if (dataRow == null || (destRows = this.result.getDestRows(unitKey)) == null) continue;
                for (int columnIndex = 0; columnIndex < dataRow.length; ++columnIndex) {
                    ...
                    Integer destColumnIndex = (Integer)fieldIndexes.get(columnIndex);
                    for (CaliberSumDSRow destRow : destRows) {
                        destRow.statisticValue(destColumnIndex, value);
                    }
                }
            ...
            this.context.getLogger().error("\u8ba1\u7b97\u53d6\u503c\u8868\u8fbe\u5f0f\u51fa\u9519", e);
        }
    }

    private void tryDestEval() {
        if (this.destExpFields.isEmpty()) {
            return;
        }
        try {
            ArrayList<Integer> fieldIndexes = new ArrayList<Integer>(this.destExpFields.size());
            ArrayList<String> exps = new ArrayList<String>(this.destExpFields.size());
            for (CaliberSumDSField field : this.destExpFields) {
                fieldIndexes.add(field.getIndex());
                exps.add(field.getEvalExpression());
            }
            IDataAccessProvider accessProvider = (IDataAccessProvider)SpringBeanProvider.getBean(IDataAccessProvider.class);
            IExpressionEvaluator evaluator = accessProvider.newExpressionEvaluator();
            Map results = evaluator.evalBatch(exps, this.context.getExecutorContext(), this.context.getDestMasterKeys());
            for (Map.Entry entry : results.entrySet()) {
                CaliberSumDSRow destRow;
                String unitKey = (String)entry.getKey();
                Object[] dataRow = (Object[])entry.getValue();
                if (dataRow == null || (destRow = this.result.findRowByMainDim(unitKey)) == null) continue;
                for (int columnIndex = 0; columnIndex < dataRow.length; ++columnIndex) {
                    Object value = dataRow[columnIndex];
                    Integer destColumnIndex = (Integer)fieldIndexes.get(columnIndex);
                    destRow.setValue(destColumnIndex, value);
                }
            }
        }
        catch (Exception e) {
