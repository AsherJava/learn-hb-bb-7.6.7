...
@Component
public class ApplyToUnitXJFunction
extends Function
implements IGcFunction {
    private final String FUNCTION_NAME = "APPLYTOUNITXJ";

    public ApplyToUnitXJFunction() {
        this.parameters().add(new Parameter("unitCode", 6, "\u5355\u4f4d\u4ee3\u7801"));
    }

    public String name() {
        return "APPLYTOUNITXJ";
    }
    ...
    public Object evalute(IContext iContext, List<IASTNode> parameters) throws SyntaxException {
        ...
        GcBaseExecutorContext context = (GcBaseExecutorContext)queryContext.getExeContext();
        DimensionValueSet dimensionValueSet = queryContext.getCurrentMasterKey();
        String mdOrg = context.getOrgId();
        String orgType = GcOrgTypeUtils.getOrgTypeByContextOrTaskId((String)context.getTaskId());
        String dataTime = String.valueOf(dimensionValueSet.getValue("DATATIME"));
        YearPeriodObject yp = new YearPeriodObject(null, dataTime);
        GcOrgCenterService orgTool = GcOrgPublicTool.getInstance((String)String.valueOf(orgType), (GcAuthorityType)GcAuthorityType.NONE, (YearPeriodObject)yp);
        List allOrgS = orgTool.listAllOrgByParentIdContainsSelf(unitCode);
        if (CollectionUtils.isEmpty((Collection)allOrgS)) {
            return false;
        }
        boolean contains = allOrgS.stream().map(GcOrgCacheVO::getCode).collect(Collectors.toSet()).contains(mdOrg);
        if (contains) {
            return true;
        }
        return false;
    ...
    public String toDescription() {
        ...
        buffer.append("    ").append("APPLYTOUNITXJ(\"123456\")").append(StringUtils.LINE_SEPARATOR);
        buffer.append("    ").append("\u516c\u5f0f\u542b\u4e49\u662f\u89c4\u5219\u9002\u7528\u5f53\u524d\u53ca\u6240\u6709\u4e0b\u7ea7\u5408\u5e76\u5355\u4f4d");
