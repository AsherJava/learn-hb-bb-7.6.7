...
public class FixedTableRuleJsonStringVO2EOHelper {
    private UnionRuleVO unionRuleVO;

    private FixedTableRuleJsonStringVO2EOHelper() {
    }

    public static FixedTableRuleJsonStringVO2EOHelper newInstance(UnionRuleVO unionRuleVO) {
        FixedTableRuleJsonStringVO2EOHelper helper = new FixedTableRuleJsonStringVO2EOHelper();
    ...
    private Map<String, Object> handleSaveOneDetailedSetting(Map<String, Object> detailedSetting, String debitOrCredit, int fetchConfigGroupIndex, int rowDebitOrCreditIndex) {
        String oldSubjectCodeString;
        GcBaseDataVO subject;
        if (!Objects.isNull(detailedSetting.get("unit"))) {
            String oldUnitStr = JsonUtils.writeValueAsString((Object)detailedSetting.get("unit"));
            OrgDataVO gcOrgCacheVO = (OrgDataVO)JsonUtils.readValue((String)oldUnitStr, (TypeReference)new TypeReference<OrgDataVO>(){});
            detailedSetting.put("unit", gcOrgCacheVO.getCode());
        }
        if (Objects.isNull(subject = (GcBaseDataVO)JsonUtils.readValue((String)(oldSubjectCodeString = JsonUtils.writeValueAsString((Object)detailedSetting.get("subjectCode"))), (TypeReference)new TypeReference<GcBaseDataVO>(){}))) {
