...
@Component
public class IntermediatePushAsyncTask
implements NpAsyncTaskExecutor {
    @Lazy
    @Autowired
    private IntermediateLibraryService intermediateLibraryService;
    private static final Logger logger = LoggerFactory.getLogger(IntermediatePushAsyncTask.class);
    ...
    public void execute(Object args, AsyncTaskMonitor asyncTaskMonitor) {
        ...
        try {
            ...
            if (asyncTaskMonitor.isCancel()) {
                ...
                asyncTaskMonitor.finish(GcI18nUtil.getMessage((String)"gc.intermediate.push.success"), intermediateMessages);
            }
            logger.info("\u5171\u6267\u884c:" + (System.currentTimeMillis() - startTime) / 1000L + "\u79d2");
        }
        catch (Exception e) {
            asyncTaskMonitor.error(GcI18nUtil.getMessage((String)"gc.intermediate.push.error") + e.getMessage(), (Throwable)e);
            logger.error("\u5171\u6267\u884c:" + (System.currentTimeMillis() - startTime) / 1000L + "\u79d2", e);
        }
    }

    public String getTaskPoolType() {
        return "GC_ASYNC_TASK_INTERMEDIATE_PUSH";
    }
}
