...
@Component
public class AfterAbolishBackWriteType
implements VaBillBackWriteTypeIntf {
    @Override
    public String getType() {
        return VaBackWriteEnum.AFTER_ABORT.getName();
    }

    @Override
    public String getTitle() {
        return VaBackWriteEnum.AFTER_ABORT.getTitle();
    }
    ...
    public boolean needExecute(BillModel model) {
        ...
        if (oldbillstate == BillState.TEMPORARY.getValue()) {
            return false;
        }
        return billstate == BillState.DELETED.getValue() && billstate != oldbillstate;
    }
}
