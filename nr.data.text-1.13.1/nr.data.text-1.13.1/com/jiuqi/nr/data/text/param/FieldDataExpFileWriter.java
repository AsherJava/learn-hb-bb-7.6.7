/*
 * Decompiled with CFR 0.152.
 * 
 * Could not load the following classes:
 *  com.jiuqi.nr.data.common.service.FileWriter
 */
package com.jiuqi.nr.data.text.param;

import com.jiuqi.nr.data.common.service.FileWriter;
import java.io.File;
import java.io.IOException;
import java.io.InputStream;
import java.io.OutputStream;

public class FieldDataExpFileWriter
implements FileWriter {
    private FileWriter fileWriter;
    private String basePath;

    public FieldDataExpFileWriter(FileWriter fileWriter, String basePath) {
        this.fileWriter = fileWriter;
        this.basePath = basePath;
    }

    public void addBytes(String path, byte[] data) throws IOException {
        this.fileWriter.addBytes(this.basePath + "/" + path, data);
    }

    public void addFile(String path, File file) throws IOException {
        this.fileWriter.addFile(this.basePath + "/" + path, file);
    }

    public void addStream(String path, InputStream stream) throws IOException {
        this.fileWriter.addStream(this.basePath + "/" + path, stream);
    }

    public OutputStream createFile(String path) throws IOException {
        return this.fileWriter.createFile(this.basePath + "/" + path);
    }
}

