...
extends PDFPrintUtil {
    ...
    public static void printPDF(ITemplateDocument tmplDoc, IPaginateInteractor paginateInteractor, IPaintInteractor paintInteractor, OutputStream out, IProcessMonitor monitor) {
        block43: {
            ...
            IContentStream[] streams = ProcessUtil.getPaginateContentStreams((String)nature, (ITemplateDocument)tmplDoc, (IPaginateInteractor)paginateInteractor, (IProcessMonitor)monitor);
            try (IContentStream[] normalOut = new PrintStream(new Log4jPrintStream(logger));
                 PrintStream errorOut = new PrintStream(new Log4jPrintStream(logger));
                 PrintStream trackOut = new PrintStream(new Log4jPrintStream(logger));){
                drawManager.print(streams, paintInteractor, (IPrintDevice)device, (IProcessMonitor)new SimpleProcessMonitor((PrintStream)normalOut, errorOut, trackOut));
            }
            catch (Exception e) {
                ...
                g.end();
                if (streams == null) break block43;
                for (IContentStream stream : streams) {
