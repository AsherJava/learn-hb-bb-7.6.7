...
@Component
public class BatchUploadSumService {
    private static final Logger logger = LoggerFactory.getLogger(BatchUploadSumService.class);
    @Autowired
    ...
    public List<UploadSumInfo> batchUploadState(BatchQueryUpload batchQueryUpload) {
        ...
        Map statisticalStateMap = this.workflow.getStatisticalStates(batchQueryUpload.getFormSchemeKey());
        ...
        if (batchQueryUpload.getSummaryScope() > 1) {
            ...
            if (!StringUtils.isEmpty((String)formKey)) {
                List<IEntityRow> list = this.overViewBaseService.filterAuthByEntity(context, (List<IEntityRow>)childEntity, formKey, queryStartType);
            } else {
            ...
            if (StringUtils.isNotEmpty((String)batchQueryUpload.getFilter())) {
                ...
                if (filter.equals(UploadStateEnum.DELAY.getCode())) {
                    this.filterDelayUnitData((List<IEntityRow>)var24_28, filterEntity, formScheme, dwEntity.getDimensionName(), formKey, abortTime);
                    if (this.isDelay(abortTime)) {
                        batchQueryUpload.setFilter(UploadStateEnum.ORIGINAL_UPLOAD.getCode());
                        this.filterUnitData((List<IEntityRow>)var24_28, dimensionValueSet, dwEntity, formScheme, batchQueryUpload, queryStartType, defaultWorkflow, filterEntity, statisticalStateMap, filterKeys);
                ...
                entitypage = this.subList((List<IEntityRow>)var24_28, batchQueryUpload.getPageIndex(), batchQueryUpload.getSize());
            }
        } else {
            childEntity = iEntityTable.getChildRows(dwId);
            ArrayList<IEntityRow> arrayList = new ArrayList<IEntityRow>((Collection<IEntityRow>)childEntity);
            ...
            if (StringUtils.isNotEmpty((String)batchQueryUpload.getFilter())) {
                ...
                if (filter.equals(UploadStateEnum.DELAY.getCode())) {
                    ...
                    if (this.isDelay(abortTime)) {
                        batchQueryUpload.setFilter(UploadStateEnum.ORIGINAL_UPLOAD.getCode());
                        this.filterUnitData(acessChildEntity, dimensionValueSet, dwEntity, formScheme, batchQueryUpload, queryStartType, defaultWorkflow, filterEntity, statisticalStateMap, filterKeys);
                        filterEntity = this.mergeDelayUnit(filterEntity);
                        batchQueryUpload.setFilter(UploadStateEnum.DELAY.getCode());
                    }
                } else {
        ...
        for (IEntityRow iEntityRow : entitypage) {
            ...
            this.getUploadSum(iEntityRow, dimensionValueSet, batchQueryUpload.getFormKey(), formScheme, uploadSums, batchQueryUpload.getSummaryScope(), context, uploadActionInfos, defaultWorkflow, queryStartType, iEntityTable, batchQueryUpload.isLeafEntity(), batchQueryUpload.isFilterDiffUnit(), batchQueryUpload.isOnlyDirectChild(), abortTime, false, confirmBrforeUpload, statisticalStates);
        }
        if (uploadSums.size() > 0) {
            HashMap<String, String> stateToColor = new HashMap<String, String>();
            ((UploadSumInfo)uploadSums.get(0)).setKeyTitle(this.getTitleMap(batchQueryUpload.getFormSchemeKey(), period, defaultWorkflow, stateToColor));
            HashMap<String, String> hashMap = new HashMap<String, String>();
            for (Map.Entry color : stateToColor.entrySet()) {
                hashMap.put("custom@" + (String)color.getKey(), (String)color.getValue());
            }
            ((UploadSumInfo)uploadSums.get(0)).setKeyTitleCustomColor(hashMap);
        }
        return uploadSums;
    }
    ...
    public void filterUnitData(List<IEntityRow> acessChildEntity, DimensionValueSet dimensionValueSet, EntityViewData entityViewData, FormSchemeDefine formScheme, BatchQueryUpload batchQueryUpload, WorkFlowType queryStartType, boolean defaultWorkflow, List<IEntityRow> filterEntity, Map<String, List<String>> statisticalStateMap, List<String> filterKeys) {
        for (IEntityRow entity : acessChildEntity) {
            ...
            if (filter.startsWith("custom")) {
                this.filterCustomData(filter, actionState, filterEntity, entity, statisticalStateMap, filterKeys);
                continue;
            }
    ...
    private void filterCustomData(String filter, ActionStateBean actionState, List<IEntityRow> filterEntity, IEntityRow entity, Map<String, List<String>> statisticalStateMap, List<String> filterKeys) {
        String[] split;
        String keyWord;
        List<String> keyWordList;
        if (filter != null && filter.startsWith("custom") && (keyWordList = statisticalStateMap.get(keyWord = (split = filter.split("@"))[1])) != null) {
            for (String keyWord2 : keyWordList) {
                String[] nodeAndAction = keyWord2.split("@");
                String node = nodeAndAction[0];
                String action = nodeAndAction[1];
                if (actionState == null) {
                    if (!keyWord2.contains("start") || filterKeys.contains(entity.getEntityKeyData())) continue;
                    filterEntity.add(entity);
                    filterKeys.add(entity.getEntityKeyData());
                    continue;
                }
                if (actionState == null) continue;
                if (actionState.getCode().equals(UploadStateEnum.ORIGINAL_UPLOAD.getCode()) || actionState.getCode().equals(UploadStateEnum.ORIGINAL_SUBMIT.getCode())) {
                    String taskKey = actionState.getTaskKey();
                    if (!keyWordList.contains(taskKey + "@" + "start") || filterKeys.contains(entity.getEntityKeyData())) continue;
                    filterEntity.add(entity);
                    filterKeys.add(entity.getEntityKeyData());
                    continue;
                }
                if (!actionState.getCode().equals(action) || !actionState.getPreNode().equals(node) || filterKeys.contains(entity.getEntityKeyData())) continue;
                filterEntity.add(entity);
                filterKeys.add(entity.getEntityKeyData());
            }
        }
    ...
    public List<UploadAllFormSumInfo> batchQueryState(BatchQueryUpload batchQueryUpload) {
        ...
        if (queryAllFormState.size() > 0) {
            HashMap<String, String> stateToColor = new HashMap<String, String>();
            ((UploadAllFormSumInfo)queryAllFormState.get(0)).setKeyTitle(this.getTitleMap(batchQueryUpload.getFormSchemeKey(), period, defaultWorkflow, stateToColor));
        }
    ...
    public void getUploadSum(IEntityRow entityRow, DimensionValueSet dimensionValueSet, String formKey, FormSchemeDefine formScheme, List<UploadSumInfo> uploadSums, int summaryScope, JtableContext jtableContext, List<UploadActionInfo> uploadActions, boolean flowsType, WorkFlowType queryStartType, IEntityTable iEntityTable, boolean leafEntity, boolean filterDiffUnit, boolean onlyDirectChild, Calendar abortTime, boolean self, boolean confirmBrforeUpload, Map<String, List<String>> statisticalStates) {
        ...
        try {
            uploadSum = this.queryUploadStateService.queryUploadSum(dimensionAuth, formKey, formScheme, flowsType, entityKeyData, dwEntity.getDimensionName(), entityViewDefine, iEntityTable, leafEntity, filterDiffUnit, onlyDirectChild, abortTime, statisticalStates);
        }
        catch (Exception e) {
            logger.error(e.getMessage(), e);
        }
        if (uploadActions != null && uploadActions.size() > 0) {
    ...
    public List<ActionInfo> batchWorkFlow(QueryUploadStateInfo queryUploadStateInfo) {
        ...
        try {
            ...
            if (flowsType) {
                ...
                if (actionInfo.containsKey("start")) {
                    if (actionInfo.containsKey("act_submit") || actionInfo.containsKey("cus_submit") && flowsType) {
                        ...
                        String actionName = (String)actionInfo.get("start");
                        if (null == actionName) {
                            actionName = "\u672a\u4e0a\u62a5";
                        }
                        original.setTitle(actionName);
                        original.setType("1");
            ...
            if (!flowsType) {
                Map statisticalStates = this.workflow.getStatisticalStates(formSchemeKey);
                for (Map.Entry entry : statisticalStates.entrySet()) {
                    ActionInfo customInfo = new ActionInfo();
                    customInfo.setKey("custom@" + (String)entry.getKey());
                    customInfo.setTitle((String)entry.getKey());
                    customInfo.setType("1");
                    tableInfo.add(customInfo);
                }
                ActionInfo uploadedper = new ActionInfo();
                uploadedper.setKey("uploadedper");
                String UploadTitle = actionCodeAndActionName.containsKey("act_upload") ? (String)actionCodeAndActionName.get("act_upload") + "\u7387" : "\u4e0a\u62a5\u7387";
    ...
    public void exportAllFormState(ExportExcelState exportExcelState, HttpServletResponse response, HttpServletRequest request) throws IOException {
        ...
        if ((queryAllFormState = this.queryUploadStateService.queryAllFormState(dimension, exportExcelState.getFormKey(), formScheme, entityIds, queryStartType, formToSum)).size() > 0) {
            HashMap<String, String> stateToColor = new HashMap<String, String>();
            ((UploadAllFormSumInfo)queryAllFormState.get(0)).setKeyTitle(this.getTitleMap(exportExcelState.getFormSchemeKey(), period, flowsType, stateToColor));
        }
    ...
    public Map<String, String> getTitleMap(String formschemeKey, String period, boolean flowsType, Map<String, String> stateToColor) {
        Map actionInfo = this.treeWorkFlow.getActionCodeAndStateName(formschemeKey);
        Map actionCodeAndActionName = this.treeWorkFlow.getActionCodeAndActionName(formschemeKey);
        ...
        if (flowsType) {
            if (actionInfo.containsKey("start")) {
                if ((actionInfo.containsKey("act_submit") || actionInfo.containsKey("cus_submit")) && flowsType) {
                    ...
                    result.put("originalNum", actionName);
                }
            }
            if (actionInfo.containsKey("act_submit") || actionInfo.containsKey("cus_submit")) {
                if (actionInfo.containsKey("act_submit")) {
        ...
        if (!flowsType) {
            Map statisticalStates = this.workflow.getStatisticalStates(formschemeKey, stateToColor);
            for (Map.Entry entry : statisticalStates.entrySet()) {
