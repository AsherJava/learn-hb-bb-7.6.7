...
@RealTimeJob(group="ASYNCTASK_BATCH_FORM_LOCK", groupTitle="\u6279\u91cf\u9501\u5b9a", subject="\u62a5\u8868", tags={"\u957f\u4efb\u52a1"})
public class BatchFormLockAsyncTaskExecutor
extends NpRealTimeTaskExecutor {
    private static final Logger logger = LoggerFactory.getLogger(BatchFormLockAsyncTaskExecutor.class);

    public void executeWithNpContext(JobContext jobContext) {
        String errorInfo = "task_error_info";
        String cancelInfo = "task_cancel_info";
        IFormLockService formLockService = (IFormLockService)BeanUtil.getBean(IFormLockService.class);
        String taskId = jobContext.getInstanceId();
        AbstractRealTimeJob job = jobContext.getRealTimeJob();
        Map params = job.getParams();
        RealTimeTaskMonitor asyncTaskMonitor = new RealTimeTaskMonitor(taskId, AsynctaskPoolType.ASYNCTASK_BATCH_FORM_LOCK.getName(), jobContext);
        try {
            if (Objects.nonNull(params) && Objects.nonNull(params.get("NR_ARGS"))) {
                FormLockParam param = (FormLockParam)((Object)SimpleParamConverter.SerializationUtils.deserialize((String)((String)params.get("NR_ARGS"))));
                formLockService.batchLockForm(param, (AsyncTaskMonitor)asyncTaskMonitor);
            }
        }
    ...
    public String getTaskPoolType() {
        return AsynctaskPoolType.ASYNCTASK_BATCH_FORM_LOCK.getName();
    }
}
