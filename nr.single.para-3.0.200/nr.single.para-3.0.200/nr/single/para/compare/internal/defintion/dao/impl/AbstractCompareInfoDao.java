...
public abstract class AbstractCompareInfoDao<DO extends CompareInfoDO>
extends BaseDao
implements ICompareInfoDao<DO> {
    private final Logger logger = LoggerFactory.getLogger(AbstractCompareInfoDao.class);
    @Autowired
    ...
    public String insert(DO compareInfoDO) throws DataAccessException {
        ...
        if (((CompareInfoDO)compareInfoDO).getKey() == null) {
            ((CompareInfoDO)compareInfoDO).setKey(UUID.randomUUID().toString());
        }
        if (((CompareInfoDO)compareInfoDO).getUpdateTime() == null) {
            ((CompareInfoDO)compareInfoDO).setUpdateTime(Instant.now());
        }
        super.insert(compareInfoDO);
        return ((CompareInfoDO)compareInfoDO).getKey();
    }
    ...
    public void update(DO compareInfoDO) throws DataAccessException {
        Assert.notNull(compareInfoDO, "compareInfoDO must not be null.");
        Assert.notNull((Object)((CompareInfoDO)compareInfoDO).getKey(), "dataSchemeKey must not be null.");
    ...
    public DO get(String key) throws DataAccessException {
        Assert.notNull((Object)key, "key must not be null.");
        return (DO)((CompareInfoDO)super.getByKey(key, this.getClz()));
    }

    @Override
    public DO getByCode(String code) throws DataAccessException {
        Assert.notNull((Object)code, "code must not be null.");
        return (DO)((CompareInfoDO)super.list(new String[]{"CI_CODE"}, new Object[]{code}, this.getClz()).stream().findFirst().orElse(null));
    }

    @Override
    public List<DO> getByTask(String taskKey) throws DataAccessException {
        Assert.notNull((Object)taskKey, "taskKey must not be null.");
        return super.list(new String[]{"CI_TASK_KEY"}, new Object[]{taskKey}, this.getClz());
    }

    @Override
    public List<DO> getByformScheme(String formSchemeKey) throws DataAccessException {
        Assert.notNull((Object)formSchemeKey, "formSchemeKey must not be null.");
        return super.list(new String[]{"CI_TASK_KEY"}, new Object[]{formSchemeKey}, this.getClz());
    }

    @Override
    public void batchInsert(List<DO> compareInfoDO) throws DataAccessException {
        Assert.notNull(compareInfoDO, "compareInfoDO must not be null.");
        for (CompareInfoDO schemeDO : compareInfoDO) {
