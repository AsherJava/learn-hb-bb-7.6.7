...
@Repository
public class CompareDataFormulaFormDao {
    private static final String TABLE_NAME = "NR_SINGLE_COMPARE_FORMULAFRM";
    ...
    private static final String ITEM_CHANGETYPE = "CD_ITEM_CHANGETYPE";
    ...
    private static final String ALL_FIELD = "CD_INFOKEY,CD_DATATYPE,CD_SINGLECODE,CD_SINGLETITLE,CD_SINGLEDATA,CD_MATCHKEY,CD_NETKEY,CD_NETCODE,CD_NETTITLE,CD_NETDATA,CD_CHANGETYPE,CD_UPDATETYPE,CD_ITEM_CHANGETYPE,CD_SCHEME_COMPAREKEY,CD_ORDER,CD_UPDATETIME,CD_KEY";
    private static final Function<ResultSet, CompareDataFormulaFormDTO> ENTITY = rs -> {
        CompareDataFormulaFormDTO bean = new CompareDataFormulaFormDTO();
        int index = 1;
        try {
            Object itemChagneType;
            Object updateType;
            ...
            if ((itemChagneType = TransUtil.parseInteger(rs.getObject(index++))) != null) {
                bean.setItemChangeType(CompareChangeType.valueOf(Integer.parseInt(itemChagneType.toString())));
            }
            ...
            throw new RuntimeException("Read CompareDataFormulaFormDTO Error.", e);
        }
        return bean;
    };
    @Autowired
    ...
    public List<CompareDataFormulaFormDTO> getByScheme(String infoKey, String fmlSchemeCompareKey) {
    ...
    @Transactional
    public void batchAdd(List<CompareDataFormulaFormDTO> beans) {
        String SQL_INSERT = String.format("INSERT INTO %s (%s) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? ?, ?, ?, ? ?, ?, ?)", TABLE_NAME, ALL_FIELD);
        this.batchSQL(beans, SQL_INSERT);
    }

    @Transactional
    public void batchUpdate(List<CompareDataFormulaFormDTO> sources) {
        String SQL_UPDATE = String.format("UPDATE %s SET %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ? WHERE %s = ?", TABLE_NAME, INFOKEY, DATATYPE, SINGLECODE, SINGLETITLE, SINGLEDATA, MATCHKEY, NETKEY, NETCODE, NETTITLE, NETDATA, CHANGETYPE, UPDATETYPE, ITEM_CHANGETYPE, FMLSCHEME_COMPAREKEY, ORDER, UPDATETIME, KEY);
    ...
    private void batchSQL(List<CompareDataFormulaFormDTO> beans, String SQL_INSERT) {
        int batchSize = 500;
        ArrayList<Object[]> args = new ArrayList<Object[]>(Math.min(beans.size(), batchSize));
        for (int i = 0; i < beans.size(); ++i) {
            CompareDataFormulaFormDTO bean = beans.get(i);
            args.add(new Object[]{bean.getInfoKey(), bean.getDataType() != null ? Integer.valueOf(bean.getDataType().getValue()) : null, bean.getSingleCode(), bean.getSingleTitle(), bean.getSingleData(), bean.getNetKey(), bean.getNetCode(), bean.getNetTitle(), bean.getNetData(), bean.getChangeType() != null ? Integer.valueOf(bean.getChangeType().getValue()) : null, bean.getUpdateType() != null ? Integer.valueOf(bean.getUpdateType().getValue()) : null, bean.getItemChangeType() != null ? Integer.valueOf(bean.getItemChangeType().getValue()) : null, bean.getFmlSchemeCompareKey(), bean.getOrder(), bean.getUpdateTime() != null ? Timestamp.from(bean.getUpdateTime()) : null, bean.getKey()});
            if (i <= 0 || i % batchSize != 0) continue;
