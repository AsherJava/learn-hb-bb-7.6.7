...
@Component
@Deprecated
public class WorkflowRetractQueueListenerDeprecated
extends AbstractWorkflowQueueListener {
    ...
    protected R doMessage(ObjectNode param, String messageId) throws InterruptedException {
        ...
        if (!StringUtils.hasText(bizCode)) {
            return R.error((String)"\u4e1a\u52a1\u7f16\u53f7\u4e0d\u80fd\u4e3a\u7a7a");
        }
        ProcessDTO processDTO = new ProcessDTO();
        processDTO.setBizcode(bizCode);
        final ProcessDO processDO = this.vaWorkflowProcessService.get(processDTO);
        if (processDO == null) {
            return R.error((String)"\u6d41\u7a0b\u6ca1\u6709\u63d0\u4ea4\u6216\u8005\u5df2\u7ecf\u5ba1\u6279\u7ed3\u675f");
        }
        final String processInstanceId = processDO.getId();
        final R r = R.ok();
        Runnable retractProcessRunnable = new Runnable(){

            @Override
            public void run() {
                WorkflowModel workflowModel = WorkflowRetractQueueListenerDeprecated.this.getModel(processDO.getDefinekey(), processDO.getDefineversion().longValue());
