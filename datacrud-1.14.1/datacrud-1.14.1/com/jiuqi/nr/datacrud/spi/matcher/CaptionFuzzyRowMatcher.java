...
@Component
@Order(value=50)
public class CaptionFuzzyRowMatcher
    ...
    public IEntityRow match(MatchSource matchSource, MatchValue matchValue) {
        ...
        if (bizKeyIndex >= 0 && StringUtils.hasLength(entityKey = innerValues.get(bizKeyIndex))) {
            ...
            for (IEntityRow row : allRows2) {
                if (!row.getEntityKeyData().toUpperCase().contains(entityKey)) continue;
                entityRow = row;
                break;
            }
        }
        if (entityRow == null && (codeIndex = captionCodes.indexOf(codeField.getCode())) >= 0 && StringUtils.hasLength(codeValue = innerValues.get(codeIndex))) {
            ...
            for (IEntityRow row : allRows) {
                if (!row.getCode().toUpperCase().contains(codeValue)) continue;
                entityRow = row;
                break;
            }
        }
        if (entityRow == null && (nameIndex = captionCodes.indexOf(nameField.getCode())) >= 0 && StringUtils.hasLength(nameValue = innerValues.get(nameIndex))) {
