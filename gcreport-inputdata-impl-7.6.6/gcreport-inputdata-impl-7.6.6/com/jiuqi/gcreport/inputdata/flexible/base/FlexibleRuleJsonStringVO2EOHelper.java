...
public class FlexibleRuleJsonStringVO2EOHelper {
    private UnionRuleVO unionRuleVO;

    private FlexibleRuleJsonStringVO2EOHelper() {
    }

    public static FlexibleRuleJsonStringVO2EOHelper newInstance(UnionRuleVO unionRuleVO) {
        FlexibleRuleJsonStringVO2EOHelper helper = new FlexibleRuleJsonStringVO2EOHelper();
    ...
    private Map<String, Object> handleSaveOneFetchSetting(Map<String, Object> oldFetchConfig, int fetchConfigGroupIndex) {
        ...
        String associatedSubjectStr = "associatedSubject";
        ArrayList<String> associatedSubjectListStr = new ArrayList<String>();
        if (!org.springframework.util.StringUtils.isEmpty(oldFetchConfig.get(associatedSubjectStr))) {
            List associatedSubjectList = (List)JsonUtils.readValue((String)JsonUtils.writeValueAsString((Object)oldFetchConfig.get("associatedSubject")), (TypeReference)new TypeReference<List<GcBaseDataVO>>(){});
            for (int i = 0; i < associatedSubjectList.size(); ++i) {
                associatedSubjectListStr.add(((GcBaseDataVO)associatedSubjectList.get(i)).getCode());
            }
        }
        oldFetchConfig.put("associatedSubject", associatedSubjectListStr);
        String debit = "\u501f\u65b9";
        String credit = "\u8d37\u65b9";
