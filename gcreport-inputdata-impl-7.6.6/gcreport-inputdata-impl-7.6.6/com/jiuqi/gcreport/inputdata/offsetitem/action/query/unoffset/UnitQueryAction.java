    ...
    public Object execute(GcOffsetExecutorVO gcOffsetExecutorVO) {
        ...
        try {
            ...
            if (this.queryActionUtils.isUnitTreeSort(FilterMethodEnum.UNIT.getCode(), queryParamsVO.getDataSourceCode(), GcOffsetItemNotOffsetPageImpl.newInstance().getPageCode())) {
                if (queryParamsVO.getPageNum() == -1 || queryParamsVO.getPageSize() == -1) {
                    pagination = this.gcOffsetAppInputDataService.listUnOffsetRecords(queryParamsVO, sql, params);
                    List unOffsetDatas = this.queryActionUtils.sortListByUnitTree(pagination.getContent(), queryParamsVO);
                    pagination.setContent(unOffsetDatas);
                } else {
                    queryParamsVO.setPageNum(-1);
                    queryParamsVO.setPageSize(-1);
                    pagination = this.gcOffsetAppInputDataService.listUnOffsetRecords(queryParamsVO, sql, params);
                    List unOffsetDatas = this.queryActionUtils.sortListByUnitTree(pagination.getContent(), queryParamsVO).stream().skip((long)(pageNum - 1) * (long)pageSize).limit(pageSize).collect(Collectors.toList());
                    pagination.setContent(unOffsetDatas);
                }
            } else {
                pagination = this.gcOffsetAppInputDataService.listUnOffsetRecords(queryParamsVO, sql, params);
