...
@RealTimeJob(group="NOTIFICATION_ASYNC_TASK", groupTitle="\u6d41\u7a0b2.0 \u901a\u77e5\u4e8b\u4ef6\u6279\u91cf\u53d1\u9001\u6d88\u606f\u6267\u884c\u4efb\u52a1", subject="\u62a5\u8868")
public class NotificationBatchTaskExecutor
extends NpRealTimeTaskExecutor {
    public void executeWithNpContext(JobContext jobContext) throws JobExecutionException {
        ...
        RealTimeTaskMonitor asyncTaskMonitor = new RealTimeTaskMonitor(taskId, "ASYNC_TASK_BATCH_NOTIFICATION", jobContext);
        if (Objects.nonNull(params) && Objects.nonNull(this.getArgs())) {
            ...
            try {
                ...
                List businessObjectList = (List)objectMapper.readValue(execParam.get("businessObjectList").toString(), (TypeReference)new TypeReference<List<Object>>(){});
                WorkflowObjectType workflowObjectType = workflowSettingsService.queryTaskWorkflowObjectType(envParam.getTaskKey());
                List<Object> businessObjects = workflowObjectType.equals((Object)WorkflowObjectType.FORM) ? businessObjectList.stream().map(e -> (IFormObject)objectMapper.convertValue(e, IFormObject.class)).collect(Collectors.toList()) : (workflowObjectType.equals((Object)WorkflowObjectType.FORM_GROUP) ? businessObjectList.stream().map(e -> (IFormGroupObject)objectMapper.convertValue(e, IFormGroupObject.class)).collect(Collectors.toList()) : businessObjectList.stream().map(e -> (IDimensionObject)objectMapper.convertValue(e, IDimensionObject.class)).collect(Collectors.toList()));
                double progress = 0.0;
                double part_progress = 1.0 / (double)eventJsonConfig.keySet().size();
                for (String messageType : eventJsonConfig.keySet()) {
                    ...
                    Map<String, List<IBusinessObject>> receiverMap = notificationSendUtil.getReceiverSet(envParam, messageConfig, businessObjects, isTodoEnabled);
                    double increaseProgress = part_progress / (double)receiverMap.size();
                    for (Map.Entry<String, List<IBusinessObject>> entry : receiverMap.entrySet()) {
                        String receiver = entry.getKey();
                        List<IBusinessObject> userBusinessObjects = entry.getValue();
                        MessageRelevantInfo relevantInfo = notificationSendUtil.buildVaribleReplaceMap(envParam, userBusinessObjects, receiver);
                        ...
                        dto.setReceiveUserIds(Collections.singletonList(receiver));
                        dto.setTitle(messageType.equals(MessageType.EMAIL.code) ? messageInfo.getSubject() : messageInfo.getTitle());
                ...
                LoggerFactory.getLogger(((Object)((Object)this)).getClass()).error(e2.getMessage(), e2);
