 ...
 *  com.jiuqi.nr.query.querymodal.QueryModalTreeNode
 ...
 */
package com.jiuqi.nr.zbselector.web;
...
import com.jiuqi.nr.zbselector.service.impl.FieldSelectHelper;
import com.jiuqi.nr.zbselector.web.vo.QueryModalTreeNodeVO;
...
@RestController
@RequestMapping(value={"/zb-selector-design"})
public class IFieldSelectDesignController {
    ...
    @Autowired
    private IZBSelectorDesignService zbSelectorDesignService;

    @RequiresPermissions(value={"nr:zbselector:griddata"})
    @RequestMapping(value={"/fieldselect-design-griddata"}, method={RequestMethod.POST})
    public String getFormData(@RequestBody FieldSelectFormFilter fieldSelectFormFilter) {
        try {
            FieldSelectHelper helper = new FieldSelectHelper();
            FieldSelectFormData formData = helper.getDesignFormData(fieldSelectFormFilter);
    ...
    private ITree<QueryModalTreeNode> getAllRegionsInForm(FormDefine form) {
        try {
            QueryModalTreeNode node = QueryModalTreeNode.buildTreeNodeData((String)form.getKey(), (String)form.getTitle(), (Date)form.getUpdateTime(), (int)3);
            ...
            for (DataRegionDefine regionDefine : listRegionDefine) {
                node = QueryModalTreeNode.buildTreeNodeData((String)regionDefine.getKey(), (String)regionDefine.getTitle(), (Date)regionDefine.getUpdateTime(), (int)4);
    ...
    private ITree<QueryModalTreeNode> getAllFormsInGroup(FormGroupDefine group, TreeLoadLevel loadLevel) {
        try {
            ...
            QueryModalTreeNode node = QueryModalTreeNode.buildTreeNodeData((String)group.getKey(), (String)group.getTitle(), (Date)group.getUpdateTime(), (int)2);
            ...
            if (loadLevel.equals((Object)TreeLoadLevel.DIRECTSECOND)) {
                ...
                if (listFormDefine != null) {
                    ...
                    for (FormDefine formDefine : listFormDefine) {
                        node = QueryModalTreeNode.buildTreeNodeData((String)formDefine.getKey(), (String)formDefine.getTitle(), (Date)formDefine.getUpdateTime(), (int)3);
    ...
    private ITree<QueryModalTreeNode> getAllGroupsInScheme(FormSchemeDefine scheme, TreeLoadLevel loadLevel) {
        try {
            ...
            QueryModalTreeNode node = QueryModalTreeNode.buildTreeNodeData((String)scheme.getKey(), (String)scheme.getTitle(), (Date)scheme.getUpdateTime(), (int)1);
    ...
    private ITree<QueryModalTreeNode> getAllSchemeInTask(TaskDefine task) {
        try {
            QueryModalTreeNode node = QueryModalTreeNode.buildTreeNodeData((String)task.getKey(), (String)task.getTitle(), (Date)task.getUpdateTime(), (int)0);
            ...
            for (FormSchemeDefine scheme : formSchemes) {
                node = QueryModalTreeNode.buildTreeNodeData((String)scheme.getKey(), (String)scheme.getTitle(), (Date)scheme.getUpdateTime(), (int)1);
    ...
    public String queryTreeData(String taskid, String curSchemeKey) {
        try {
            ...
            for (FormSchemeDefine scheme : formSchemes) {
                ...
                List groups = this.runTimeAuthViewController.getAllFormGroupsInFormScheme(scheme.getKey());
                for (FormGroupDefine group : groups) {
                    QueryModalTreeNode node = QueryModalTreeNode.buildTreeNodeData((String)group.getKey(), (String)group.getTitle(), (Date)group.getUpdateTime(), (int)0);
                    ITree treeLevel3 = new ITree((INode)node);
                    List listFormDefine = null;
                    listFormDefine = this.runTimeAuthViewController.getAllFormsInGroup(group.getKey());
                    if (listFormDefine != null) {
                        for (FormDefine formDefine : listFormDefine) {
                            node = QueryModalTreeNode.buildTreeNodeData((String)formDefine.getKey(), (String)formDefine.getTitle(), (Date)formDefine.getUpdateTime(), (int)1);
                            node.setParentId(group.getKey());
    ...
    private List<ITree<QueryModalTreeNode>> tableTreeNode(QueryModalTreeNodeVO parentVO) {
        ArrayList<ITree<QueryModalTreeNode>> tree = new ArrayList<ITree<QueryModalTreeNode>>();
        String parentId = parentVO.getKey();
        try {
            List tables;
            int leval = parentVO.getType();
            List tableGroups = this.iRuntimeDataSchemeService.getDataGroupByParent(parentId);
            if (tableGroups != null) {
                tableGroups.forEach(group -> {
                    QueryModalTreeNode node = QueryModalTreeNode.buildTreeNodeData((String)group.getKey(), (String)group.getTitle(), null, (int)(leval + 1));
            ...
            if ((tables = this.iRuntimeDataSchemeService.getDataTableByGroup(parentId)) != null) {
                tables.forEach(table -> {
                    QueryModalTreeNode node = QueryModalTreeNode.buildTreeNodeData((String)table.getKey(), (String)this.getTitle((DataTable)table), null, (int)(leval + 1));
                    node.setParentId(parentId);
                    ITree childNode = new ITree((INode)node);
    ...
    public String queryChildTreeData(@RequestBody QueryModalTreeNodeVO treeNodeVO) {
        try {
            List<Object> treeNodes = new ArrayList();
            treeNodes = this.tableTreeNode(treeNodeVO);
            LogHelper.info((String)LogModuleEnum.NRQUERY.getTitle(), (String)"\u8bbe\u8ba1\u671f-\u83b7\u53d6\u6307\u6807\u5206\u7ec4\u5b50\u8282\u70b9\u6570\u636e", (String)("\u6210\u529f\uff0c\u8282\u70b9Id:" + treeNodeVO.getKey()));
            return this.objectMapper.writeValueAsString(treeNodes);
        }
        catch (Exception e) {
    ...
    public String queryAllForm(String formschemekey) {
        try {
            ...
            if (formschemekey != null) {
                ...
                if (listFormDefine != null) {
                    for (FormDefine formDefine : listFormDefine) {
                        QueryModalTreeNode node = QueryModalTreeNode.buildTreeNodeData((String)formDefine.getKey(), (String)formDefine.getTitle(), (Date)formDefine.getUpdateTime(), (int)3);
            ...
            if (formid != null) {
                ...
                if (listDataRegionDefine != null) {
                    for (DataRegionDefine dataRegionDefine : listDataRegionDefine) {
                        QueryModalTreeNode node = QueryModalTreeNode.buildTreeNodeData((String)dataRegionDefine.getKey(), (String)dataRegionDefine.getTitle(), (Date)dataRegionDefine.getUpdateTime(), (int)4);
                        ITree tree = new ITree((INode)node);
                        tree.setLeaf(true);
                        groups.add(tree);
                    }
            ...
            if (dataregionid != null) {
                ...
                if (listFieldDefine != null) {
                    for (FieldDefine fieldDefine : listFieldDefine) {
                        QueryModalTreeNode node = QueryModalTreeNode.buildTreeNodeData((String)fieldDefine.getKey(), (String)fieldDefine.getTitle(), null, (int)5);
                        ITree tree = new ITree((INode)node);
                        tree.setLeaf(true);
                        groups.add(tree);
                    }
    ...
    public List<DesignTaskDefine> getAllTaskDefines(String taskCode) {
        if (!StringUtils.hasLength(taskCode)) {
    ...
    public String getFieldsByRegionKey(@RequestBody QueryModalTreeNodeVO treeNodeVO) {
        try {
            FieldSelectHelper fieldHelper = new FieldSelectHelper(null, null);
            Grid2Data fields = fieldHelper.getChildGridByTable(treeNodeVO.getKey(), treeNodeVO.getTitle());
            LogHelper.info((String)LogModuleEnum.NRQUERY.getTitle(), (String)"\u8bbe\u8ba1\u671f-\u83b7\u53d6\u6811\u8282\u70b9\u4e0b\u7684\u6307\u6807\u5217\u8868", (String)("\u6210\u529f\uff0cnodeKey\uff1a" + treeNodeVO.getKey()));
            return this.objectMapper.writeValueAsString((Object)fields);
        }
        catch (Exception ex) {
            logger.error(ex.getMessage(), ex);
    ...
    @RequestMapping(value={"/fieldselect-design-gotPeriodObjByTask"}, method={RequestMethod.GET})
    public String getPeriodObjByTask(String taskKey, String period) {
        return this.zbSelectorDesignService.getPeriodObjByTask(taskKey, period);
    }

    @RequestMapping(value={"/fieldselect-design-gottasktrees"}, method={RequestMethod.GET})
    public List<ITree<TaskTreeNode>> getAllTaskTrees(String[] taskTypes, boolean complete) {
        return this.zbSelectorDesignService.getRootTaskTreeNodes(complete);
    }

    @GetMapping(value={"/fieldselect-design-getTaskLinksByFormScheme"})
    public List<TaskLinkImpl> getTaskLinksByFormScheme(String formSchemeKey) {
        return this.zbSelectorDesignService.getTaskLinksByFormScheme(formSchemeKey);
    }
