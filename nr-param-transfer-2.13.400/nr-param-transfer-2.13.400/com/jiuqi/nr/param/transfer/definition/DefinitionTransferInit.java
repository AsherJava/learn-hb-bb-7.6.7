...
import com.jiuqi.nr.param.transfer.formScheme.FormSchemeTransferFactory;
import com.jiuqi.nr.param.transfer.task.TaskTransferFactory;
...
@Component
public class DefinitionTransferInit
implements ApplicationInitialization {
    private static final Logger LOGGER = LoggerFactory.getLogger(DefinitionTransferInit.class);
    @Value(value="${jiuqi.nr.sync.paramPackage.task:false}")
    private boolean paramPackage;
    @Autowired
    private DefinitionTransferFactory definitionTransferFactory;
    @Autowired
    private TaskTransferFactory taskTransferFactory;
    @Autowired
    private FormSchemeTransferFactory formSchemeTransferFactory;
    ...
    public void init(boolean isSysTenant) {
        try {
            ...
            TransferFactoryManager.getInstance().register((TransferFactory)this.taskTransferFactory);
            TransferFactoryManager.getInstance().register((TransferFactory)this.definitionTransferFactory);
            TransferFactoryManager.getInstance().register((TransferFactory)this.formSchemeTransferFactory);
            LOGGER.info("\u6ce8\u518c\u5de5\u5382- \u4efb\u52a1\u8bbe\u8ba1\u5b8c\u6210");
        }
        catch (TransferException e) {
            LOGGER.error("\u4efb\u52a1\u8bbe\u8ba1\u5bfc\u5165\u5bfc\u51fa\u6a21\u5757\u6ce8\u518c\u5931\u8d25", e);
        }
    }
}
