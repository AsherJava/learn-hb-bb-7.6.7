...
@Component
public class Default_1_0_ProcessRuntimeService
implements IProcessRuntimeService {
    ...
    public boolean existsInstance(String taskKey) {
        ...
        for (FormSchemeDefine formSchemeDefine : formSchemeDefines) {
            if (!this.isExistProcessInstance(formSchemeDefine)) continue;
            return true;
        }
        return false;
    }
    ...
    private boolean isExistProcessInstance(FormSchemeDefine formSchemeDefine) {
        String sql = String.format("SELECT MDCODE FROM SYS_UP_ST_%s", formSchemeDefine.getFormSchemeCode());
        List items = this.jdbcTemplate.queryForList(sql, String.class);
