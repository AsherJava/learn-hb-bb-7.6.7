 ...
 *  com.jiuqi.gcreport.onekeymerge.vo.GcCalcResultVO
...
import com.jiuqi.gcreport.onekeymerge.vo.GcCalcResultVO;
...
@Component
public class GcDoCalcTaskImpl
implements GcCenterTask {
    private static final Logger logger = LoggerFactory.getLogger(GcDoCalcTaskImpl.class);
    @Autowired
    ...
    private ReturnObject doCalc(GcActionParamsVO paramsVO) {
        ...
        try {
            ...
            taskResultEO.setTaskCode(TaskTypeEnum.CALC.getCode());
            ...
            for (GcOrgCacheVO org : orgs) {
                GcCalcResultVO gcCalcResultVO = new GcCalcResultVO();
                gcCalcResultVO.setStartTime(new Date());
                ...
                if (this.onekeyMergeService.getStopOrNot(paramsVO.getTaskLogId().toString())) {
                    throw new BusinessRuntimeException("\u624b\u52a8\u505c\u6b62,\u4efb\u52a1\u4e2d\u65ad");
                }
                boolean containsKey = filterMap.containsKey(org.getId());
                if (!containsKey) {
                    taskLog.writeWarnLog(org.getTitle() + "\u5df2\u7ecf\u4e0a\u62a5\u6216\u9001\u5ba1\uff0c\u8df3\u8fc7", null);
                    restList.add((GcBaseTaskStateVO)this.buildCalcResultVO(gcCalcResultVO, org, "\u8df3\u8fc7", null));
                    continue;
                }
                ...
                this.buildCalcResultVO(gcCalcResultVO, org, successFlag ? "\u6210\u529f" : "\u5931\u8d25", gcCalcResultVO.getStartTime().getTime());
                gcCalcResultVO.setEndTime(new Date());
                gcCalcResultVO.setPrcessResult(String.join((CharSequence)";\n", result));
                restList.add((GcBaseTaskStateVO)gcCalcResultVO);
                if (successFlag) {
                    taskLog.writeInfoLog(org.getTitle() + "\u5408\u5e76\u8ba1\u7b97\u6210\u529f: ", null);
                    continue;
                }
                throw new BusinessRuntimeException(org.getTitle() + "\u5408\u5e76\u8ba1\u7b97\u5931\u8d25:");
            }
            ...
            taskLog.writeErrorLog("\u5408\u5e76\u8ba1\u7b97\u51fa\u73b0\u5f02\u5e38\uff0c\u7a0b\u5e8f\u7ec8\u6b62\uff1a" + e.getMessage(), Float.valueOf(taskLog.getProcessPercent()));
            logger.debug("\u5408\u5e76\u8ba1\u7b97\u51fa\u73b0\u5f02\u5e38,\u7a0b\u5e8f\u7ec8\u6b62:" + e.getMessage(), e);
        }
        finally {
            taskLog.setFinish(true);
    ...
    private GcCalcResultVO buildCalcResultVO(GcCalcResultVO vo, GcOrgCacheVO org, String msg, Long start) {
