...
@Component
public class CheckAllNonLeaves
implements IRowChecker {
    public static final String KEYWORD = "#check-all-non-leaves";
    @Resource
    ...
    public String getShowText() {
        return this.i18nHelper.getMessage(UnitSelectorI18nKeys.CHECK_ALL_NON_LEAVES.i18nKey, "\u6240\u6709\u975e\u53f6\u5b50\u8282\u70b9");
    }
    ...
    public IRowCheckExecutor getExecutor(IUnitTreeContext ctx) {
        return (values, entityRowsProvider) -> {
            List<IEntityRow> allNonLeavesRows = entityRowsProvider.getAllNonLeaveRows();
            if (allNonLeavesRows != null && !allNonLeavesRows.isEmpty()) {
                return allNonLeavesRows.stream().map(IEntityItem::getEntityKeyData).collect(Collectors.toList());
            }
