...
@Component(value="#check-all-children-v2")
public class CheckAllChildren
implements IRowChecker {
    public static final String KEYWORD = "#check-all-children";
    @Resource
    ...
    public String getShowText() {
        return this.i18nHelper.getMessage(UnitSelectorI18nKeys.CHECK_ALL_CHILDREN.i18nKey, "\u52fe\u9009\u6240\u6709\u4e0b\u7ea7");
    }
    ...
    public IRowCheckExecutor getExecutor(IUnitTreeContext ctx) {
        return (values, entityRowsProvider) -> {
            ...
            List<IEntityRow> allChildRows;
            List<Map<String, String>> valueMaps;
            if (null != values && null != (valueMaps = values.getValues()) && null != (allChildRows = entityRowsProvider.getAllChildRows(checkValues = valueMaps.stream().map(map -> (String)map.get("value")).collect(Collectors.toList()))) && !allChildRows.isEmpty()) {
                return allChildRows.stream().map(IEntityItem::getEntityKeyData).collect(Collectors.toList());
            }
