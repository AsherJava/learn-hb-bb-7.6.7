...
@Component(value="#check-direct-children-with-shift-v2")
public class CheckDirectChildrenWithShift
implements IRowChecker {
    public static final String KEYWORD = "#check-direct-children-with-shift";
    @Resource
    ...
    public String getShowText() {
        return this.i18nHelper.getMessage(UnitSelectorI18nKeys.CHECK_DIRECT_CHILDREN_WITH_SHIFT.i18nKey, "Shift\u52fe\u9009\u76f4\u63a5\u4e0b\u7ea7");
    }
    ...
    public IRowCheckExecutor getExecutor(IUnitTreeContext ctx) {
        return (values, entityRowsProvider) -> {
            ...
            List<IEntityRow> childRowsAndSelf;
            List<Map<String, String>> valueMaps;
            if (null != values && null != (valueMaps = values.getValues()) && null != (childRowsAndSelf = entityRowsProvider.getChildRowsAndSelf(checkValues = valueMaps.stream().map(map -> (String)map.get("value")).collect(Collectors.toList()))) && !childRowsAndSelf.isEmpty()) {
                return childRowsAndSelf.stream().map(IEntityItem::getEntityKeyData).collect(Collectors.toList());
            }
