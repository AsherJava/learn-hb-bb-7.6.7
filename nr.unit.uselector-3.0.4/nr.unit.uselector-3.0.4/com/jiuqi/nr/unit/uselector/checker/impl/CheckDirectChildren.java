...
@Component(value="#check-direct-children-v2")
public class CheckDirectChildren
implements IRowChecker {
    public static final String KEYWORD = "#check-direct-children";
    @Resource
    ...
    public String getShowText() {
        return this.i18nHelper.getMessage(UnitSelectorI18nKeys.CHECK_DIRECT_CHILDREN.i18nKey, "\u52fe\u9009\u76f4\u63a5\u4e0b\u7ea7");
    }
    ...
    public IRowCheckExecutor getExecutor(IUnitTreeContext ctx) {
        return (values, entityRowsProvider) -> {
            ...
            if (null != values && null != (valueMaps = values.getValues()) && null != (childRows = entityRowsProvider.getChildRows(checkValues = valueMaps.stream().map(map -> (String)map.get("value")).collect(Collectors.toList()))) && !childRows.isEmpty()) {
                return childRows.stream().map(IEntityItem::getEntityKeyData).collect(Collectors.toList());
            }
