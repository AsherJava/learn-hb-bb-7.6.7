 ...
 */
package com.jiuqi.bde.bizmodel.execute.model.cfl.fetch;
...
import com.jiuqi.bde.bizmodel.execute.model.cfl.AbstractCflBalanceModelExecute;
...
public class CflBalanceModelExecute
extends AbstractCflBalanceModelExecute {
    @Autowired
    ...
    protected ModelExecuteContext rewriteContext(FetchTaskContext fetchTaskContext, FetchSettingCacheKey fetchSettingCacheKey) {
        ...
        ArrayList<Dimension> assTypeList = new ArrayList<Dimension>(dimTypeArr.length);
        for (String str : dimTypeArr) {
            Dimension dimension = new Dimension(str, Boolean.valueOf(false));
            assTypeList.add(dimension);
        }
        executeContext.setAssTypeList(assTypeList);
        return executeContext;
    }
    ...
    private BigDecimal sumByData(BaseDataMatchCondi matchCondi, ExecuteSettingVO fetchSetting, Map<String, Map<String, List<AssBalanceData>>> balanceData) {
        ...
        SumTypeEnum sumType = SumTypeEnum.fromCode((String)fetchSetting.getSumType());
        ...
        for (Map.Entry<String, Map<String, List<AssBalanceData>>> balanceKmEntry : balanceData.entrySet()) {
            ...
            for (Map.Entry<String, List<AssBalanceData>> balanceAssistEntry : balanceKmEntry.getValue().entrySet()) {
                for (AssBalanceData assBalance : balanceAssistEntry.getValue()) {
                    ...
                    this.caclVal(sumType, fetchType, sumVal, assBalance);
                }
            }
        }
        for (BigDecimal val : sumVal.values()) {
            zbValue = NumberUtils.sum((BigDecimal)zbValue, (BigDecimal)this.cflSumVal(sumType, fetchType, val));
        }
        return zbValue;
    }
    ...
    private Map<String, Map<String, List<AssBalanceData>>> convertResult(BalanceCondition condi, FetchData cache) {
        ...
        block0: for (Object[] rowData : cache.getRowDatas()) {
            ...
            if (result.get(subjectCode) == null) {
                result.put(subjectCode, new HashMap(512));
            }
