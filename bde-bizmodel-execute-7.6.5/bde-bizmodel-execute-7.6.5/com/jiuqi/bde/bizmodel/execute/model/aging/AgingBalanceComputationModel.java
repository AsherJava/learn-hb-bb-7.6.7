...
public class AgingBalanceComputationModel
extends AbstractFinComputationModel {
    @Autowired
    private AgingDataModel dataModel;
    ...
    public String getName() {
        return ComputationModelEnum.AGINGBALANCE.getName();
    }

    public String getIcon() {
        return "#icon-a-16_TY_E_GC_zhanglingyuebiao";
    }
    ...
    public List<SelectOptionVO> getFetchTypes() {
        ...
        fetchTypeEnums.add(FetchTypeEnum.HXNC);
        fetchTypeEnums.add(FetchTypeEnum.HXYE);
        if (this.enableOrgnCurrency().booleanValue()) {
            fetchTypeEnums.add(FetchTypeEnum.WHXNC);
            fetchTypeEnums.add(FetchTypeEnum.WHXYE);
        }
    ...
    public List<ColumnDefineVO> getFixedFields() {
        ...
        fixedFields.add(new ColumnDefineVO(FetchFixedFieldEnum.DIMTYPE.getCode(), GcI18nUtil.getMessage((String)"bde.fixe.column.agingDimType"), true, "MULTIPLE", "SELECT", null, this.listDimensionOptionByBizDataModelCode(this.getBizDataCode())));
        ...
        ArrayList<SelectOptionVO> agingRangeTypes = new ArrayList<SelectOptionVO>();
        agingRangeTypes.add(new SelectOptionVO(AgingPeriodTypeEnum.Y.getCode(), AgingPeriodTypeEnum.Y.getName()));
        agingRangeTypes.add(new SelectOptionVO(AgingPeriodTypeEnum.M.getCode(), AgingPeriodTypeEnum.M.getName()));
        ColumnDefineVO agingRangeType = new ColumnDefineVO(FetchFixedFieldEnum.AGINGRANGETYPE.getCode(), GcI18nUtil.getMessage((String)"bde.fixe.column.agingRangeType"), true, "SINGLE", "SELECT", null, agingRangeTypes);
        agingRangeType.setWidth(Integer.valueOf(80));
        fixedFields.add(agingRangeType);
        ColumnDefineVO agingRangeStart = new ColumnDefineVO(FetchFixedFieldEnum.AGINGRANGESTART.getCode(), GcI18nUtil.getMessage((String)"bde.fixe.column.agingRangeStart"), true, "SINGLE", "INPUTNUMBER", null, null);
        agingRangeStart.setWidth(Integer.valueOf(120));
        fixedFields.add(agingRangeStart);
        ColumnDefineVO agingRangeEnd = new ColumnDefineVO(FetchFixedFieldEnum.AGINGRANGEEND.getCode(), GcI18nUtil.getMessage((String)"bde.fixe.column.agingRangeEnd"), true, "SINGLE", "INPUTNUMBER", null, null);
        agingRangeEnd.setWidth(Integer.valueOf(120));
        fixedFields.add(agingRangeEnd);
        return fixedFields;
    }
    ...
    public String getOptimizeRuleGroup(FetchSettingVO fetchSettingDes) {
        ...
        if (FetchTypeEnum.HXNC.getCode().equals(fetchSettingDes.getFetchType()) || FetchTypeEnum.WHXNC.getCode().equals(fetchSettingDes.getFetchType())) {
            dimCombs.put("agingFetchType", FetchTypeEnum.NC.getCode());
        } else {
            dimCombs.put("agingFetchType", FetchTypeEnum.YE.getCode());
        }
        dimCombs.put(FetchFixedFieldEnum.AGINGRANGETYPE.getCode(), fetchSettingDes.getAgingRangeType());
        dimCombs.put(FetchFixedFieldEnum.AGINGRANGESTART.getCode(), fetchSettingDes.getAgingRangeStart());
        dimCombs.put(FetchFixedFieldEnum.AGINGRANGEEND.getCode(), fetchSettingDes.getAgingRangeEnd());
    ...
    public String getMemo(FetchSettingVO fetchSetting) {
        ...
        memo.append(FetchFixedFieldEnum.AGINGRANGETYPE.getName()).append(AgingPeriodTypeEnum.fromCode((String)fetchSetting.getAgingRangeType()).getName()).append(",");
        memo.append(FetchFixedFieldEnum.AGINGRANGESTART.getName()).append(fetchSetting.getAgingRangeStart()).append(",");
        memo.append(FetchFixedFieldEnum.AGINGRANGEEND.getName()).append(fetchSetting.getAgingRangeEnd()).append(",");
