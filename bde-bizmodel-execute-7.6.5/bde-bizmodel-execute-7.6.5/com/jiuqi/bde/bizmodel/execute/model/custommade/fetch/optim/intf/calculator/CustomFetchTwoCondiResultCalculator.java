...
public class CustomFetchTwoCondiResultCalculator
extends AbstractCustomFetchResultCalculator {
    public CustomFetchTwoCondiResultCalculator(CustomBizModelDTO bizModel, AbstractCustomFetchResult fetchResult) {
        super(bizModel, fetchResult);
    }

    @Override
    public CustomFetchCalcResult doCalc(CustomFetchExecuteSetting fetchSetting) {
        CustomFetchTwoCondiResult fetchResult = (CustomFetchTwoCondiResult)this.getFetchResult();
        ...
        for (Map.Entry<String, CustomCondition> custCondiEntry : fetchSetting.getCustomConditionMap().entrySet()) {
            if (fetchResult.getCondiKey1().equals(custCondiEntry.getKey()) || fetchResult.getCondiKey2().equals(custCondiEntry.getKey())) continue;
            directFilterMap.put(custCondiEntry.getKey(), custCondiEntry.getValue());
        }
        CustomFetchCalcStepResult setpResult = null;
        for (Map.Entry<String, Map<String, List<Object[]>>> condiKeyEntry1 : fetchResult.getDataMap().entrySet()) {
            if (!this.matchRule(fetchSetting.getCustomConditionMap().get(fetchResult.getCondiKey1()), condiKeyEntry1.getKey())) continue;
            for (Map.Entry<String, List<Object[]>> condiKeyEntry2 : condiKeyEntry1.getValue().entrySet()) {
                if (!this.matchRule(fetchSetting.getCustomConditionMap().get(fetchResult.getCondiKey2()), condiKeyEntry2.getKey())) continue;
                block3: for (Object[] row : condiKeyEntry2.getValue()) {
