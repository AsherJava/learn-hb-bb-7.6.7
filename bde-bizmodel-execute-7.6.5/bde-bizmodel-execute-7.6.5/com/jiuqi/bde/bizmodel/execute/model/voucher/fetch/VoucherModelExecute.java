 ...
 */
package com.jiuqi.bde.bizmodel.execute.model.voucher.fetch;
...
@Component
public class VoucherModelExecute
extends AbstractGenericModelExecute {
    @Autowired
    private IVoucherDataLoaderGather loaderGather;
    ...
    protected List<FetchResult> doExecute(ModelExecuteContext executeContext, List<ExecuteSettingVO> fetchSettingList, StringBuilder logContent) {
        ...
        AbstractVoucherDataLoader loader = this.loaderGather.getLoader(condi.getOrgMapping().getPluginType(), this.getComputationModelCode());
        Map<String, Map<String, List<AssBalanceData>>> balanceData = this.convertResult(condi, (FetchData)loader.loadData(condi));
        ...
        for (ExecuteSettingVO executeSettingVO : fetchSettingList) {
            ...
            result = new FetchResult(executeSettingVO);
            for (Map.Entry<String, Map<String, List<AssBalanceData>>> balanceKmEntry : balanceData.entrySet()) {
                ...
                for (Map.Entry<String, List<AssBalanceData>> balanceAssistEntry : balanceKmEntry.getValue().entrySet()) {
                    block3: for (AssBalanceData assBalance : balanceAssistEntry.getValue()) {
