...
import com.jiuqi.bde.bizmodel.execute.datamodel.gather.IInvestBillDataLoaderGather;
...
@Component
public class InvestBillDataLoaderGather
implements IInvestBillDataLoaderGather {
    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private IBizModelGather bizModelGather;
    private List<AbstractInvestBillDataLoader> loaderList;
    private final Table<String, String, AbstractInvestBillDataLoader> dataModelLoaderTable = TreeBasedTable.create();
    private final Table<String, String, AbstractInvestBillDataLoader> bizModelLoaderTable = TreeBasedTable.create();

    public InvestBillDataLoaderGather(@Autowired(required=false) IBizModelGather bizModelGather, @Autowired(required=false) List<AbstractInvestBillDataLoader> loaderList) {
    ...
    private void init() {
        if (this.loaderList == null) {
            this.loaderList = new ArrayList<AbstractInvestBillDataLoader>();
        }
    ...
    @Override
    public AbstractInvestBillDataLoader getLoader(String pluginType, String computationModelCode) {
        ...
        AbstractInvestBillDataLoader loaderByDataModel = (AbstractInvestBillDataLoader)this.dataModelLoaderTable.get((Object)pluginType, (Object)modelDefineByCode.getBizDataCode());
        if (loaderByDataModel != null) {
            return loaderByDataModel;
        }
        loaderByDataModel = (AbstractInvestBillDataLoader)this.bizModelLoaderTable.get((Object)pluginType, (Object)computationModelCode);
        if (loaderByDataModel != null) {
            return loaderByDataModel;
        }
