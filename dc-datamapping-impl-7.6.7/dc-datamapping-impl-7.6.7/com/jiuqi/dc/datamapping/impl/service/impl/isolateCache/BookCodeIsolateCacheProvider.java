...
@Component
public class BookCodeIsolateCacheProvider
implements FieldMappingBizDataCacheProvider {
    ...
    public static final String CACHE_KEY = "BookCodeFieldMapping";
    private final Logger logger = LoggerFactory.getLogger(this.getClass());

    @Override
    public IsolationStrategy getIsolateType() {
        return IsolationStrategy.BOOKCODE;
    }
    ...
    public Map<String, DataRefMappingCacheDTO> getCache(IsolationParamContext isolateDimension, String dimCode) {
        ...
        return cache.computeIfAbsent(isolateDimension.getBookCode(), k -> new HashMap());
    }
    ...
    public List<DataRefDTO> getBaseMappingCache(IsolationParamContext isolateDimension, String dimCode) {
        ...
        Map dimensionCache = cacheMap.computeIfAbsent(isolateDimension.getBookCode(), k -> new HashMap());
    ...
    public Map<Object, Map<String, DataRefMappingCacheDTO>> getRefDataMap(String schemeCode, String dimCode) {
        List<DataRefDTO> dataRefDTOS = this.isolateFieldMappingDao.loadIsolateAllMapping(IsolationStrategy.BOOKCODE, schemeCode, dimCode);
        ...
        for (DataRefDTO dataRefDTO : dataRefDTOS) {
            ...
            Map innerMap = resultMap.computeIfAbsent(dataRefDTO.get((Object)"ODS_BOOKCODE"), k -> new HashMap());
    ...
    public Set<String> getUnitCodeRange(IsolationParamContext isolateDimension, String dimCode, List<DataRefDTO> refOrgList) {
        return refOrgList.stream().filter(item -> isolateDimension.getBookCode().equals(item.get((Object)"ODS_BOOKCODE"))).map(DataRefDTO::getCode).collect(Collectors.toSet());
    }
}
