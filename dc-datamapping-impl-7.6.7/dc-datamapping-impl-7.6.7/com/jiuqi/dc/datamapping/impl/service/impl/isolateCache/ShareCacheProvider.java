...
@Component
public class ShareCacheProvider
implements FieldMappingBizDataCacheProvider {
    ...
    public static final String CACHE_KEY = "ShareFieldMapping";
    @Autowired
    ...
    public IsolationStrategy getIsolateType() {
        return IsolationStrategy.SHARE;
    }
    ...
    public List<DataRefDTO> getBaseMappingCache(IsolationParamContext isolateDimension, String dimCode) {
        ...
        Cache.ValueWrapper valueWrapper = this.cacheManger.getCache(CACHE_KEY).get(isolateDimension.getSchemeCode() + "|" + dimCode);
        if (ObjectUtils.isEmpty(valueWrapper)) {
            this.getRefDataMap(isolateDimension.getSchemeCode(), dimCode);
            caches = this.isolateFieldMappingDao.loadShareMapping(isolateDimension.getSchemeCode(), dimCode);
        } else {
            caches = new ArrayList(((Map)valueWrapper.get()).values());
        }
        ArrayList<DataRefDTO> result = new ArrayList<DataRefDTO>();
        for (DataRefMappingCacheDTO dataRefMappingCacheDTO : caches) {
            DataRefDTO dataRefDTO = new DataRefDTO();
            dataRefDTO.setId(dataRefMappingCacheDTO.getId());
            dataRefDTO.setCode(dataRefMappingCacheDTO.getCode());
            dataRefDTO.setOdsName(dataRefMappingCacheDTO.getOdsName());
            dataRefDTO.setOdsCode(dataRefMappingCacheDTO.getOdsCode());
    ...
    @Override
    public void loadCache(String schemeCode, String dimension) {
        this.cacheManger.getCache(CACHE_KEY).evict(schemeCode + "|" + dimension);
        this.getRefDataMap(schemeCode, dimension);
    }
    ...
    public Map<String, DataRefMappingCacheDTO> getRefDataMap(String dataSchemeCode, String tableName) {
        return (Map)this.cacheManger.getCache(CACHE_KEY).get(dataSchemeCode + "|" + tableName, () -> Optional.ofNullable(this.isolateFieldMappingDao.loadShareMapping(dataSchemeCode, tableName).stream().collect(Collectors.toMap(DataRefMappingCacheDTO::getOdsCode, e -> e, (k1, k2) -> k2))).orElse(CollectionUtils.newHashMap()));
    }
