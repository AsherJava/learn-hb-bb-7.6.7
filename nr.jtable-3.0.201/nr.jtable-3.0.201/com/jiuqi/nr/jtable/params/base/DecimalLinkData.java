...
public class DecimalLinkData
extends LinkData {
    private static final Logger logger = LoggerFactory.getLogger(DecimalLinkData.class);
    ...
    public DecimalLinkData(DataLinkDefine dataLinkDefine, FieldDefine fieldDefine, FormDefine formDefine) {
    ...
    public DecimalLinkData(DataLinkDefine dataLinkDefine, ColumnModelDefine columnModelDefine, FormDefine formDefine) {
    ...
    public DecimalLinkData(DataLinkDefine dataLinkDefine, FieldDefine fieldDefine) {
        super(dataLinkDefine, fieldDefine);
        this.type = LinkType.LINK_TYPE_DECIMAL.getValue();
        FormatProperties formatProperties = dataLinkDefine.getFormatProperties();
        if (formatProperties == null && fieldDefine != null) {
        ...
        this.showPostfixZero = true;
        this.measureUnit = fieldDefine.getMeasureUnit();
        this.style = DataLinkStyleUtil.getNumberLinkStyle(numberFormatParser, fieldDefine, formatProperties == null, this.formDefine);
        if (formatProperties != null && formatProperties.getFormatType() == 6) {
            this.cstyle = formatProperties.getPattern();
        }
    }

    public DecimalLinkData(DataLinkDefine dataLinkDefine, ColumnModelDefine columnModelDefine) {
        super(dataLinkDefine, columnModelDefine);
        this.type = LinkType.LINK_TYPE_DECIMAL.getValue();
        FormatProperties formatProperties = dataLinkDefine.getFormatProperties();
        NumberFormatParser numberFormatParser = NumberFormatParser.parse((FormatProperties)formatProperties);
        ...
        this.showPostfixZero = true;
        this.measureUnit = columnModelDefine.getMeasureUnit();
        this.style = DataLinkStyleUtil.getNumberLinkStyle(numberFormatParser, columnModelDefine, formatProperties == null, this.formDefine);
        if (formatProperties != null && formatProperties.getFormatType() == 6) {
            this.cstyle = formatProperties.getPattern();
        }
    }
    ...
    public Object getFormatData(AbstractData data, DataFormaterCache dataFormaterCache, JtableContext jtableContext) {
        ...
        try {
            ...
            boolean hasMeasureUnit = true;
            if (StringUtils.isNotEmpty((String)this.measureUnit) && this.measureUnit.indexOf("NotDimession") >= 0 || StringUtils.isEmpty((String)this.measureUnit) && this.formDefine != null && (this.formDefine.getMeasureUnit() == null || this.formDefine.getMeasureUnit().indexOf("NotDimession") >= 0)) {
                hasMeasureUnit = false;
            }
            if (jtableContext.getDecimal() != null && !jtableContext.getDecimal().equals("") && hasMeasureUnit) {
                this.fraction = Integer.valueOf(jtableContext.getDecimal());
            }
            if (this.fraction > 0) {
                formatStr.append(".");
            }
            for (int i = 0; i < this.fraction; ++i) {
                formatStr.append("0");
            }
            Map<String, DecimalFormat> formMatMap = dataFormaterCache.getFormatMap();
            DecimalFormat df = null;
            String format = formatStr.toString();
            if (formMatMap.containsKey(format)) {
                df = formMatMap.get(format);
            } else {
                df = new DecimalFormat(format);
                df.setRoundingMode(RoundingMode.HALF_UP);
                formMatMap.put(format, df);
            }
            String valueData = String.valueOf(floatData);
            if (valueData.contains("E") || valueData.contains("e")) {
                BigDecimal bigDecimal = data.getAsCurrency();
                return df.format(bigDecimal);
            }
            return df.format(new BigDecimal(valueData));
        }
