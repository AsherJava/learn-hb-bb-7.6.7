...
public class SnapshotGradeDataLoader {
    private RegionDataSet regionDataSet;
    private IRegionDataSet crudRegionDataSet;
    private int currIndex;
    public static final int GROUPDATA = 1;
    public static final int DETAILDATA = 2;

    public SnapshotGradeDataLoader(IRegionDataSet crudRegionDataSet, RegionDataSet regionDataSet) {
        this.regionDataSet = regionDataSet;
        this.crudRegionDataSet = crudRegionDataSet;
    }

    public void loadRegionGradeData() {
        if (this.crudRegionDataSet.supportTreeGroup()) {
            this.currIndex = 0;
            while (this.currIndex < this.crudRegionDataSet.getTotalCount()) {
                this.loadRootRowData();
            }
        } else {
            for (int rowIndex = 0; rowIndex < this.crudRegionDataSet.getRowCount(); ++rowIndex) {
