...
extends Function {
    ...
    public InCollection() {
        ...
        this.parameters().add(new Parameter("collectionTable", 6, "\u96c6\u5408\u6570\u636e\u6240\u5728\u8868"));
        this.parameters().add(new Parameter("collectionColumn", 6, "\u96c6\u5408\u6570\u636e\u6240\u5728\u5217", true));
    }
    ...
    public Object evalute(IContext paramIContext, List<IASTNode> paramList) throws SyntaxException {
        ...
        if (set == null) {
            try {
                ...
                Connection conn = context.getQueryParam().getConnection();
                StringBuilder sql = new StringBuilder();
                sql.append(" select ct.").append(columnName).append(" from ").append(tableName).append(" ct");
                try (SqlQueryHelper helper = new SqlQueryHelper();
                     ResultSet rs = helper.executeQuery(conn, sql.toString());){
                    while (rs.next()) {
        ...
        return set != null && set.contains(p0);
    }
