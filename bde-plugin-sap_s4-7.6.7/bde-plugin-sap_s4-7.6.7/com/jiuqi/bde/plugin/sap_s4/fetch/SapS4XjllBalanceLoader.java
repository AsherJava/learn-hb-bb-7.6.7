...
@Component
public class SapS4XjllBalanceLoader
extends BaseXjllDataLoader {
    ...
    protected FetchData queryData(BalanceCondition condi) {
        ...
        sql.append("       SUM(  \n");
        sql.append("       CASE WHEN MASTER.MONAT >= ${PERIOD_START} AND MASTER.MONAT <= ${PERIOD_END} THEN  \n");
        ...
        sql.append("       SUM(  \n");
        sql.append("       CASE WHEN MASTER.MONAT >= 1 AND MASTER.MONAT <= ${PERIOD_END} THEN  \n");
        sql.append("       CASE WHEN RTRIM(SHKZG) = 'S' THEN COALESCE(DMBTR, 0) ELSE 0 END - CASE WHEN RTRIM(SHKZG) = 'H' THEN COALESCE(DMBTR, 0) ELSE 0 END  \n");
        ...
        sql.append(SapS4FetchUtil.buildUnitSql("MASTER.BUKRS", orgMappingType, orgMapping));
        sql.append(SapS4FetchUtil.buildAssistSql("MASTER.BUKRS", "T.KOSTL", orgMappingType, orgMapping));
        sql.append("   AND MASTER.GJAHR = '${YEAR}'  \n");
        sql.append(SapS4FetchUtil.buildUnitSql("T.BUKRS", orgMappingType, orgMapping));
        sql.append(SapS4FetchUtil.buildAssistSql("T.BUKRS", "T.KOSTL", orgMappingType, orgMapping));
        sql.append("   AND T.GJAHR = '${YEAR}'  \n");
        sql.append("   AND T.RSTGR IS NOT NULL  \n");
        sql.append("  GROUP BY T.RSTGR, T.HKONT, MASTER.WAERS ${GROUPFIELD}  \n");
