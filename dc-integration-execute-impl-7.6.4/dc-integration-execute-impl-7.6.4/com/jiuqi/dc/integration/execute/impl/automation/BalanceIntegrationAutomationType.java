...
import com.jiuqi.dc.integration.execute.impl.mq.model.bizdata.BizBalanceInitConvertTaskHandler;
...
@AutomationType(category="datacenter", id="datacenter-integration-auto", title="\u6570\u636e\u6574\u5408", icon="icon-64-xxx")
@CommonAutomation(name="balanceIntegration", title="\u671f\u521d\u6570\u636e\u6574\u5408", path="/\u6570\u636e\u6574\u5408")
public class BalanceIntegrationAutomationType {
    ...
    public static final String AUTOMATION_INTEGRATION_PARAM_ACCTYEAR = "acctYear";
    ...
    @Autowired
    private BizBalanceInitConvertTaskHandler bizBalanceInitConvertTaskHandler;
    @Autowired
    ...
    private List<ConvertExecuteDTO> getConvertExecuteList(ExecuteContext context, Column<DatasetColumnInfo> column, Column<DatasetColumnInfo> schemeColumn) throws AutomationException {
        ...
        String year = context.getParameterValueAsString(AUTOMATION_INTEGRATION_PARAM_ACCTYEAR);
        String unitType = context.getParameterValueAsString(AUTOMATION_INTEGRATION_PARAM_UNITTYPE);
        int acctYear = StringUtils.isEmpty((String)year) ? DateUtils.getYearOfDate((Date)DateUtils.now()) : Integer.parseInt(year);
        ...
        schemeMap.forEach((k, v) -> {
            ...
            convertRefDefineDTO.setCode(this.bizBalanceInitConvertTaskHandler.getName());
            convertRefDefineDTO.setName(this.bizBalanceInitConvertTaskHandler.getTitle());
            UnitCodeConvertParam unitCodeConvertParam = new UnitCodeConvertParam();
            unitCodeConvertParam.setUnitType(unitType);
            unitCodeConvertParam.setAcctYear(acctYear);
    ...
    public IMetaInvoker meta() {
        return instance -> {
            ...
            AutomationParameter acctYear = new AutomationParameter(AUTOMATION_INTEGRATION_PARAM_ACCTYEAR, "\u4f1a\u8ba1\u5e74\u5ea6(\u9ed8\u8ba4\u4e3a\u5f53\u524d\u5e74\u5ea6)", AutomationParameterDataTypeEnum.INTEGER, String.valueOf(DateUtils.getYearOfDate((Date)DateUtils.now())), true);
