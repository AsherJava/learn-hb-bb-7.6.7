...
@Component
public class ReclassifyExportTask
extends AbstractLossGainExportTask {
    ExportExcelSheet exportExcelSheet(ExportContext context, LossGainOffsetVO lossGainOffsetVO, int sheetNo) {
        List currDeferredIncomeTaxResult = lossGainOffsetVO.getCurrReclassifyResult();
        List<DesignFieldDefineVO> otherShowColumns = this.convert(lossGainOffsetVO.getReclassifyDimension());
        IntervalColorExportExcelSheet exportExcelSheet = this.getExportExcelSheet(context, sheetNo, GcI18nUtil.getMessage((String)"gc.calculate.lossGain.reclassification"));
