...
import com.jiuqi.gcreport.invest.calculate.rule.base.fvch.PublicValueAdjustmentRuleJsonStringEO2VOHelper;
import com.jiuqi.gcreport.invest.calculate.rule.base.fvch.PublicValueAdjustmentRuleJsonStringVO2EOHelper;
...
@Component
public class PublicValueAdjustmentRuleHandler
extends RuleHandler {
    public String convertJsonStringWhenVO2EO(UnionRuleVO unionRuleVO) {
        return PublicValueAdjustmentRuleJsonStringVO2EOHelper.newInstance(unionRuleVO).convertJsonStringWhenVO2EO();
    }

    public String convertJsonStringWhenEO2VO(UnionRuleEO unionRule) {
        return PublicValueAdjustmentRuleJsonStringEO2VOHelper.newInstance(unionRule.getJsonString(), unionRule.getReportSystem()).convertJsonStringWhenEO2VO();
    }

    public AbstractUnionRule convertEO2DTO(UnionRuleEO unionRule) {
        AbstractUnionRule rule = (AbstractUnionRule)JsonUtils.readValue((String)unionRule.getJsonString(), PublicValueAdjustmentRuleDTO.class);
    ...
    public UnionRuleEO convertDTO2EO(AbstractUnionRule rule) {
        ...
        PublicValueAdjustmentRuleDTO publicValueAdjustmentRuleDTO = new PublicValueAdjustmentRuleDTO();
        BeanUtils.copyProperties(rule, publicValueAdjustmentRuleDTO);
        map.put("debitItemList", publicValueAdjustmentRuleDTO.getDebitItemList());
        map.put("creditItemList", publicValueAdjustmentRuleDTO.getCreditItemList());
    ...
    public AbstractUnionRule convertMapToDTO(Map<String, Object> importJsonListMap) {
        return (AbstractUnionRule)JsonUtils.readValue((String)JsonUtils.writeValueAsString(importJsonListMap), PublicValueAdjustmentRuleDTO.class);
    }

    public List<String> getFormulaFetchDataSourceTable(String reportSystemId) {
        return Arrays.asList("GC_FVCHBILL", "GC_FVCH_FIXEDITEM", "GC_FVCH_OTHERITEM");
    }
}
