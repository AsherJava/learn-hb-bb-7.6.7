...
@Component
public class GcCalcDirectInvestBillSegmentRuleProcessorImpl
extends AbstractGcCalcInvestBillRuleProcessor {
    ...
    public boolean isMatch(AbstractUnionRule rule, GcCalcEnvContext env) {
        return RuleTypeEnum.DIRECT_INVESTMENT_SEGMENT.getCode().equals(rule.getRuleType());
    }

    @Override
    protected void execute(@NotNull AbstractInvestmentRule rule, GcCalcEnvContext env, List<GcInvestBillGroupDTO> segmentDirectInvestmentEOs, boolean isMonthlyIncrement) {
        if (isMonthlyIncrement) {
            this.monthlyExecutor.execute(rule, env, segmentDirectInvestmentEOs);
        } else {
            this.executor.execute(rule, env, segmentDirectInvestmentEOs);
        }
    }
}
