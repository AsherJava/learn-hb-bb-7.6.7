...
@Component
public class GcCalcDirectInvestBillRuleProcessorImpl
extends AbstractGcCalcInvestBillRuleProcessor {
    ...
    public boolean isMatch(AbstractUnionRule rule, GcCalcEnvContext env) {
        ...
        return RuleTypeEnum.DIRECT_INVESTMENT.getCode().equals(rule.getRuleType());
    }

    @Override
    protected void execute(@NotNull AbstractInvestmentRule rule, GcCalcEnvContext env, List<GcInvestBillGroupDTO> directInvestmentEOs, boolean isMonthlyIncrement) {
        if (isMonthlyIncrement) {
            this.monthlyExecutor.execute(rule, env, directInvestmentEOs);
        } else {
            this.executor.execute(rule, env, directInvestmentEOs);
        }
    }
}
