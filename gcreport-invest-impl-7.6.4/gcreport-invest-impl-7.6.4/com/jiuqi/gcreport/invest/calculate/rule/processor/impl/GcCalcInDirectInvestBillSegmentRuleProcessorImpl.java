...
@Component
public class GcCalcInDirectInvestBillSegmentRuleProcessorImpl
extends AbstractGcCalcInvestBillRuleProcessor {
    ...
    public boolean isMatch(AbstractUnionRule rule, GcCalcEnvContext env) {
        return RuleTypeEnum.INDIRECT_INVESTMENT_SEGMENT.getCode().equals(rule.getRuleType());
    }

    @Override
    protected void execute(@NotNull AbstractInvestmentRule rule, GcCalcEnvContext env, List<GcInvestBillGroupDTO> segmentInDirectInvestmentEOs, boolean isMonthlyIncrement) {
        if (isMonthlyIncrement) {
            this.monthlyExecutor.execute(rule, env, segmentInDirectInvestmentEOs);
        } else {
            this.executor.execute(rule, env, segmentInDirectInvestmentEOs);
        }
    }
}
