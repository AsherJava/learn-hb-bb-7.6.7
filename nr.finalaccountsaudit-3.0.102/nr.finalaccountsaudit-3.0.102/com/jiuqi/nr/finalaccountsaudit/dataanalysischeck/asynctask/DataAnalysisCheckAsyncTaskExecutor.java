...
@RealTimeJob(group="ASYNCTASK_DATAANALYSISCHECK", groupTitle="\u6570\u636e\u5206\u6790\u5ba1\u6838")
public class DataAnalysisCheckAsyncTaskExecutor
extends NpRealTimeTaskExecutor {
    private static final Logger logger = LoggerFactory.getLogger(DataAnalysisCheckAsyncTaskExecutor.class);

    public void execute(JobContext jobContext) {
        block18: {
            IDataAnalysisCheckService dataAnalysisCheckService = (IDataAnalysisCheckService)BeanUtil.getBean(IDataAnalysisCheckService.class);
            ...
            RealTimeTaskMonitor monitor = new RealTimeTaskMonitor(taskId, AsynctaskPoolType.ASYNCTASK_DATAANALYSISCHECK.getName(), jobContext);
            try {
                ...
                if (args instanceof ZBQueryParamInfo) {
                    ...
                    for (String modelid : param.getZbQueryModelIds()) {
                        try {
                            DataAnalysisItem analysisItem = dataAnalysisCheckService.hasQueryData(modelid, param.getContext());
                ...
                try {
                    DataAnalysisItem analysisItem = dataAnalysisCheckService.hasQueryData(modelid, null);
                    if (!analysisItem.getResult()) {
                        resultInfos.setResult(false);
                    }
                    analysisItems.add(analysisItem);
