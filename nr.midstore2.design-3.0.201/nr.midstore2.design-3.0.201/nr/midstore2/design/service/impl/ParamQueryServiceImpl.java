 ...
 *  nr.midstore2.data.publish.internal.ReportMidstoreSDKLib
 */
package nr.midstore2.design.service.impl;
...
import nr.midstore2.design.service.IParamQueryService;
import nr.midstore2.design.vo.LabelVO;
import nr.midstore2.design.vo.MidstoreFormTreeVO;
import nr.midstore2.design.vo.MistoreDataLinkVO;
import nr.midstore2.design.vo.MistoreFieldVO;
import nr.midstore2.design.vo.MistoreFormVO;
import nr.midstore2.design.vo.MistoreGroupVO;
import nr.midstore2.design.vo.MistoreTableVO;
import nr.midstore2.design.vo.MistoreTaskVO;
import nr.midstore2.design.vo.TaskListVO;
...
implements IParamQueryService {
    ...
    @Override
    public TaskListVO initTaskTree(String taskKey, String periodType) {
        TaskListVO res = new TaskListVO();
        ArrayList<ITree<TaskTreeNodeVO>> taskTree = new ArrayList<ITree<TaskTreeNodeVO>>();
        taskTree.add(this.buildRootNode(taskKey, periodType));
        res.setTaskTree(taskTree);
        if (!StringUtils.isEmpty((String)taskKey)) {
            res.setTask(this.getTaskDefine(taskKey));
        }
        return res;
    }

    private ITree<TaskTreeNodeVO> buildRootNode(String taskKey, String periodType) {
        ...
        this.buildTaskTree(children, taskKey, periodType);
        node.setChildren(children);
        return node;
    }

    private void buildTaskTree(List<ITree<TaskTreeNodeVO>> children, String taskKey, String periodType) {
        List allTaskGroup = this.designTime.getAllTaskGroup();
        List allTaskDefines = this.runTimeViewController.getAllTaskDefines().stream().filter(t -> t.getPeriodType().name().equals(periodType)).collect(Collectors.toList());
    ...
    @Override
    public TaskListVO initTaskTree(String taskKey, List<String> periodTypes) {
        TaskListVO res = new TaskListVO();
        ArrayList<ITree<TaskTreeNodeVO>> taskTree = new ArrayList<ITree<TaskTreeNodeVO>>();
        taskTree.add(this.buildRootNode(taskKey, periodTypes));
        res.setTaskTree(taskTree);
        if (!StringUtils.isEmpty((String)taskKey)) {
            res.setTask(this.getTaskDefine(taskKey));
        }
    ...
    private ITree<TaskTreeNodeVO> buildRootNode(String taskKey, List<String> periodTypes) {
        TaskTreeNodeVO root = new TaskTreeNodeVO();
        root.setKey("00000000000000000000000000000000");
        root.setCode("00000000000000000000000000000000");
        root.setTitle("\u5168\u90e8\u4efb\u52a1");
        ...
        node.setLeaf(false);
        ArrayList<ITree<TaskTreeNodeVO>> children = new ArrayList<ITree<TaskTreeNodeVO>>();
        this.buildTaskTree(children, taskKey, periodTypes);
        node.setChildren(children);
        return node;
    }

    private void buildTaskTree(List<ITree<TaskTreeNodeVO>> children, String taskKey, List<String> periodTypes) {
        List allTaskGroup = this.designTime.getAllTaskGroup();
        List allTaskDefines = CollectionUtils.isEmpty(periodTypes) ? this.runTimeViewController.getAllTaskDefines() : this.runTimeViewController.getAllTaskDefines().stream().filter(t -> periodTypes.contains(t.getPeriodType().name())).collect(Collectors.toList());
        Map<String, TaskDefine> allTaskMap = allTaskDefines.stream().collect(Collectors.toMap(IBaseMetaItem::getKey, task -> task));
        HashSet<String> tasksHasGroup = new HashSet<String>();
        this.buildChildTree(null, allTaskGroup, allTaskMap, children, tasksHasGroup, taskKey);
        for (TaskDefine task2 : allTaskDefines) {
    ...
    private ITree<TaskTreeNodeVO> convertTaskTreeNode(TaskDefine task, String taskKey) {
        ...
        r.setPeriodType(task.getPeriodType().name());
        r.setPeriodEntity(task.getDateTime());
        r.setOrgLinks(this.getOrgLinks(task.getKey()));
    ...
    private List<LabelVO> getOrgLinks(String taskKey) {
        ArrayList<LabelVO> orgLinks = new ArrayList<LabelVO>();
        ...
        for (TaskOrgLinkDefine orgLinkDef : orgLinkDefs) {
            ...
            vo.setLabel(entityDefine.getTitle() + "(" + entityDefine.getCode() + ")");
            orgLinks.add(vo);
        }
        return orgLinks;
    }

    private static ITree<TaskTreeNodeVO> convertGroupTreeNode(DesignTaskGroupDefine group) {
        TaskTreeNodeVO r = new TaskTreeNodeVO();
    ...
    private static boolean equals(String a, String b) {
        ...
        if (!org.springframework.util.StringUtils.hasText(b)) {
            b = null;
        }
        return a == null ? b == null : a.equals(b);
    }

    private void buildChildTree(String parentId, List<DesignTaskGroupDefine> allTaskGroup, Map<String, TaskDefine> allTaskMap, List<ITree<TaskTreeNodeVO>> children, Set<String> tasksHasGroup, String taskKey) {
        List links;
        for (DesignTaskGroupDefine group : allTaskGroup) {
            if (!ParamQueryServiceImpl.equals(group.getParentKey(), parentId)) continue;
            ITree<TaskTreeNodeVO> node = ParamQueryServiceImpl.convertGroupTreeNode(group);
            children.add(node);
        ...
        boolean isRoot = !org.springframework.util.StringUtils.hasText(parentId);
        List taskList = null;
    ...
    public List<MistoreFieldVO> getFieldByBaseData(String baseDataKey) {
        ...
        if (dataModel != null) {
            ...
            for (DataModelColumn column : dataModel.getColumns()) {
                ...
                field.setFieldType(ReportMidstoreSDKLib.getDataTypeByColumn((DataModelType.ColumnType)column.getColumnType()));
                field.setLinkType(0);
    ...
    public List<MistoreFieldVO> getFieldByOrg(String orgDataName) {
        ...
        if (orgDefine != null) {
            ...
            for (ZB orgZb : orgZbList) {
                ...
                field.setFieldType(ReportMidstoreSDKLib.getDataTypeByOrg((int)orgZb.getDatatype()));
