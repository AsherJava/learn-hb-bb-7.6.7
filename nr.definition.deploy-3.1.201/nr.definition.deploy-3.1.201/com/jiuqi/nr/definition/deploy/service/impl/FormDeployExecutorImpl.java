...
@Service
public class FormDeployExecutorImpl
extends AbstractResourceDeployExecutor {
    ...
    public void deploy(String schemeKey, List<String> sourceKeys) {
        if (CollectionUtils.isEmpty(sourceKeys)) {
            this.deploy(ParamDeploySQL.FormScheme.values(), schemeKey);
        } else {
            for (String sourceKey : sourceKeys) {
                this.deploy(ParamDeploySQL.Form.values(), sourceKey);
            }
        }
    }

    private void deploy(ParamDeploySQL[] sql, String param) {
    ...
    public void rollback(String schemeKey, List<String> sourceKeys) {
        if (CollectionUtils.isEmpty(sourceKeys)) {
            this.rollback(ParamDeploySQL.FormScheme.values(), schemeKey);
        } else {
            for (String sourceKey : sourceKeys) {
                this.rollback(ParamDeploySQL.Form.values(), sourceKey);
            }
        }
    }

    private void rollback(ParamDeploySQL[] sql, String param) {
        for (ParamDeploySQL item : sql) {
