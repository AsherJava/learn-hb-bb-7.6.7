...
public class AnalysisWildcardCellDataNode
extends WildcardCellNode {
    private static final Logger logger = LoggerFactory.getLogger(AnalysisWildcardCellDataNode.class);
    private static final long serialVersionUID = 1332263154565086497L;
    private List<AnalysisCellDataNode> expandCells = new ArrayList<AnalysisCellDataNode>();
    private String endAppend = null;

    public AnalysisWildcardCellDataNode(Token token, IWorksheet workSheet, CompatiblePosition position, int options, List<IASTNode> restrictions) {
    ...
    public Set<Position> expandWildcards(IContext context, List<Integer> rowRange, List<Integer> colRange) throws CellExcpetion {
        ...
        if (rowRange == null || rowRange.size() == 0) {
            ...
            if (this.position.isRowWildcard()) {
                for (i = info.getMinRow(); i <= info.getMaxRow(); ++i) {
                    rowRange.add(i);
                }
            } else {
        ...
        if (colRange == null || colRange.size() == 0) {
            ...
            if (this.position.isColWildcard()) {
                for (i = info.getMinCol(); i <= info.getMaxCol(); ++i) {
                    colRange.add(i);
                }
            } else {
    ...
    public IASTNode replaceWildcard(IContext context, int col, int row) throws CellExcpetion {
        ...
        try {
            AnalysisWorkSheet workSheet = (AnalysisWorkSheet)this.workSheet;
            ...
            if (column != null) {
                if (column.getColumModel() != null) {
                    restrictInfo = ExpressionUtils.parseRestrictInfo((QueryContext)qContext, (String)workSheet.name(), (NodeShowInfo)column.getShowInfo(), (List)this.restrictions);
                }
                IASTNode dataNode = AnalysisFormulaParseUtils.createNodeByDataLinkColumn(qContext, this.token, column, restrictInfo);
                node = new AnalysisCellDataNode(this.token, this.workSheet, column, dataNode, qContext.getExeContext().isJQReportModel());
    ...
    protected void toExplain(IContext context, StringBuilder buffer, Object info) throws InterpretException {
        try {
            ...
            if (this.restrictions != null) {
                ...
                RestrictInfo restrictInfo = ExpressionUtils.parseRestrictInfo((QueryContext)((QueryContext)context), (String)this.workSheet.name(), null, (List)this.restrictions);
                ...
                if ((dimensionRestriction = restrictInfo.getDimensionRestriction()) != null && (period = (String)dimensionRestriction.getValue("DATATIME")) != null) {
                    QueryContext qContext = (QueryContext)context;
    ...
    public List<AnalysisCellDataNode> getExpandCells() {
