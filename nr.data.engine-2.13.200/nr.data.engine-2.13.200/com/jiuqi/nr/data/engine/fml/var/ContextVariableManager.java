...
import com.jiuqi.nr.data.engine.fml.var.VarParentAttribute;
    ...
    public IASTNode find(IContext context, Token token, String refName) throws DynamicNodeException {
        ...
        if (index >= 0) {
            try {
                ...
                if (entityTableName.equalsIgnoreCase("PARENT_ATTR")) {
                    String attributeName = refName.substring(index + 1);
                    var = new VarParentAttribute(refName, attributeName);
                    return new VariableDataNode(token, var);
                }
