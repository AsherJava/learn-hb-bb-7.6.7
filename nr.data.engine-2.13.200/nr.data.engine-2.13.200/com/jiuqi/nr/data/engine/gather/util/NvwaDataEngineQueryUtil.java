...
public class NvwaDataEngineQueryUtil {
    ...
    public ColumnModelDefine getColumnModelDefine(String tableName, String dimensionName) {
        DimensionChanger dimensionChanger = this.dataEngineAdapter.getDimensionChanger(tableName);
        return dimensionChanger.getColumn(dimensionName);
    }
    ...
    public INvwaDataSet queryDataSet(String tableName, DimensionValueSet dimensionValueSet, List<ColumnModelDefine> allColumns, List<String> groupColumns, Map<String, Object> filterMap, Map<String, Boolean> orderColumns, TuplesFilter tuplesFilter, boolean readOnly) {
        NvwaQueryModel queryModel = new NvwaQueryModel();
        if (tuplesFilter != null) {
            queryModel.addTuplesFilter(tuplesFilter);
        }
        DimensionChanger dimensionChanger = this.dataEngineAdapter.getDimensionChanger(tableName);
    ...
    public INvwaDataUpdator getUpdateDataSet(List<ColumnModelDefine> allColumns, DataAccessContext context) {
        NvwaQueryModel queryModel = new NvwaQueryModel();
        for (ColumnModelDefine columnModelDefine : allColumns) {
            queryModel.getColumns().add(new NvwaQueryColumn(columnModelDefine));
        }
        INvwaDataUpdator updator = null;
        try {
            INvwaUpdatableDataAccess nvwaDataAccess = this.nvwaDataAccessProvider.createUpdatableDataAccess(queryModel);
            updator = nvwaDataAccess.openForUpdate(context);
        }
        catch (Exception e) {
            logger.error("\u67e5\u8be2\u5f02\u5e38\uff01", e);
        }
