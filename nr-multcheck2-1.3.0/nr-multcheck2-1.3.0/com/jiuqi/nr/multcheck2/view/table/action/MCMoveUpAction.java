 ...
 *  com.jiuqi.nvwa.resourceview.table.inner.MoveUpTableAction
 */
...
import com.jiuqi.nvwa.resourceview.table.inner.MoveUpTableAction;
...
public class MCMoveUpAction
extends MoveUpTableAction {
    protected final Logger logger = LoggerFactory.getLogger(MCMoveUpAction.class);
    ...
    public ActionResult run(ActionContext actionContext) throws Exception {
        ...
        MultcheckScheme upScheme = (MultcheckScheme)schemes.get(ctIndex - 1);
        if (!ctScheme.getFormScheme().equals(upScheme.getFormScheme())) {
            return ActionResult.error(null, (String)"\u62a5\u8868\u65b9\u6848\u4e0d\u540c\uff0c\u65e0\u6cd5\u79fb\u52a8");
        }
        try {
            this.schemeService.moveScheme(ctScheme, upScheme);
        }
        catch (Exception e) {
            this.logger.error("\u4e0a\u79fb\u5931\u8d25", e);
            return ActionResult.error(null, (String)"\u4e0a\u79fb\u5931\u8d25");
        }
