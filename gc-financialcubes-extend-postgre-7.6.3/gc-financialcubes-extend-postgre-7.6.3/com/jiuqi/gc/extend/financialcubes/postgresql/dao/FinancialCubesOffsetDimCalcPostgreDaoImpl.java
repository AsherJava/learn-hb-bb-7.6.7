 ...
 *  com.jiuqi.gc.financialcubes.offset.dao.impl.FinancialCubesOffsetBizDaoImpl
...
import com.jiuqi.gc.financialcubes.offset.dao.impl.FinancialCubesOffsetBizDaoImpl;
...
@Repository
@Primary
public class FinancialCubesOffsetDimCalcPostgreDaoImpl
extends FinancialCubesOffsetBizDaoImpl {
    public void updateTempTableByPriorPeriod(FinancialCubesOffsetTaskDto offsetTaskDto, String priorPeriod, Set<String> dimensionCodeSet) {
        ...
        sql.append("   SET NC=B.NC,BF=B.BF,DEBIT=TEMP.DEBITSUM-B.DEBITSUM,CREDIT=TEMP.CREDITSUM-B.CREDITSUM, \n");
        ...
        sql.append("  FROM (SELECT SUM(NC) AS NC,SUM(CF) AS BF, \n");
        ...
        sql.append("               DATATIME,MDCODE,UNITCODE,OPPUNITCODE,MD_GCORGTYPE,MD_CURRENCY,ORGNCURRENCY,MD_AUDITTRAIL,SUBJECTCODE \n");
        sql.append("               %2$s  \n");
        sql.append("          FROM %3$s \n");
        sql.append("         WHERE DATATIME = '##PRIORPERIOD##'  \n");
        sql.append("           ##UNITWHERESQL##  \n");
        sql.append("           AND MDCODE = ? \n");
        sql.append("          GROUP BY DATATIME,MDCODE,UNITCODE,OPPUNITCODE,MD_GCORGTYPE,  \n");
        sql.append("                   MD_CURRENCY,ORGNCURRENCY,MD_AUDITTRAIL,SUBJECTCODE  \n");
        sql.append("                   %2$s  \n");
        sql.append("        ) B\n");
        sql.append(" WHERE 1 = 1  \n");
        sql.append("   AND TEMP.UNITCODE = B.UNITCODE AND TEMP.MD_CURRENCY = B.MD_CURRENCY\n");
        sql.append("   AND TEMP.ORGNCURRENCY = B.ORGNCURRENCY AND TEMP.SUBJECTCODE = B.SUBJECTCODE\n");
        sql.append("    %4$s \n");
        ...
        String formatSql = String.format(sql.toString(), "GC_FINCUBES_DIM_TEMP", assistDimsFields, FinancialCubesCommonUtil.getFinancialCubesDimTableName((FinancialCubesPeriodTypeEnum)offsetTaskDto.getPeriodTypeEnum()), existsFieldsEqualsSql);
    ...
    public int updateFinancialCubes(FinancialCubesOffsetTaskDto offsetTaskDto, Set<String> dimensionCodeSet) {
        ...
        sql.append("   SET SN=TEMP.SN,NC=TEMP.NC,BF=TEMP.BF,DEBIT=TEMP.DEBIT,CREDIT=TEMP.CREDIT,DEBITSUM=TEMP.DEBITSUM, \n");
        ...
        sql.append("  FROM (SELECT ? AS SN,SUM(NC) AS NC,SUM(BF) AS BF,SUM(DEBIT) AS DEBIT,SUM(CREDIT)  AS CREDIT,SUM(DEBITSUM)  AS DEBITSUM, \n");
        ...
        sql.append("               T.UNITCODE AS UNITCODE,MD_CURRENCY,ORGNCURRENCY,MD_AUDITTRAIL,SUBJECTCODE  \n");
        sql.append("               %2$s  \n");
        ...
        sql.append("         GROUP BY  UNITCODE,MD_CURRENCY,ORGNCURRENCY,MD_AUDITTRAIL,SUBJECTCODE  \n");
        sql.append("                   %2$s   \n");
        sql.append("        ) TEMP \n");
        sql.append(" WHERE 1 = 1 \n");
        ...
        sql.append("   AND TEMP.UNITCODE = B.UNITCODE  AND TEMP.SUBJECTCODE = B.SUBJECTCODE  \n");
        sql.append("   AND TEMP.MD_CURRENCY = B.MD_CURRENCY AND TEMP.ORGNCURRENCY = B.ORGNCURRENCY\n");
        sql.append("    %4$s \n");
        StringBuilder existsFieldsEqualsSql = new StringBuilder();
        StringBuilder assistDimsFields = new StringBuilder();
        if (!CollectionUtils.isEmpty(dimensionCodeSet)) {
        ...
        args.add(FinancialCubesAuditTrailEnum.OFFSET.getCode() + "%");
        args.add(offsetTaskDto.getOrgType());
        String formatSql = String.format(sql.toString(), FinancialCubesCommonUtil.getFinancialCubesDimTableName((FinancialCubesPeriodTypeEnum)offsetTaskDto.getPeriodTypeEnum()), assistDimsFields, "GC_FINCUBES_DIM_TEMP", existsFieldsEqualsSql);
        formatSql = formatSql.replace("##UNITWHERESQL## ", unitWhereSql);
