...
@Component
public class FinishCalcMonitorScene
implements MonitorScene {
    public MonitorSceneEnum getMonitorScene() {
        return MonitorSceneEnum.NODE_FINISH_MERGE;
    }
    ...
    public Map<String, MonitorStateEnum> getStates(MonitorArgument argument) {
        ...
        List gcCalcLogInfoEOS = gcCalcLogService.queryLatestCalcLogEOs(GcCalcLogOperateEnum.COMPLETE_CALC, argument.getTaskId(), "CNY", argument.getPeriodStr(), argument.getAdjustCode());
        ...
        try {
            Map<String, MonitorStateEnum> stateEnumMap = argument.getOrgIds().stream().collect(Collectors.toMap(org1 -> org1, o -> {
                ...
                boolean b = TaskStateEnum.SUCCESS.getCode().equalsIgnoreCase(gcCalcLogInfoEO.getTaskState());
                return b ? MonitorStateEnum.FINISH_IS : MonitorStateEnum.FINISH_NOT_IS;
            }));
            return stateEnumMap;
        }
