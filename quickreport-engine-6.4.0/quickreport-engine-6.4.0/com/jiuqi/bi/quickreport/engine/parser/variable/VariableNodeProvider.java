...
import com.jiuqi.bi.quickreport.engine.parser.variable.DSPageVariableNode;
...
public class VariableNodeProvider
implements IDynamicNodeProvider {
    public IASTNode find(IContext context, Token token, String refName) throws DynamicNodeException {
        for (String varName : DSPageVariableNode.DS_VARNAMES) {
            if (!varName.equalsIgnoreCase(refName)) continue;
            return new DSPageVariableNode(token, varName);
        }
        return null;
    }
