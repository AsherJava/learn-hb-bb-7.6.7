...
public class FcCustomDataSqlBuilderHelper
extends FcBaseSqlBuilderHelper {
    ...
    private void buildLevelSql(FinancialCheckQueryConditionDTO condition, StringBuilder sql, String orgType, Set<String> tempGroupIds, boolean isLocal) {
        ...
        if (!StringUtils.isEmpty((String)(oppUnitId = condition.getOppUnitId()))) {
            GcOrgCenterService noneService = GcOrgPublicTool.getInstance((String)orgType, (GcAuthorityType)GcAuthorityType.NONE, (YearPeriodObject)yp);
            Set oppLevelsChildrenAndSelf = noneService.listAllOrgByParentIdContainsSelf(oppUnitId).stream().map(GcOrgCacheVO::getCode).collect(Collectors.toSet());
            TempTableCondition oppUnitTempCond = ReltxSqlUtils.getConditionOfMulStr(oppLevelsChildrenAndSelf, (String)(isLocal ? "item.OPPUNITID" : "item.UNITID"));
