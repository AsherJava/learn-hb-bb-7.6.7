...
public class FcCurrentDataSqlBuilderHelper
extends FcBaseSqlBuilderHelper {
    ...
    public String buildCheckedConditionSql(FinancialCheckQueryConditionDTO condition, String orgType, List<Object> param, Set<String> tempGroupIds) {
        ...
        this.buildLevelSql(condition, sql, orgType);
    ...
    public String buildUncheckedGroupByOppUnitSql(FinancialCheckQueryConditionDTO condition, String orgType, Map<String, String> sortField2WayMap, List<Object> param, Set<String> tempGroupIds) {
        StringBuilder sql = new StringBuilder();
        String columnsSql = SqlUtils.getColumnsSqlByTableDefine((String)"GC_RELATED_ITEM", (String)"item");
        sql.append(" select ").append(columnsSql).append(" from ").append("GC_RELATED_ITEM").append(" item ").append(" where 1=2 ");
        return sql.toString();
    }
    ...
    private void buildLevelSql(FinancialCheckQueryConditionDTO condition, StringBuilder sql, String orgType) {
        ...
        GcOrgCacheVO gcOrg = tool.getOrgByCode(condition.getUnitId());
        String mergeOrgParents = gcOrg.getGcParentStr();
        int currentLength = gcOrg.getGcParentStr().length();
        int codeLength = tool.getOrgCodeLength();
        sql.append(" and ").append("bfdw.gcparents like '").append(mergeOrgParents).append("%' and ").append("dfdw.gcparents like '").append(mergeOrgParents).append("%' and substr(").append("bfdw.gcparents, 1,").append(currentLength + codeLength + 1).append(") <> substr(").append("dfdw.gcparents, 1,").append(currentLength + codeLength + 1).append(") ");
    }
