...
public class FinancialCheckRuleJsonStringEO2VOHelper {
    private String jsonStr;
    private String reportSystemId;

    private FinancialCheckRuleJsonStringEO2VOHelper() {
    }

    public static FinancialCheckRuleJsonStringEO2VOHelper newInstance(String jsonStr, String reportSystemId) {
        FinancialCheckRuleJsonStringEO2VOHelper helper = new FinancialCheckRuleJsonStringEO2VOHelper();
    ...
    public String convertJsonStringWhenEO2VO() {
        ...
        debitItemListCodes.forEach(debitItemListCode -> {
            GcBaseDataVO baseDataVO = GcBaseDataCenterTool.getInstance().convertGcBaseDataVO(GcBaseDataCenterTool.getInstance().queryBasedataByCode("MD_ACCTSUBJECT", debitItemListCode));
            if (!Objects.isNull(baseDataVO)) {
                debitItemList.add(baseDataVO);
            }
        });
        creditItemListCodes.forEach(creditItemListCode -> {
            GcBaseDataVO baseDataVO = GcBaseDataCenterTool.getInstance().convertGcBaseDataVO(GcBaseDataCenterTool.getInstance().queryBasedataByCode("MD_ACCTSUBJECT", creditItemListCode));
            if (!Objects.isNull(baseDataVO)) {
                creditItemList.add(baseDataVO);
            }
        });
    ...
    private Map<String, Object> handleOneFetchSetting(Map<String, Object> oldFetchConfig) {
        ...
        if (!StringUtils.isEmpty(jsonStringBusinessTypeCode)) {
