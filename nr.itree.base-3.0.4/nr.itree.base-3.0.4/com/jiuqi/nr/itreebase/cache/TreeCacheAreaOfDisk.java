...
public class TreeCacheAreaOfDisk
implements ITreeCacheArea {
    private final FileService fileService;
    private static final String EXT = ".uscsd";

    public TreeCacheAreaOfDisk(FileService fileService) {
        this.fileService = fileService;
    }

    @Override
    public boolean contains(String sourceId) {
        FileAreaService area = this.getFileAreaService();
        return area.getInfo(sourceId) != null;
    }

    @Override
    public <T extends Serializable> T getCacheData(String sourceId, Class<T> clazz) {
        FileAreaService area = this.getFileAreaService();
        FileInfo file = area.getInfo(sourceId);
        if (null != file) {
            byte[] fileContent = area.download(file.getKey());
            return this.deserialize(fileContent, clazz);
        }
        return null;
    }
    ...
    public <T extends Serializable> void putCacheData(String sourceId, T contentData) {
        ...
        if (this.contains(sourceId)) {
            area.delete(sourceId);
        }
        byte[] serialize = this.serialize(contentData);
        area.uploadByKey(this.getFileName(sourceId), sourceId, null, serialize);
    }

    private String getFileName(String sourceId) {
        return sourceId + EXT;
    }

    protected FileAreaService getFileAreaService() {
        return this.fileService.area("dim_tree_cache");
    }
