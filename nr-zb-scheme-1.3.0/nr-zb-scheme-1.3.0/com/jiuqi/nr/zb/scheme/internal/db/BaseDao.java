...
import com.jiuqi.nr.zb.scheme.exception.DBParaException;
import com.jiuqi.nr.zb.scheme.internal.anno.DBAnno;
import com.jiuqi.nr.zb.scheme.internal.db.TransUtil;
...
public abstract class BaseDao {
    ...
    protected List<String> dbFieldList = new ArrayList<String>();
    protected Map<String, String> dbFieldMap = new HashMap<String, String>();
    protected Map<String, Field> fieldMap = new HashMap<String, Field>();
    protected HashMap<String, Method> methodMap = new HashMap();
    protected HashMap<String, HashMap<Class<?>, Method>> transMethodMap = new HashMap();
    protected HashMap<String, HashMap<Class<?>, Method>> externalTransMethodMap = new HashMap();
    ...
    protected void readRecord(ResultSet rs, Object obj) {
        for (String dbField : this.dbFieldList) {
            this.readRecord(dbField, rs, obj);
        }
    }

    protected void readRecord(String dbField, ResultSet rs, Object obj) {
        Field field = this.fieldMap.get(this.dbFieldMap.get(dbField));
        DBAnno.DBField fieldAnno = field.getAnnotation(DBAnno.DBField.class);
        this.readRecord(field, fieldAnno, rs, obj);
    }

    protected void readRecord(Field field, DBAnno.DBField fieldAnno, ResultSet rs, Object obj) {
        block30: {
            try {
                String dbField = fieldAnno.dbField();
                if (fieldAnno.dbType() == Timestamp.class) {
                    Timestamp v = rs.getTimestamp(dbField);
                    this.transValueByType(obj, v, field, fieldAnno);
                    break block30;
                }
                if (fieldAnno.dbType() == Blob.class) {
                    try (InputStream v = rs.getBinaryStream(dbField);){
                        this.transValueByType(obj, v, field, fieldAnno);
                        break block30;
                    }
                }
                if (fieldAnno.dbType() == Clob.class) {
                    ...
                    Clob v = rs.getClob(dbField);
                    this.transValueByType(obj, v, field, fieldAnno);
                    break block30;
                }
                Object v = rs.getObject(dbField);
                this.transValueByType(obj, v, field, fieldAnno);
            }
            catch (Exception e) {
    ...
    protected void transValueByType(Object obj, Object v, Field field, DBAnno.DBField fieldAnno) throws SQLException, Exception, IllegalAccessException, NoSuchMethodException {
        if (v != null) {
            int t = 0;
    ...
    protected Object transValue(String methodName, Class<?> type, Object v) throws Exception {
        if (v.getClass() == type) {
            return v;
        }
    ...
    public int insert(Object obj) throws BeanParaException, DBParaException {
        ...
        try {
            ...
            for (String dbFieldName : this.dbFieldList) {
                ...
                if (!StringUtils.isEmpty(fieldAnno.get())) {
                ...
                if (v != null && !StringUtils.isEmpty(fieldAnno.tranWith())) {
        ...
        try {
            ...
            for (Object obj : objs) {
                ...
                for (String dbFieldName : this.dbFieldList) {
                    Field field = this.fieldMap.get(this.dbFieldMap.get(dbFieldName));
                    DBAnno.DBField fieldAnno = field.getAnnotation(DBAnno.DBField.class);
                    lst.add(this.transValue(field, fieldAnno, obj));
                }
                argList.add(lst.toArray());
            }
    ...
    protected Object transValue(Field field, DBAnno.DBField fieldAnno, Object obj) throws Exception {
        ReflectionUtils.makeAccessible(field);
        Object v = field.get(obj);
        if (!StringUtils.isEmpty(fieldAnno.get())) {
            Method getMethod = this.methodMap.get(fieldAnno.get());
            if (getMethod == null) {
                ...
                v = getMethod.invoke(obj, new Object[0]);
            }
        }
        try {
            if (fieldAnno.autoDate()) {
    ...
    public int delete(Object obj) throws BeanParaException, DBParaException {
        ...
        try {
            ...
            if (obj != null && !StringUtils.isEmpty(dbField.tranWith())) {
    ...
    public int update(Object obj) throws BeanParaException, DBParaException {
        ...
        try {
            ...
            if (!StringUtils.isEmpty(pkDbField.get())) {
            ...
            if (v != null && !StringUtils.isEmpty(pkDbField.tranWith())) {
    ...
    public int[] update(Object[] objs) throws BeanParaException, DBParaException {
        ...
        try {
            ...
            for (Object obj : objs) {
                ...
                Object pkValue = this.transValue(pkfield, pkDbField, obj);
                lst.add(pkValue);
    ...
    public <T> T getBy(String whereSQL, Object[] obj, Class<? extends T> t) throws BeanParaException {
        ...
        Object result = null;
        String sql = "";
        sql = StringUtils.isEmpty(whereSQL) ? this.selectSQL.toString() : this.selectSQL.toString() + " where " + whereSQL;
        ...
        try {
            ...
            if (filterAttrs2 != null) {
                for (int idx = 0; idx < filterAttrs2.length; ++idx) {
                    ...
                    if (v != null && !StringUtils.isEmpty(annotation.tranWith())) {
    ...
    public <T> List<T> listByForeign(Object obj, String[] queryParams, Class<? extends T> t) throws BeanParaException {
        ...
        try {
            ...
            for (int i = 0; i < queryParams.length; ++i) {
                ...
                sql.append("=? ");
                if (!StringUtils.isEmpty(annotation.tranWith())) {
                    v = this.transValue(annotation.tranWith(), annotation.dbType(), v);
                }
                listParam.add(v);
            }
    ...
    public int count(String whereSQL) {
        ...
        sql = StringUtils.isEmpty(whereSQL) ? sqlSb.toString() : sqlSb.append(" WHERE ").append(whereSQL).toString();
        Integer rs = (Integer)this.jdbcTemplate.queryForObject(sql, Integer.class);
        return rs;
    }

    protected BeanParaException throwBeanParaException(Exception e) throws BeanParaException {
        throw new BeanParaException(e);
    }

    protected BeanParaException throwBeanParaException(String message) throws BeanParaException {
    ...
    private boolean clobReadByStream() {
        ...
        return database.isDatabase("polardb") || database.isDatabase("POSTGRESQL") || database.isDatabase("Informix");
    }
}
