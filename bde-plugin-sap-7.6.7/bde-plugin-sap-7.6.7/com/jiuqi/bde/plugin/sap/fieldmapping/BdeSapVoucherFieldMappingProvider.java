...
@Component
public class BdeSapVoucherFieldMappingProvider
extends AbstractBdeSapFieldMappingProvider {
    ...
    public String getName() {
        return MemoryBalanceTypeEnum.VOUCHER.getBizModel();
    }
    ...
    public List<FieldDTO> listOdsField(DataSchemeDTO dataSchemeDTO) {
        List<FieldDTO> sapFields = this.getAssistList().stream().filter(e -> !StringUtils.isEmpty((String)e.getVoucherTableField())).map(col -> {
        ...
        List<Column> columnList = this.getColumnsByTableName(SapFetchUtil.getDbSchemeCode(dataSchemeDTO), dataSchemeDTO.getDataSourceCode(), "BSEG");
