...
@Component
public class NoOffsetNoLegalDataCheck
implements OffsetItemCheck {
    ...
    public OffsetCheckResultDTO check(List<Map<String, Object>> originOffsetItems, List<GcOffSetVchrItemDTO> preCalcOffSetItems, String gcDataTraceType) {
        if (CollectionUtils.isEmpty(originOffsetItems) && !CollectionUtils.isEmpty(preCalcOffSetItems)) {
            ...
            if (isAllZero) {
                return new OffsetCheckResultDTO(OffsetCheckInfoEnum.CALCULATION_RESULT_ZERO.getOffsetCheckSceneTypeName(), CheckStatusEnum.UNGENERATED.getCode());
            }
            if (preCalcOffSetItemDebitSum.compareTo(preCalcOffSetItemCreditSum) != 0) {
                return new OffsetCheckResultDTO(OffsetCheckInfoEnum.DEBIT_CREDIT_MISMATCH.getOffsetCheckSceneTypeName(), CheckStatusEnum.CHECK_INCONSISTENT.getCode());
            }
        }
        return null;
    }
