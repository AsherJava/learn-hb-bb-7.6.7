...
@Component
public class NoOffsetWithLegalDataCheck
    ...
    public OffsetCheckResultDTO check(List<Map<String, Object>> originOffsetItems, List<GcOffSetVchrItemDTO> preCalcOffSetItems, String gcDataTraceType) {
        if (CollectionUtils.isEmpty(originOffsetItems) && !CollectionUtils.isEmpty(preCalcOffSetItems)) {
            ...
            if (preCalcOffSetItemDebitSum.equals(preCalcOffSetItemCreditSum) && !isAllZero) {
                return new OffsetCheckResultDTO(OffsetCheckInfoEnum.MERGE_CALC_NOT_EXECUTED.getOffsetCheckSceneTypeName(gcDataTraceType), CheckStatusEnum.CHECK_INCONSISTENT.getCode());
            }
        }
        return null;
    }
