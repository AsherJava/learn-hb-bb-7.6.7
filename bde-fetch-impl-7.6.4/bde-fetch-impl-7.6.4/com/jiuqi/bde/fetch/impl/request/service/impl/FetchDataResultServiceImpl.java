 ...
 *  com.jiuqi.bde.base.util.FloatRegionResponseCaculator
...
@Service
public class FetchDataResultServiceImpl
implements FetchDataResultService {
    @Autowired
    private FetchResultDao fetchResultDao;
    @Autowired
    private INvwaSystemOptionService optionService;
    private final Logger logger = LoggerFactory.getLogger(FetchDataResultServiceImpl.class);

    @Transactional(rollbackFor={Exception.class}, propagation=Propagation.REQUIRES_NEW)
    public void insertFixedResult(FetchResultDim fetchResultDim, List<FetchResult> fixedResultEOList) {
        this.fetchResultDao.insertFixedResult(fetchResultDim, fixedResultEOList);
    }

    @Transactional(rollbackFor={Exception.class}, propagation=Propagation.REQUIRES_NEW)
    public void insertFloatRowResult(FetchResultDim fetchResultDim, FetchFloatRowDTO fetchFloatRowDTO) {
        this.fetchResultDao.insertFloatRowDefine(fetchResultDim, fetchFloatRowDTO.getFloatDefineEOList());
        this.fetchResultDao.insertFloatRowResult(fetchResultDim, fetchFloatRowDTO.getFloatRowResultEOList());
    }

    @Transactional(rollbackFor={Exception.class}, propagation=Propagation.REQUIRES_NEW)
    public void insertFloatColResult(FetchResultDim fetchResultDim, List<FetchResult> floatColResultEOList) {
        this.fetchResultDao.insertFloatColResult(fetchResultDim, floatColResultEOList);
    }

    public Integer countByTableName(String tableName) {
        return this.fetchResultDao.countByTableName(tableName);
    }

    @Transactional(rollbackFor={Exception.class}, propagation=Propagation.REQUIRES_NEW)
    public void truncateByTableName(String tableName) {
        this.fetchResultDao.truncateByTableName(tableName);
    }

    public List<Map<String, Object>> getFloatRowResultsWithType(FetchResultDim fetchResultDim) {
        return this.fetchResultDao.getFloatRowResultsWithType(fetchResultDim);
    }

    public FetchResultDTO getFetchResult(FetchDataRequestDTO fetchRequestDTO) {
    ...
    private FetchResultDTO getSyncFixedResults(FetchDataRequestDTO fetchRequestDTO) {
        ...
        Map envParam = FetchFormulaUtil.convtEnvByFetchContext((FetchRequestDTO)fetchRequestDTO);
    ...
    public Map<String, Object> getFixedResults(FetchDataRequestDTO fetchRequestDTO) {
    ...
    public FloatRegionResultDTO getFloatResults(FetchDataRequestDTO fetchRequestDTO) {
        FloatRegionResponseCaculator floatRegionResultCaculator = new FloatRegionResponseCaculator((FetchRequestDTO)fetchRequestDTO, fetchRequestDTO.getRouteNum());
