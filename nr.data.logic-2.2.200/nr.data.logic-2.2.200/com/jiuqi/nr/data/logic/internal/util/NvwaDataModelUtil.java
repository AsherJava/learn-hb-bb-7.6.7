...
public class NvwaDataModelUtil {
    private static final Logger logger = LoggerFactory.getLogger(NvwaDataModelUtil.class);
    @Autowired
    ...
    public ColumnModelDefine initColumnModel(ColumnModelType columnType, int size, String defaultValue, String tableID, String columnCode, String referColumnID, DeployColumnCollection deployColumnCollection) {
        ...
        if (columnModelDefine == null) {
            ...
            deployColumnCollection.appendCreCol(columnCode, columnModelDefine);
        } else {
            deployColumnCollection.appendModCol(columnCode, columnModelDefine);
        }
    ...
    public String initEntityColumns(TaskDefine taskDefine, FormSchemeDefine scheme, String tableID, DeployColumnCollection deployColumnCollection) throws Exception {
        ...
        for (String entityKeyStr : entitiesKeyArr) {
            ...
            if (this.isPeriodEntity(entityKeyStr)) {
                ...
                masterEntityKeys.append(this.initColumnModel(ColumnModelType.STRING, 50, "-", tableID, "PERIOD", referColumnID, deployColumnCollection).getID()).append(";");
                continue;
            }
            if (flag) {
    ...
    public void deployTable(String tableID) {
        try {
            ...
            throw new CreateSystemTableException(String.format("deploy table error,id: %s;msg: %s ", tableID, e.getMessage()), (Throwable)e);
        }
    }

    public void deployTable(TableInfo tableInfo) {
        try {
            this.dataModelDeployService.deployTable(tableInfo.getTableId());
        }
        catch (Exception e) {
            throw new CreateSystemTableException(String.format("deploy table error,id: %s;name: %s;msg: %s ", tableInfo.getTableId(), tableInfo.getTableName(), e.getMessage()), (Throwable)e);
        }
    }

    public void deployTableUncheck(TableInfo tableInfo) {
        try {
            this.dataModelDeployService.deployTableUnCheck(tableInfo.getTableId());
        }
        catch (Exception e) {
            throw new CreateSystemTableException(String.format("deploy table error,id: %s;name: %s;msg: %s ", tableInfo.getTableId(), tableInfo.getTableName(), e.getMessage()), (Throwable)e);
        }
