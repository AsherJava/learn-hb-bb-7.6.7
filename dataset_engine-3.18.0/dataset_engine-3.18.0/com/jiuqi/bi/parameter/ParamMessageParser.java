...
public class ParamMessageParser {
    ...
    public ParamMessageParser() {
    }

    public ParamMessageParser(String msg) {
        this.parse(msg);
    }
    ...
    public void parse(String msg, boolean extraProcess) {
        String[] paramItem = ParamMessageParser.splitWithSpaceElement(msg, this.paramDelimiter);
        ...
        if (extraProcess) {
            this.processParamName();
        }
    }

    private void processParamName() {
        int i;
        HashSet<String> names = new HashSet<String>();
        ArrayList<ParamItem> extra = new ArrayList<ParamItem>();
        ParamItem paramItem = null;
        String name = null;
        String newName = null;
        int index = -1;
        for (i = 0; i < this.paramList.size(); ++i) {
            paramItem = this.paramList.get(i);
            names.add(paramItem.name);
        }
        for (i = 0; i < this.paramList.size(); ++i) {
            paramItem = this.paramList.get(i);
            name = paramItem.name;
            index = name.indexOf(this.nameDelimiter);
            if (index == -1 || index == name.length() - 1 || names.contains(newName = name.substring(index + 1))) continue;
            names.add(newName);
            paramItem = new ParamItem(newName, paramItem.value);
            extra.add(paramItem);
        }
        this.paramList.addAll(extra);
    }
    ...
    public String getParamStr() {
        ...
        for (String name : this.getParamNames()) {
    ...
    public boolean addParam(String paramName, Object paramValue) {
        if (paramValue instanceof String) {
            return this.addParam(paramName, (String)paramValue);
        }
        if (paramValue instanceof List) {
            List arr = (List)paramValue;
            ArrayList<String> vals = new ArrayList<String>(arr.size());
            for (Object v : arr) {
                vals.add(DataType.formatValue((int)0, v));
            }
            return this.addParam(paramName, vals);
        }
        return this.addParam(paramName, DataType.formatValue((int)0, (Object)paramValue));
    }
    ...
    public void processParamWithPoint(List<String> names) {
        ...
        for (String string : paramNames) {
            ...
            if (paramNameWithSep || (index2 = string.indexOf(nameSeparator)) == -1 || index2 == string.length() - 1) continue;
            paramNameWithSep = true;
        }
        boolean nameWithSep = false;
        for (String name : names) {
            if (name == null || (index = name.indexOf(nameSeparator)) == -1 || index == name.length() - 1) continue;
            nameWithSep = true;
            break;
        }
        ...
        index = -1;
        for (String name : names) {
            if (name == null || (index = name.indexOf(nameSeparator)) == -1 || index == name.length() - 1 || paramNameSet.contains(name) || !paramNameSet.contains(shortName = name.substring(index + 1)) || hashSet.contains(shortName)) continue;
    ...
    public void splitParam(String separator) {
        ...
        for (String paramName : paramNames) {
            ...
            for (String value : values) {
                ...
                String[] _values = ParamMessageParser.splitWithSpaceElement(value, separator);
