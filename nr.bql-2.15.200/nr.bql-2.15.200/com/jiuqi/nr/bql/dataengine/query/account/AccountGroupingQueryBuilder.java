...
import com.jiuqi.nr.bql.dataengine.query.account.AccountGroupingQuerySqlBuilder;
...
public class AccountGroupingQueryBuilder
extends AccountDataQueryBuilder {
    ...
    public AccountGroupingQueryBuilder(QueryDataReader reader) {
        super(reader);
    }

    public void buildGroupingQuery(QueryContext qContext, AccountGroupingQueryImpl groupingQueryImpl) throws ExpressionException, SyntaxException, SQLException {
        ...
        sqlBuilder.setTrackHistory((Boolean)this.isVersionFunc.apply(qContext, fields)).setContainTimeField((Boolean)this.isDataTimeFunc.apply(qContext, fields));
    ...
    public AccountQuerySqlBuilder getSqlBuilder(QueryContext qContext) {
        if (this.querySqlBuilder == null) {
            this.querySqlBuilder = new AccountGroupingQuerySqlBuilder();
        }
    ...
    protected void buildQuerySql(QueryContext qContext) {
        AccountGroupingQuerySqlBuilder mainSqlBuilder = (AccountGroupingQuerySqlBuilder)this.querySqlBuilder;
    ...
    private void checkFieldsGatherType(QueryContext qContext, AccountGroupingQueryImpl queryInfo) throws SyntaxException, ExpressionException {
        ...
        for (int columnIndex = 0; columnIndex < c; ++columnIndex) {
            ...
            if (fieldGatherType != null) {
                statId = statId + fieldGatherType.name();
            }
