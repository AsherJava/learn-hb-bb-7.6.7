 ...
 *  com.jiuqi.np.dataengine.nrdb.query.GroupQueryInfo
...
extends QuerySqlBuilder {
    ...
    protected DBQueryInfo createDBQueryInfo() {
        DBQueryInfo queryInfo = super.createDBQueryInfo();
        ...
        groupInfo.uidGatherTypes = this.toGatherTypes();
        queryInfo.groupQueryInfo = groupInfo;
        return queryInfo;
    }
    ...
    protected String getFieldGatherSql(String uid, AggrType gatherType, int dataType) {
        ...
        for (ASTNode grpNode : this.grpByColIndex2Node.values()) {
            ...
            if (queryField == null || !queryField.getUID().equals(uid)) continue;
            return null;
        }
        if (dataType == 4 || dataType == 3 || dataType == 10) {
            return "sum(";
    ...
    private void buildDetailsSql(QueryContext context, StringBuilder sqlBuilder) throws Exception {
        ...
        try {
            ...
            sqlBuilder.append(" union all select ").append((CharSequence)builder.selectFields).append(" from ").append((CharSequence)builder.fromJoinsTables);
    ...
    private HashMap<String, FieldGatherType> toGatherTypes() {
        HashMap<String, FieldGatherType> types = new HashMap<String, FieldGatherType>();
        this.uidGatherTypes.forEach((k, v) -> {
            FieldGatherType type = FieldGatherType.FIELD_GATHER_MIN;
            switch (v) {
                case NONE: {
                    type = FieldGatherType.FIELD_GATHER_NONE;
                    break;
                }
                case SUM: {
                    type = FieldGatherType.FIELD_GATHER_SUM;
                    break;
                }
                case MIN: {
                    type = FieldGatherType.FIELD_GATHER_MIN;
                    break;
                }
                case MAX: {
                    type = FieldGatherType.FIELD_GATHER_MAX;
                    break;
                }
                case COUNT: {
                    type = FieldGatherType.FIELD_GATHER_COUNT;
                    break;
                }
                case AVERAGE: {
                    type = FieldGatherType.FIELD_GATHER_AVG;
                    break;
                }
                case DISTINCT_COUNT: {
                    type = FieldGatherType.FIELD_GATHER_DISTINCT_COUNT;
                }
            }
            types.put((String)k, type);
        });
        return types;
    }
}
