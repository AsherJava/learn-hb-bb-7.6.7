...
@Component
public class BillSlaveTableTemplate
implements DataModelBillExtendTemplate {
    public String getName() {
        return "billSlaveTable";
    }

    public String getTitle() {
        return BillMessageSourceUtil.getMessage("datamodel.attribute.biztype.billSlave", new Object[0]);
    }

    @Override
    public int getSubBizType() {
        return BillSubBizType.BILLSLAVE.getIndex();
    }
    ...
    public List<DataModelColumn> getTemplateFields() {
        ...
        columns.add(new DataModelColumn().columnName("VER").columnTitle(BillTemplateFieldConsts.getFtVer()).columnType(DataModelType.ColumnType.NUMERIC).lengths(new Integer[]{19, 0}).columnAttr(DataModelType.ColumnAttr.SYSTEM));
        ...
        columns.add(new DataModelColumn().columnName("ORDINAL").columnTitle(BillTemplateFieldConsts.getFtOrdinal()).columnType(DataModelType.ColumnType.NUMERIC).lengths(new Integer[]{19, 6}).columnAttr(DataModelType.ColumnAttr.SYSTEM));
        return columns;
    }
    ...
    public List<DataModelIndex> getTemplateIndexs(String tableName) {
        ...
        DataModelIndex index1 = new DataModelIndex().columnList(new String[]{"GROUPID", "MASTERID"});
        if (tableName != null) {
            index1.setIndexName(tableName + "_GD_MD");
        } else {
