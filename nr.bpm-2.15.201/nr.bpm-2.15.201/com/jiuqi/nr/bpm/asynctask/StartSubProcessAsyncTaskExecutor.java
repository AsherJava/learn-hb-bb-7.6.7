...
@RealTimeJob(group="ASYNCTASK_SUB_STARTPROCESS", groupTitle="\u5f00\u542f\u6d41\u7a0b\u5b50\u4efb\u52a1", subject="\u62a5\u8868")
public class StartSubProcessAsyncTaskExecutor
extends NpRealTimeTaskExecutor {
    private static final long serialVersionUID = 1L;
    private static final Logger logger = LoggerFactory.getLogger(StartSubProcessAsyncTaskExecutor.class);
    public static final String ASYNCTASK_SUB_STARTPROCESS = "ASYNCTASK_SUB_STARTPROCESS";

    public void executeWithNpContext(JobContext jobContext) throws JobExecutionException {
        IStartProcess starProcess = (IStartProcess)BeanUtils.getBean(IStartProcess.class);
        String taskId = jobContext.getInstanceId();
        AbstractRealTimeJob job = jobContext.getRealTimeJob();
        Map params = job.getParams();
        RealTimeTaskMonitor asyncTaskMonitor = new RealTimeTaskMonitor(taskId, ASYNCTASK_SUB_STARTPROCESS, jobContext);
        try {
            String args = (String)params.get("NR_ARGS");
            if (Objects.nonNull(params) && Objects.nonNull(args)) {
                SubProcessBean stateChange = (SubProcessBean)SimpleParamConverter.SerializationUtils.deserialize((String)args);
                starProcess.startProcess(stateChange.getFormSchemeKey(), stateChange.getBusinessKeyMap(), stateChange.getFlowType());
