...
@RealTimeJob(group="ASYNCTASK_UNBINDPROCESS", groupTitle="\u505c\u7528\u6d41\u7a0b", subject="\u62a5\u8868")
public class UnBindProcessAsyncTaskExecutor
extends NpRealTimeTaskExecutor {
    ...
    public void executeWithNpContext(JobContext jobContext) throws JobExecutionException {
        WorkflowSettingService workflowSettingService = (WorkflowSettingService)BeanUtils.getBean(WorkflowSettingService.class);
        String taskId = jobContext.getInstanceId();
        AbstractRealTimeJob job = jobContext.getRealTimeJob();
        Map params = job.getParams();
        RealTimeTaskMonitor asyncTaskMonitor = new RealTimeTaskMonitor(taskId, AsynctaskPoolType.ASYNCTASK_UNBINDPROCESS.getName(), jobContext);
        try {
            String args = (String)params.get("NR_ARGS");
            if (Objects.nonNull(params) && Objects.nonNull(args)) {
                StateChangeObj stateChangeObj = (StateChangeObj)SimpleParamConverter.SerializationUtils.deserialize((String)args);
                workflowSettingService.unBindProcess(stateChangeObj, (AsyncTaskMonitor)asyncTaskMonitor);
    ...
    public String getTaskPoolType() {
        return AsynctaskPoolType.ASYNCTASK_UNBINDPROCESS.getName();
    }
}
