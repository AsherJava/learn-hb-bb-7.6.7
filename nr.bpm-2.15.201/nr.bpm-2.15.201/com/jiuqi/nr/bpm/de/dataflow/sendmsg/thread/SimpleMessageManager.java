...
public class SimpleMessageManager {
    private final Logger logger = LoggerFactory.getLogger(SimpleMessageManager.class);
    private int upperLimitValue;
    private int sumTotal;
    private List<SimpleMessageEvent> messageObjs = new ArrayList<SimpleMessageEvent>();
    ...
    public void addMessageObj(SimpleMessageEvent messageBody) {
        if (messageBody != null) {
    ...
    public void check() {
        if (this.messageObjs != null && this.messageObjs.size() == this.upperLimitValue) {
            ...
            this.logger.info("SimpleMessageManager >>>> \u53d1\u9001\u4ee3\u529e\uff0c\u7b2c" + ++this.count + "\u6b21\u53d1\u9001\uff0c\u5bb9\u91cf\u5927\u5c0f\uff1a" + this.messageObjs.size() + "\u6761");
    ...
    public void clear() {
        if (this.messageObjs != null && !this.messageObjs.isEmpty()) {
            this.sendTodo(this.messageObjs);
            this.logger.info("SimpleMessageManager >>>> \u53d1\u9001\u4ee3\u529e\uff0c\u7b2c" + ++this.count + "\u6b21\uff0c\u6700\u540e\u4e00\u6b21\u53d1\u9001\uff0c\u5bb9\u91cf\u5927\u5c0f\uff1a" + this.messageObjs.size() + "\u6761");
            this.messageObjs.clear();
        }
    }

    private void sendTodo(List<SimpleMessageEvent> subList) {
        if (SendMessageTaskConfig.canSendMessage()) {
            try {
                if (this.messageEventListener == null) {
                    this.logger.error("messageEventListener\u5bf9\u8c61\u4e3a\u7a7a");
                }
                this.messageEventListener.sendSimpleMessage(subList);
            }
            catch (TodoMsgException e) {
                this.logger.error("\u5f85\u529e\u53d1\u9001\u5f02\u5e38", e.getCause());
            }
        }
    }
}
