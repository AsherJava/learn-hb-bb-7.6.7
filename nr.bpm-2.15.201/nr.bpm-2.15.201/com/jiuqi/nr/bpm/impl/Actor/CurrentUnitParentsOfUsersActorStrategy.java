...
@Component
public class CurrentUnitParentsOfUsersActorStrategy
extends ActorStrategyBase<ActorStrategyParameter.Useless> {
    ...
    public boolean isUserMatch(BusinessKeyInfo businessKey, ActorStrategyParameter.Useless strategyParameter, Actor actor, Task task) {
        ...
        while (parentsEntityKey != null && i < parentsEntityKey.length) {
            Boolean grantEntity = this.entityLinkService.isGrantedWithEntity(businessKey.getFormSchemeKey(), entityView, parentsEntityKey[i++], actor.getIdentityId(), businessKey.getPeriod());
            if (!grantEntity.booleanValue() || !isCanExecute.booleanValue()) continue;
            return true;
        }
        return false;
    }
    ...
    public String getDescription() {
        return "\u5c5e\u4e8e\u5f53\u524d\u5355\u4f4d\u7684\u4e0a\u7ea7\u5355\u4f4d\uff08\u76f4\u63a5\u548c\u95f4\u63a5\u4e0a\u7ea7\uff09\u7684\u7528\u6237\u53ef\u4ee5\u770b\u5230\u5f53\u524d\u5355\u4f4d\u3001\u5f53\u524d\u6d41\u7a0b\u8282\u70b9\u7684\u52a8\u4f5c\u5e76\u6267\u884c\u3002";
    }
}
