...
@Component
public final class CanAuditActorStrategy
extends AuthorityActorStrategyBase {
    private static final Logger logger = LoggerFactory.getLogger(CanAuditActorStrategy.class);

    public CanAuditActorStrategy() {
        super("\u6709\u5ba1\u6279\u6743\u9650\u7684\u4eba");
    }
    ...
    protected Set<String> getCanOperateIdentityKeys(EntityViewDefine entityView, String entityKeyData, Date queryVersionStartDate, Date queryVersionDate, BusinessKeyInfo businessKey) {
        ...
        Set<String> identityKeysByFormSchemeKey = this.nrProcessAuthorityProvider.getCanAuditIdentityKeys(businessKey);
        Set identityKeysByEntity = null;
        try {
            identityKeysByEntity = this.entityAuthorityService.getCanAuditIdentityKeys(entityView.getEntityId(), entityKeyData, queryVersionStartDate, queryVersionDate);
        }
    ...
    protected boolean canOperateEntity(EntityViewDefine entityView, String entityKeyData, Date queryVersionStartDate, Date queryVersionDate, BusinessKeyInfo businessKey) {
        ...
        boolean canAuditEntity = false;
        try {
            canAuditEntity = this.entityAuthorityService.canAuditEntity(entityView.getEntityId(), entityKeyData, queryVersionStartDate, queryVersionDate);
        }
        catch (UnauthorizedEntityException e) {
            logger.error(e.getMessage(), e);
        }
        boolean canAudit = this.nrProcessAuthorityProvider.canAudit(businessKey);
        return canAuditEntity && canAudit;
    }
    ...
    protected Map<BusinessKey, Boolean> canBatchOperateEntity(EntityViewDefine entityView, String entityKeyData, Date queryVersionStartDate, Date queryVersionDate, List<BusinessKey> businessKeys) {
        ...
        try {
            Map map = this.entityAuthorityService.canAuditEntity(entityView.getEntityId(), entityKeyDatas, queryVersionDate);
        }
        catch (UnauthorizedEntityException e) {
            logger.error(e.getMessage(), e);
        }
        for (BusinessKey business : businessKeys) {
            ...
            boolean canAudit = this.nrProcessAuthorityProvider.canAudit(business);
