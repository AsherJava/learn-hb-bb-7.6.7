    ...
    public static DataValidationExpression createExpression(QueryContext qContext, String formula) throws SyntaxException {
        ...
        if (expression != null) {
            ...
            if (root instanceof FunctionNode) {
                ...
                if (functionNode.getDefine().name().equalsIgnoreCase(CompareType.MOBILEPHONE.getSign())) {
                    IASTNode leftNode = root.getChild(0);
                    DynamicDataNode dataNode = (DynamicDataNode)leftNode;
                    fieldDefine = qContext.getExeContext().getCache().extractFieldDefine((IASTNode)dataNode);
                    FieldDefine field = qContext.getExeContext().getCache().getDataModelDefinitionsCache().getFieldDefine(fieldDefine);
                    return DataValidationExpressionFactory.createExpression(field, CompareType.MOBILEPHONE, null);
                }
            } else {
                IASTNode leftNode = root.getChild(0);
                IASTNode rightNode = root.childrenSize() > 1 ? root.getChild(1) : null;
                Object value = null;
                if (leftNode instanceof DynamicDataNode) {
                ...
                } else if (leftNode instanceof FunctionNode) {
                    FunctionNode functionNode = (FunctionNode)leftNode;
                    if (CompareType.NOTNULL.getSign().toLowerCase().contains(functionNode.getDefine().name().toLowerCase())) {
                        IASTNode childNode = functionNode.getChild(0);
                        DynamicDataNode dataNode = (DynamicDataNode)childNode;
                        fieldDefine = qContext.getExeContext().getCache().extractFieldDefine((IASTNode)dataNode);
                        FieldDefine field = qContext.getExeContext().getCache().getDataModelDefinitionsCache().getFieldDefine(fieldDefine);
                        return DataValidationExpressionFactory.createExpression(field, CompareType.NOTNULL, null);
                    }
                    if (CompareType.MAXLEN.getSign().toLowerCase().contains(functionNode.getDefine().name().toLowerCase())) {
                        ...
                        value = rightNode.evaluate((IContext)qContext);
                        FieldDefine field = qContext.getExeContext().getCache().getDataModelDefinitionsCache().getFieldDefine(fieldDefine);
                        return DataValidationExpressionFactory.createExpression(field, CompareType.MAXLEN, value);
                    }
