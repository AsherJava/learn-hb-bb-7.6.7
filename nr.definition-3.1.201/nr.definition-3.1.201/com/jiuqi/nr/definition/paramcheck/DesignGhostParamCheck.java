...
@Component
@CheckParam(name="designGhostParamCheck", title="\u8bbe\u8ba1\u671f\u5e7d\u7075\u53c2\u6570\u68c0\u67e5", type=ParamType.NR_PARAM)
public class DesignGhostParamCheck
extends ACheckService {
    private static final Logger logger = LogFactory.getLogger(DesignGhostParamCheck.class);
    @Autowired
    private DesignFormSchemeDefineService formSchemeService;
    @Autowired
    private DesignFormDefineService formService;
    @Autowired
    private DesignDataRegionDefineService regionService;
    @Autowired
    ...
    private static final String GHOST_FORMULA = "GHOST_FORMULA";
    ...
    private void checkFormula() {
        List<DesignFormulaDefine> ghostFormulas = this.formulaService.listGhostFormula();
        ...
        if (ghostFormulas.size() > 0) {
            ...
            if (ghostFormulaKeys.size() > 0) {
                ErrorParam error = new ErrorParam(UUIDUtils.getKey(), ErrorTypeEnum.GHOST_ERROR.getError(), "\u516c\u5f0f\u65e0\u6548", (Object)new GhostObject(GHOST_FORMULA, ghostFormulaKeys).toString(), "\u5b58\u5728" + ghostFormulaKeys.size() + "\u4e2a\u65e0\u6548\u516c\u5f0f", null);
    ...
    private void checkFormulaScheme() {
        List<DesignFormulaSchemeDefine> ghostSchmes = this.formulaSchemeService.listGhostScheme();
    ...
    private void checkDataLink() {
        List<DesignDataLinkDefine> ghostLinks = this.dataLinkService.listGhostLink();
    ...
    private void checkDataRegion() {
        List<DesignDataRegionDefine> ghostRegions = this.regionService.listGhostRegion();
    ...
    private void checkForm() {
        List<DesignFormDefine> ghostForms = this.formService.listGhostForm();
    ...
    private void checkFormGroup() {
        List<DesignFormGroupDefine> ghostGroups = this.formGroupService.listGhostGroup();
    ...
    private void checkFormScheme() throws Exception {
        List<DesignFormSchemeDefine> ghostSchemes = this.formSchemeService.listGhostSchemes();
    ...
    private void deleteFormSchemes(List<String> keys) throws Exception {
        ...
        for (String key : keys) {
            List<DesignFormulaSchemeDefine> formulaSchemes;
            List<DesignFormGroupDefine> groups = this.formGroupService.queryFormGroupDefinesByScheme(key);
            if (groups.size() > 0) {
    ...
    private void deleteFormGroups(List<String> keys) throws Exception {
        ...
        for (String key : keys) {
            List<DesignFormDefine> forms = this.formService.queryFormDefineByGroupId(key);
            if (forms.size() <= 0) continue;
            for (FormDefine formDefine : forms) {
                List<DesignFormGroupLink> groups;
                this.formService.removeFormFromGroup(formDefine.getKey(), key);
                if (formDefine.getFormType() == FormType.FORM_TYPE_ENTITY || (groups = this.formService.getFormGroupLinksByFormId(formDefine.getKey())).size() != 0) continue;
    ...
    private void deleteForms(List<String> keys) throws Exception {
        ...
        for (String key : keys) {
            List<DesignDataRegionDefine> regions = this.regionService.getAllRegionsInForm(key);
    ...
    public boolean fix(Object object) throws JQException {
        ...
        try {
            ...
            for (ErrorParam errorParam : errorParams) {
                ...
                if (!ghostObj.type.equalsIgnoreCase(GHOST_FORMULA)) continue;
