...
implements Serializable {
    ...
    private Map<Key, Integer> finder = new HashMap<Key, Integer>();
    private int bufSize;
    private Key tmpKey;
    ...
    public void set(int index, byte[] value) {
        this.finder.remove(this.tmpKey.setProperty(this.list.get(index)));
        byte[] b = this.list.get(index);
        System.arraycopy(value, 0, b, 0, this.bufSize);
        this.finder.put(new Key(b), new Integer(index));
    }

    public int indexOf(byte[] propData) {
        Integer index = this.finder.get(this.tmpKey.setProperty(propData));
    ...
    void rebuild() {
        ...
        for (int i = 0; i < this.list.size(); ++i) {
            this.finder.put(new Key(this.list.get(i)), new Integer(i));
        }
    }
    ...
    public void updateToVer12() {
        GridCellV1 cell = new GridCellV1();
        boolean needRebuild = false;
        for (byte[] prop : this.list) {
            int decimal;
            cell.internalInit(null, 0, 0, prop);
            if (cell.getDataType() != 2 || (cell.getDataProperty() & 0x400) == 0) continue;
            boolean thoundsMark = (cell.getDataProperty() & 0x8000) != 0;
            int n = decimal = thoundsMark ? cell.getDataFlag() - 3 : cell.getDataFlag() - 2;
            if (decimal < 0) {
                decimal = 0;
            }
            cell.setDataFlag(decimal);
