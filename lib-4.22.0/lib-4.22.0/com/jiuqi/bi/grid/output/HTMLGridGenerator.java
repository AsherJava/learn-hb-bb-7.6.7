...
public class HTMLGridGenerator {
    ...
    private final void generateColgroup(StringBuffer output) {
        ...
        for (int col = 0; col < this.gridData.getColCount(); ++col) {
            ...
            Iterator<IPropertySet.IEntry> itr = cellProps.iterator();
            String styleStr = "";
            boolean colAutoWidth = false;
            while (itr.hasNext()) {
                IPropertySet.IEntry curProp = itr.next();
    ...
    private static String getHTMLColor(int colorValue) {
        return Html.rgb2HtmlColor(colorValue);
    }
    ...
    protected String genTDContent(GridCell cell) {
        ...
        switch (cell.getDataType()) {
            ...
            case 3: {
                ...
                try {
                    if (!StringUtils.isNotEmpty(cell.getCellData())) break;
                    String showText = currency.getShowText(cell.getCellData());
                    innerHTML = HTMLGridGenerator.htmlEncoding(showText);
                    break;
                }
                catch (IllegalArgumentException e) {
                    String message = "[" + cell.getColNum() + ". " + cell.getRowNum() + "]" + e.getMessage();
    ...
    private String getCellImageHTML(GridCell cell) {
        ...
        result.append(" src=\"");
        result.append(StringUtils.replace(this.getRealLinkAddr(imgRef), "\\", "/"));
        result.append("\"/>");
    ...
    protected void getCellTDProperties(GridCell cell, IPropertySet cellProps) {
        ...
        if (StringUtils.isEmpty(className) && StringUtils.isEmpty(this.gridData.getDataClass())) {
        ...
        if (this.printCellScript && !StringUtils.isEmpty(cellScript = cell.getScript())) {
    ...
    implements IPropertySet {
        private Map<String, Entry> finder = new HashMap<String, Entry>();
        private List<IPropertySet.IEntry> entries = new ArrayList<IPropertySet.IEntry>();
        ...
        public void put(String key, Object value) {
            Entry entry = this.finder.get(key);
            if (entry == null) {
        ...
        public void remove(String key) {
            Entry entry = this.finder.remove(key);
            if (entry != null) {
        ...
        @Override
        public Iterator<IPropertySet.IEntry> iterator() {
