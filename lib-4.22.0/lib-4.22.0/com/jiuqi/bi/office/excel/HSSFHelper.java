...
public class HSSFHelper {
    ...
    public static int getRGBColor(HSSFColor color) {
    ...
    public static HSSFColor getApproximateColor(int rgbColor) {
        ...
        HSSFColor retColor = (HSSFColor)Cache.colorFinder.get(key);
        if (retColor == null && (retColor = HSSFHelper.findApproximateColor(rgbColor)) != null) {
            Cache.colorFinder.put(key, retColor);
        }
        return retColor;
    }
    ...
    private static HSSFColor findApproximateColor(int rgbColor) {
        Cache.buildColorList();
        return Cache.findColor(rgbColor);
    }
    ...
    private static RGB colorToRGB(int value) {
        int p;
        if (value < 0 && (p = value & 0xFF) < Cache.WIN32_COLORS.length) {
            value = Cache.WIN32_COLORS[p];
        }
    ...
    private static class Cache {
        ...
        private static List<ColorElement> colorElements = new ArrayList<ColorElement>();
        private static Map<Integer, HSSFColor> colorFinder = new Hashtable<Integer, HSSFColor>();
        ...
        private static void buildColorList() {
            List<ColorElement> list = colorElements;
            synchronized (list) {
                if (colorElements.size() == 0) {
                    Map colorMaps = HSSFColor.getIndexHash();
                    Collection colorList = colorMaps.values();
                    Iterator colorItr = colorList.iterator();
                    while (colorItr.hasNext()) {
                        ColorElement element = new ColorElement();
                        element.color = (HSSFColor)colorItr.next();
        ...
        private static HSSFColor findColor(int rgbColor) {
            ...
            for (int i = 0; i < colorElements.size(); ++i) {
                double E;
                ColorElement elem = colorElements.get(i);
