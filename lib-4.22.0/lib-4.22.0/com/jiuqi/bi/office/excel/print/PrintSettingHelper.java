...
public class PrintSettingHelper {
    public static void loadPrintSetting(Workbook workbook, PrintSetting printSetting) {
        for (int i = 0; i < workbook.getNumberOfSheets(); ++i) {
            PrintSettingHelper.loadPrintSetting(workbook.getSheetAt(i), printSetting);
        }
    }
    ...
    public static void loadPrintSetting(Sheet sheet, PrintSetting printSetting) {
        ...
        PrintSettingHelper.setZoom(sheet, printSetup, printSetting.getZoom());
        printSetup.setPaperSize((short)printSetting.getPaperSize().value());
        PrintSettingHelper.setStartPage(printSetup, printSetting.getPageStart());
        PrintSettingHelper.setMargin(sheet, printSetting.getMargin());
        sheet.setHorizontallyCenter(printSetting.isHorzCenter());
        sheet.setVerticallyCenter(printSetting.isVertCenter());
        PrintSettingHelper.setHeaderAndFooter(sheet, printSetting.getHeaderFooterSetting());
        PrintSettingHelper.setRepeatRowsAndCols(sheet, printSetting);
        ...
        PrintSettingHelper.setPrintRange(sheet, printSetting);
        PrintSettingHelper.setBreaks(sheet, printSetting);
    }
    ...
    private static void setPrintRange(Sheet sheet, PrintSetting printSetting) {
        ...
        Row oneRow = (Row)rowIterator.next();
        int totalCols = oneRow.getPhysicalNumberOfCells();
        if (totalCols == 0) {
            return;
        }
    ...
    private static void setHeaderAndFooter(Sheet sheet, HeaderFooterSetting headerFooterSetting) {
        ...
        if (headerFooter != null) {
            if (headerFooter.getHeader() != null) {
                ...
                header.setLeft(PrintSettingHelper.getFontTextForHeaderOrFooter(headerFooter.getHeader().getLeft()));
                header.setCenter(PrintSettingHelper.getFontTextForHeaderOrFooter(headerFooter.getHeader().getCenter()));
                header.setRight(PrintSettingHelper.getFontTextForHeaderOrFooter(headerFooter.getHeader().getRight()));
            }
            if (headerFooter.getFooter() != null) {
                Footer footer = sheet.getFooter();
                footer.setLeft(PrintSettingHelper.getFontTextForHeaderOrFooter(headerFooter.getFooter().getLeft()));
                footer.setCenter(PrintSettingHelper.getFontTextForHeaderOrFooter(headerFooter.getFooter().getCenter()));
                footer.setRight(PrintSettingHelper.getFontTextForHeaderOrFooter(headerFooter.getFooter().getRight()));
            }
        }
