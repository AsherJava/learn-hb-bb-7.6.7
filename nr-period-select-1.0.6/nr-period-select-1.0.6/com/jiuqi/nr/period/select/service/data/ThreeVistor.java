...
import com.jiuqi.nr.period.select.common.Consts;
...
implements DataVistor {
    ...
    public ThreeVistor(DataContext dataContext) {
    ...
    public void build() {
        ...
        if (this.isLastPanel()) {
            switch (this.dataContext.getPeriodEntity().getPeriodType()) {
                ...
                case WEEK: {
                    int index = this.panelData.getSimpleTitle().indexOf(" ");
                    if (index != -1) {
                        String replace = this.panelData.getSimpleTitle().replace(" ", Consts.ZSplit);
                        this.page.setTitle(replace);
                        break;
                    }
                    this.page.setTitle(this.panelData.getSimpleTitle());
                    break;
            ...
            if (this.dataContext.getSelectedPeriod().contains(this.panelData.getCode())) {
                if (this.dataContext.getPeriodEntity().getPeriodType().equals((Object)PeriodType.DAY)) {
                    if (this.panelData.getYear() == this.year && this.panelData.getMonth() == this.month) {
    ...
    private void loadIsCurrentMonth() {
        if (this.panelData.getYear() == this.year && this.panelData.getMonth() == this.month) {
            this.page.setNoCurrentMonth(false);
        } else {
            this.page.setNoCurrentMonth(true);
        }
    }

    private boolean isLoadDay() {
        return PeriodType.DAY.equals((Object)this.dataContext.getPeriodEntity().getPeriodType());
    }
