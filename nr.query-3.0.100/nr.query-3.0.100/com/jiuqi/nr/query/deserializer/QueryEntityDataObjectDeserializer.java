...
public class QueryEntityDataObjectDeserializer
extends JsonDeserializer<QueryEntityDataObject> {
    public QueryEntityDataObject deserialize(JsonParser p, DeserializationContext ctxt) throws IOException, JsonProcessingException {
        QueryEntityDataObject entData = new QueryEntityDataObject();
        ...
        target = jNode.findValue("caption");
        String caption = target != null && !target.isNull() ? target.asText() : null;
        target = jNode.findValue("fieldkey");
        String fieldkey = target != null && !target.isNull() ? target.asText() : null;
        entData.setCaption(caption);
        entData.setKey(key);
