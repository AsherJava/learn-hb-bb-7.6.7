...
public class AnonExecuteViewController {
    Logger logger = LoggerFactory.getLogger(AnonExecuteViewController.class);
    @Autowired
    private SFRemoteResourceManage remoteResourceManage;

    @PostMapping(value={"/module/executeView"})
    public void executeModule(String serviceName, String moduleId) throws IOException {
        ...
        if (StringUtils.isEmpty((String)serviceName) || StringUtils.isEmpty((String)moduleId)) {
            return;
        }
        ...
        new Thread(() -> {
            try {
                ...
                serviceManagerResource.executeModule(moduleId);
                latch.countDown();
            }
            catch (Exception e) {
                try {
    ...
    private void printLogs(OutputStream writer, CountDownLatch latch, IServiceManager serviceManagerResource) {
        ...
        try {
            ...
            do {
                ...
                block9: for (ModuleUpgradeLockOperator.UpgradeLogInfo lockLogInfo : lockLogInfos) {
                    switch (lockLogInfo.getLogType()) {
                        case 0: {
                            writer.write(("<span style = 'color: gray;'> -- " + lockLogInfo.getSql() + "</span>\n").getBytes("utf-8"));
                            continue block9;
                        }
                        case 1: {
