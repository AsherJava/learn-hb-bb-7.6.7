...
public class AddColAction
extends AddAction {
    private InsertParams params;

    @JsonCreator
    public AddColAction(@JsonProperty(value="name") String name, @JsonProperty(value="selections") GridRegion[] selections, @JsonProperty(value="params") InsertParams params) {
    ...
    public void run(Grid2Data grid) {
        for (GridRegion region : this.getSelections()) {
            this.addCol(grid, region, this.params.getCount(), this.params.isForward());
        }
    }
}
