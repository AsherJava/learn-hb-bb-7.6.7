...
public class BaseDataCacheUtil {
    ...
    private static Map<String, BaseDataCacheExtend> cacheExtends;

    public static Map<String, BaseDataCacheExtend> getCacheExtends() {
        if (cacheExtends == null) {
            cacheExtends = ApplicationContextRegister.getBeansOfType(BaseDataCacheExtend.class);
        }
        return cacheExtends;
    }
    ...
    public static void setImportDataResult(String key, String result) {
        if (EnvConfig.getRedisEnable()) {
            BaseDataCacheUtil.initRedisTemplate();
