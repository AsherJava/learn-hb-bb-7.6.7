...
@Mapper
public interface VaDataModelMaintainDao
extends BaseOptMapper<DataModelMaintainDO> {
    @Select(value={"select GROUPCODE from DATAMODEL_DEFINE_MAINTAIN where BIZTYPE=#{biztype} group by GROUPCODE"})
    ...
    public static class DataModelProvider {
        public String findByCondition(DataModelDTO dataModelDTO) {
            SQL sql = new SQL();
            sql.SELECT("*");
            sql.FROM("DATAMODEL_DEFINE_MAINTAIN");
            this.condition(sql, dataModelDTO);
            sql.ORDER_BY("MODIFYTIME desc");
            return sql.toString();
        }
