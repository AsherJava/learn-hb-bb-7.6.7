...
@Component(value="vaDataModelBaseDataTemplate")
public class BaseDataTemplate
implements DataModelTemplate {
    ...
    public String getTitle() {
        return DataModelCoreI18nUtil.getMessage("datamodel.attribute.biztype.basedata", new Object[0]);
    }
    ...
    public List<DataModelColumn> getTemplateFields() {
        ...
        columns.add(new DataModelColumn().columnName("OBJECTCODE").columnTitle(StorageFieldConsts.getFtObjectCode()).columnType(DataModelType.ColumnType.NVARCHAR).lengths(new Integer[]{200}).nullable(Boolean.valueOf(false)).columnAttr(DataModelType.ColumnAttr.SYSTEM));
        ...
        columns.add(new DataModelColumn().columnName("UNITCODE").columnTitle(StorageFieldConsts.getFtUnitCode()).columnType(DataModelType.ColumnType.NVARCHAR).lengths(new Integer[]{60}).mapping("MD_ORG.CODE").mappingType(Integer.valueOf(4)).columnAttr(DataModelType.ColumnAttr.SYSTEM));
    ...
    public List<DataModelIndex> getTemplateIndexs(String tableName) {
        ...
        DataModelIndex index0 = new DataModelIndex().columnList(new String[]{"OBJECTCODE", "VALIDTIME", "INVALIDTIME"}).unique(Boolean.valueOf(true));
        if (tableName != null) {
            index0.setIndexName(tableName + "_OEVEIE");
        } else {
