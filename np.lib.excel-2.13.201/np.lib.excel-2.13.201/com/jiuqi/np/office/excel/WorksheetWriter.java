...
import com.jiuqi.np.office.excel.ICellOperater;
...
implements IWorksheetWriter {
        ...
        try {
            ...
            switch (gridCell.getDataType()) {
                ...
                case 2: {
                    ...
                    cell.setCellValue(new HSSFRichTextString(formatValue));
                    break;
                }
                case 5: {
    ...
    private HSSFCellStyle buildCellStyle(GridCell gridCell) {
        ...
        short formatIndex2;
        HSSFColor backColor;
        HSSFCellStyle cellStyle = (HSSFCellStyle)this.cellStyleHashMap.get(new CellStyleKey(gridCell));
        if (cellStyle != null) {
            return cellStyle;
        }
    ...
    private void adjustHW() {
        ...
        if (this.autoAdjust) {
            for (int col = 0; col < colCount; ++col) {
                this.sheet.autoSizeColumn((short)col);
            }
        } else {
            for (int col = 1; col < colCount; ++col) {
