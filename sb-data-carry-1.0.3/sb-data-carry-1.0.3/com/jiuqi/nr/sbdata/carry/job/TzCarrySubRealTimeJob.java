...
@RealTimeJob(group="ASYNCTASK_TZ_CARRY_SUB", groupTitle="\u53f0\u8d26\u6570\u636e\u7ed3\u8f6c-\u5b50\u4efb\u52a1", subject="\u62a5\u8868")
public class TzCarrySubRealTimeJob
extends AbstractRealTimeJob {
    public void execute(JobContext jobContext) throws JobExecutionException {
        ...
        ZBMappingService zbMappingService = (ZBMappingService)BeanUtil.getBean(ZBMappingService.class);
        ...
        RealTimeTaskMonitor asyncTaskMonitor = new RealTimeTaskMonitor(taskId, "ASYNCTASK_TZ_CARRY_SUB", jobContext);
        Logger defaultLogger = jobContext.getDefaultLogger();
        if (Objects.nonNull(params) && Objects.nonNull(params.get("NR_ARGS"))) {
            TzCarryDownDTO param = (TzCarryDownDTO)SimpleParamConverter.SerializationUtils.deserialize((String)((String)params.get("NR_ARGS")));
            FormDefine formDefine = runtimeViewController.queryFormById(param.getFormKey());
            List allZbMappings = zbMappingService.findByMSAndForm(param.getMappingKey(), formDefine.getFormCode());
            Map<String, List<ZBMapping>> table2Mappings = allZbMappings.stream().collect(Collectors.groupingBy(ZBMapping::getTable));
            ...
            asyncTaskMonitor.progressAndMessage(0.01, "\u5f00\u59cb\u7ed3\u8f6c\u62a5\u8868" + formDefine.getFormCode());
            defaultLogger.info("\u5f00\u59cb\u7ed3\u8f6c\u62a5\u8868" + formDefine.getFormCode());
            ...
            block0: for (DataRegionDefine region : allRegionsInForm) {
                asyncTaskMonitor.progressAndMessage(currentProgress, "\u5f00\u59cb\u7ed3\u8f6c\u533a\u57df" + region.getCode());
                ...
                for (Map.Entry<String, List<DataField>> entry : dataTable2DataField.entrySet()) {
                    DataTableCarryResult result;
                    String dataTableKey = entry.getKey();
                    DataTable dataTable = runtimeDataSchemeService.getDataTable(dataTableKey);
                    List<ZBMapping> zbMappingsByTable = table2Mappings.get(dataTable.getCode());
                    if (CollectionUtils.isEmpty(zbMappingsByTable)) {
                        result = new DataTableCarryResult();
                        results.add(result);
                        continue;
                    }
                    param.setDataTable(dataTable);
                    param.setMappings(zbMappingsByTable);
                    result = tzCarryDownService.carryDown2NextYear(param);
                    results.add(result);
                    if (result.isSuccess()) continue;
                    break block0;
                }
                asyncTaskMonitor.progressAndMessage(currentProgress += splitSize, "\u533a\u57df" + region.getCode() + "\u7ed3\u8f6c\u5b8c\u6210");
            }
            ...
            if (formCarryResult.isSuccess()) {
                defaultLogger.info("\u62a5\u8868" + formDefine.getFormCode() + "\u7ed3\u8f6c\u5b8c\u6210\uff01");
                asyncTaskMonitor.finish("\u62a5\u8868" + formDefine.getFormCode() + "\u7ed3\u8f6c\u5b8c\u6210\uff01", (Object)detail);
            } else {
                defaultLogger.error("\u62a5\u8868" + formDefine.getFormCode() + "\u7ed3\u8f6c\u5931\u8d25\uff01");
                asyncTaskMonitor.error("\u62a5\u8868" + formDefine.getFormCode() + "\u7ed3\u8f6c\u5931\u8d25\uff01", null, detail);
            }
        }
    }
}
