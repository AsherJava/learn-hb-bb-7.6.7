...
public class TableOfContents {
    ...
    private void InitWordTOC() {
        ...
        fonts.setHAnsi("\u5b8b\u4f53");
        fonts.setCs("\u5b8b\u4f53");
        rPr.addNewB().setVal(STOnOffEnum.OFF);
        rPr.addNewBCs().setVal(STOnOffEnum.OFF);
        rPr.addNewColor().setVal("auto");
        rPr.addNewSz().setVal(new BigInteger("32"));
        rPr.addNewSzCs().setVal(new BigInteger("32"));
        ...
        sz.setVal(new BigInteger("32"));
        CTP imgCpt = content.addNewP();
        XWPFParagraph paragraphMeau = new XWPFParagraph(imgCpt, this.docxDocument);
        ...
        XWPFParagraph imgpara = new XWPFParagraph(pp1, this.docxDocument);
        imgpara.setAlignment(ParagraphAlignment.CENTER);
        XWPFRun run = imgpara.createRun();
        try {
            run.addPicture((InputStream)img, XWPFDocument.PICTURE_TYPE_PNG, "", Units.toEMU(Integer.valueOf(this.tabPos) / 20), Units.toEMU(2.5));
        }
        ...
        XWPFParagraph tabImgpara = new XWPFParagraph(tabCpt, this.docxDocument);
    }
    ...
    private void InitPDFTOC() {
        ...
        fonts.setHAnsi("\u5b8b\u4f53");
        fonts.setCs("\u5b8b\u4f53");
        rpr.addNewB().setVal(STOnOffEnum.ON);
        rpr.addNewBCs().setVal(STOnOffEnum.ON);
        rpr.addNewColor().setVal("auto");
        rpr.addNewSz().setVal(new BigInteger("32"));
        rpr.addNewSzCs().setVal(new BigInteger("32"));
            ...
            try {
                ...
                switch (type) {
                    ...
                    case "pdf": {
                        ...
                        CTP paragraph = this.docxDocument.getDocument().getBody().insertNewP(n);
    ...
    private void CreateWordTOC(Map<String, Integer> pageNums) {
        ...
        CTSdtContentBlock contentBlock = this.block.getSdtContent();
        List<XWPFParagraph> paragraphs = this.docxDocument.getParagraphs();
        boolean begin = false;
        for (XWPFParagraph par : paragraphs) {
            String parStyle = par.getStyle();
            if (parStyle == null || !this.doc_styles.contains(parStyle)) continue;
    ...
    private void Create_TOC_pStyle(CTP p, int level) {
        ...
        tab.setPos(new BigInteger(this.tabPos));
    ...
    private void Create_TOC_Hyperlink(CTP p, String text, String bookmarkId, int level, Map<String, Integer> pageNums) {
        ...
        link.setHistory(STOnOffEnum.TRUE);
    ...
    public static void createTocParagraph(CTP paragraph, int level, String text, int num) {
        ...
        ctTab.setPos(new BigInteger("10060"));
        ppr.addNewRPr().addNewNoProof();
