...
public class CustomXWPFHeaderFooterPolicy {
    ...
    public XWPFHeader createHeader(STHdrFtr.Enum type, XWPFParagraph[] pars) {
        ...
        if (header == null) {
            HdrDocument hdrDoc = (HdrDocument)HdrDocument.Factory.newInstance();
            XWPFRelation relation = XWPFRelation.HEADER;
            int i = this.getRelationIndex(relation);
            XWPFHeader wrapper = (XWPFHeader)this.doc.createRelationship((POIXMLRelation)relation, (POIXMLFactory)XWPFFactory.getInstance(), i);
            wrapper.setXWPFDocument((XWPFDocument)this.doc);
            CTHdrFtr hdr = this.buildHdr(type, (XWPFHeaderFooter)wrapper, pars);
    ...
    public XWPFFooter createFooter(STHdrFtr.Enum type, XWPFParagraph[] pars) {
        ...
        if (footer == null) {
            ...
            int i = this.getRelationIndex(relation);
            XWPFFooter wrapper = (XWPFFooter)this.doc.createRelationship((POIXMLRelation)relation, (POIXMLFactory)XWPFFactory.getInstance(), i);
            wrapper.setXWPFDocument((XWPFDocument)this.doc);
            CTHdrFtr ftr = this.buildFtr(type, (XWPFHeaderFooter)wrapper, pars);
    ...
    private void setFooterReference(STHdrFtr.Enum type, XWPFHeaderFooter wrapper) {
        ...
        ref.setType(type);
        ref.setId(this.doc.getRelationId((POIXMLDocumentPart)wrapper));
    }
    ...
    public void createWatermark(PrintStyle.Watermark watermark) {
        ...
        if ("picture".equals(watermark.getType())) {
            try {
                ...
                pars[0] = new XWPFParagraph((CTP)CTP.Factory.newInstance(), (IBody)this.doc);
    ...
    private XWPFParagraph getWordWatermarkParagraph(String text, int idx) {
        ...
        wrap.setAnchory(STVerticalAnchor.MARGIN);
        pict.set((XmlObject)group);
        return new XWPFParagraph(p, (IBody)this.doc);
    }
    ...
    public String getRelationId(XWPFHeader header) {
        Iterator iterator = header.getAllPackagePictures().iterator();
        if (iterator.hasNext()) {
            XWPFPictureData pictureData = (XWPFPictureData)iterator.next();
            return header.getRelationId((POIXMLDocumentPart)pictureData);
        }
        return null;
    }
}
