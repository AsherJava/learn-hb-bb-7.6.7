...
@AutomationType(category="nr-midstore", id="nr-midstore-getdata", title="\u4e2d\u95f4\u5e93\u62c9\u53d6\u6570\u636e", icon="icon16_SHU_A_NW_S2")
public class MidstoreGetDataAutoType {
        ...
        return (automationInstance, executeContext) -> {
            try {
                ...
                if (scheme != null) {
                    ...
                    if ((result = this.getDataSerivce.excuteGetData(param, (AsyncTaskMonitor)(monitor = new SimpleAsyncTaskMonitor(this.asyncTaskDao, this.applicationEventPublisher, taskId = UUID.randomUUID().toString(), AsynctaskPoolType.ASYNCTASK_MIDSTORE_GETDATA.getName())))).isSuccess()) {
    ...
    private IOperationInvoker<DatasetResult> getDatasetResultIOperationInvoker() {
        return (automationInstance, executeContext) -> {
            throw new AutomationExecuteException("\u6570\u636e\u62c9\u53d6\u4e0d\u652f\u6301\u8be5\u64cd\u4f5c");
        };
    }
            ...
            if (workResult != null && workResult.getFailInfoList() != null) {
                for (MistoreWorkFailInfo failInfo : workResult.getFailInfoList()) {
                    ...
                    for (MistoreWorkUnitInfo unitInfo : failInfo.getUnitInfos().values()) {
                        rowData = new Object[]{"  " + unitInfo.getUnitCode() + " " + (StringUtils.isNotEmpty((CharSequence)unitInfo.getUnitTitle()) ? unitInfo.getUnitTitle() : ""), ""};
                        dataset.add(rowData);
                        if (unitInfo.getFormInfos() != null && unitInfo.getFormInfos().size() > 0) {
                            for (MistoreWorkFormInfo formInfo : unitInfo.getFormInfos().values()) {
                                rowData = new Object[]{"    " + formInfo.getFormCode() + " " + (StringUtils.isNotEmpty((CharSequence)formInfo.getFormTitle()) ? formInfo.getFormTitle() : "") + "  " + formInfo.getMessage(), ""};
                                dataset.add(rowData);
                            }
                            continue;
                        }
                        ...
                        for (MistoreWorkFormInfo tableInfo : unitInfo.getTableInfos().values()) {
                            rowData = new Object[]{"    " + tableInfo.getFormCode() + " " + (StringUtils.isNotEmpty((CharSequence)tableInfo.getFormTitle()) ? tableInfo.getFormTitle() : "") + "  " + tableInfo.getMessage(), ""};
                            dataset.add(rowData);
                        }
                    }
                }
