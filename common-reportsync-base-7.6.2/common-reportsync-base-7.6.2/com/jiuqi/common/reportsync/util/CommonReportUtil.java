...
public class CommonReportUtil {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(CommonReportUtil.class);
    ...
    public static void writeBase64(String tableName, List<? extends DefaultTableEntity> dataList, String filePath) throws Exception {
        List<ColumnModelDefine> defines = CommonReportUtil.getFieldDefines(tableName);
        Base64.Encoder encoder = Base64.getEncoder();
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(filePath));){
            bw.write(CommonReportUtil.headStr(defines));
            for (DefaultTableEntity defaultTableEntity : dataList) {
                String oneRowStr = CommonReportUtil.oneRowData(defines, defaultTableEntity);
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(txtFile));){
            ...
            while ((oneRow = reader.readLine()) != null) {
                ...
                for (int i = 0; i < columnCodes.length && i < rowDatas.length; ++i) {
                    ...
                    entity.addFieldValue(columnCodes[i], CommonReportUtil.dataTransferOri(fieldType, rowDatas[i]));
                }
    ...
    public static Workbook readWorkBook(String filePath) {
        ...
        try (FileInputStream in = new FileInputStream(file);){
            try {
                ...
                workbook = new HSSFWorkbook((InputStream)in);
            }
        }
        catch (IOException e) {
    ...
    public static String createNewPath(String filePath, String pathName) {
        String newPath = null;
        try {
            newPath = PathUtil.createNewPath((String)filePath, (String)pathName);
        }
        catch (Exception e) {
    ...
    public static void deleteDir(String dirPath) {
        try {
            PathUtil.deleteDir((String)dirPath);
        }
        catch (Exception e) {
            throw new BusinessRuntimeException(e.getMessage());
        }
        ...
        try (ZipInputStream zipInputStream = new ZipInputStream(new FileInputStream(zipFilePath));){
            ...
            while (zipEntry != null) {
                ...
                if (!zipEntry.isDirectory()) {
                    CommonReportUtil.extractFile(zipInputStream, filePath);
                } else {
                    dir = new File(filePath);
                    dir.mkdir();
                }
