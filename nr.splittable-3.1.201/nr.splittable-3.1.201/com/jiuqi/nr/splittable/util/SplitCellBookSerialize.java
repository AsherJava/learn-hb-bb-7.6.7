 ...
 *  com.jiuqi.nvwa.cellbook.model.BookStyle
 *  com.jiuqi.nvwa.cellbook.model.Cell
 *  com.jiuqi.nvwa.cellbook.model.CellBackGround
 *  com.jiuqi.nvwa.cellbook.model.CellBook
 *  com.jiuqi.nvwa.cellbook.model.CellBorder
 *  com.jiuqi.nvwa.cellbook.model.CellBorders
 *  com.jiuqi.nvwa.cellbook.model.CellColor
 *  com.jiuqi.nvwa.cellbook.model.CellFont
 *  com.jiuqi.nvwa.cellbook.model.CellFormatter
 *  com.jiuqi.nvwa.cellbook.model.CellLayout
 *  com.jiuqi.nvwa.cellbook.model.CellMerge
 *  com.jiuqi.nvwa.cellbook.model.CellSheet
 *  com.jiuqi.nvwa.cellbook.model.CellSheetGroup
 *  com.jiuqi.nvwa.cellbook.model.CellStyle
 *  com.jiuqi.nvwa.cellbook.model.CellTheme
 *  com.jiuqi.nvwa.cellbook.model.CellThemes
 *  com.jiuqi.nvwa.cellbook.model.Col
 *  com.jiuqi.nvwa.cellbook.model.DocProps
 *  com.jiuqi.nvwa.cellbook.model.Header
 *  com.jiuqi.nvwa.cellbook.model.Options
 *  com.jiuqi.nvwa.cellbook.model.Resize
 *  com.jiuqi.nvwa.cellbook.model.SimpleCellStyle
 *  org.json.JSONObject
 */
...
public class SplitCellBookSerialize
extends JsonSerializer<CellBook> {
    private List<List<CellObj>> cellLists;

    public SplitCellBookSerialize() {
    }

    public SplitCellBookSerialize(List<List<CellObj>> cellLists) {
    ...
    private void writeSheet(JsonGenerator gen, CellStyle bookStyle, CellSheet cellSheet) throws IOException {
        ...
        gen.writeEndArray();
        List columns = cellSheet.getColumns();
        ...
        List data = cellSheet.getData();
        gen.writeFieldName("data");
        gen.writeStartArray();
        CellStyle defaultStyle = cellSheet.getCellBook().getBookStyle().getDefaultStyle();
        List sheets = cellSheet.getCellBook().getSheets();
        List<CellObj> cellObjs = this.cellLists.get(sheets.indexOf(cellSheet));
        for (List list : data) {
            gen.writeStartArray();
            for (Cell cell : list) {
                CellBorders borders;
                ...
                } else if (cell.getRowIndex() == 0 || cell.getColIndex() == 0) {
                    gen.writeNumberField("realX", cell.getRowIndex());
                    gen.writeNumberField("realY", cell.getColIndex());
                } else {
                    CellObj cellObj = (CellObj)cellObjs.get((cell.getRowIndex() - 1) * (list.size() - 1) + cell.getColIndex() - 1);
                ...
                gen.writeStringField("t", cell.getDataTypeId());
                if (null != commonDataType) {
                    gen.writeStringField("b", commonDataType.getCode());
                }
                gen.writeStringField("v", StringUtils.isNotEmpty((String)cell.getValue()) ? cell.getValue() : "");
                if (StringUtils.isNotEmpty((String)cell.getFormula())) {
                    gen.writeStringField("formula", cell.getFormula());
                }
                if (StringUtils.isNotEmpty((String)cell.getShowText())) {
        ...
        CellFont defaultFont = defaultStyle.getFont();
        if (!fonts.isEmpty()) {
            gen.writeFieldName("fonts");
            gen.writeStartArray();
            for (CellFont font : fonts) {
                this.writeFont(gen, font, defaultFont);
            }
            gen.writeEndArray();
        }
        CellBackGround defaultBackGround = defaultStyle.getBackGround();
        if (!backGrounds.isEmpty()) {
            gen.writeFieldName("fills");
            gen.writeStartArray();
            for (CellBackGround backGround : backGrounds) {
                this.writeBackGround(gen, backGround, defaultBackGround);
            }
            gen.writeEndArray();
        }
        CellBorders defaultBorders = defaultStyle.getBorders();
        if (!cellBorders.isEmpty()) {
            gen.writeFieldName("borders");
            gen.writeStartArray();
            for (CellBorders borders : cellBorders) {
                this.writeBorder(gen, borders, defaultBorders);
            }
            gen.writeEndArray();
        }
    ...
    private void writeDefaultStyle(CellBook cellBook, JsonGenerator gen) throws IOException {
        ...
        gen.writeStartObject();
        if (StringUtils.isNotEmpty((String)bookStyle.getSelector())) {
            gen.writeStringField("selector", bookStyle.getSelector());
        }
        if (StringUtils.isNotEmpty((String)bookStyle.getSerialNumberHeader())) {
            gen.writeStringField("serialNumberHeader", bookStyle.getSerialNumberHeader());
        }
        if (StringUtils.isNotEmpty((String)bookStyle.getSplitLine())) {
            gen.writeStringField("splitLine", bookStyle.getSplitLine());
        }
        if (StringUtils.isNotEmpty((String)bookStyle.getPadding())) {
    ...
    private void writeFont(JsonGenerator gen, CellFont font, CellFont defaultFont) throws IOException {
        ...
        if (null == defaultFont || defaultFont.getSize() != font.getSize()) {
            gen.writeNumberField("s", font.getSize());
        }
    ...
    private void writeTheme(CellBook cellBook, JsonGenerator gen) throws IOException {
        ...
        gen.writeStartObject();
        Map themes = theme.getThemes();
        for (Map.Entry oneTheme : themes.entrySet()) {
            CellTheme value = (CellTheme)oneTheme.getValue();
            gen.writeFieldName((String)oneTheme.getKey());
            gen.writeStartObject();
            List palette = value.getPalette();
            if (null != palette) {
    ...
    private void writeGroups(CellBook cellBook, JsonGenerator gen) throws IOException {
        List groups = cellBook.getGroups();
    ...
    private void writeDoc(CellBook cellBook, JsonGenerator gen) throws IOException {
        ...
        if (null != docProps) {
            ...
            gen.writeNumberField("mode", docProps.getModel().getCode());
            gen.writeEndObject();
        }
    }

    private String handlerColor(String color) {
        return StringUtils.isNotEmpty((String)color) ? "#" + color : "";
    }
