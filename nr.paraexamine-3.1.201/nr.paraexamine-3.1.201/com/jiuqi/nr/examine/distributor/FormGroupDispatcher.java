...
public class FormGroupDispatcher
extends ExamineTaskDispatcher {
    public FormGroupDispatcher(ExamineTask task) {
        super(task);
    }
    ...
    protected List<ExamineTask> childrenTask() {
        ...
        if (StringUtils.isEmpty((String)key)) {
            ...
            childTask.setParaType(ParaType.FORM);
            childrenTask.add(childTask);
        } else {
            List forms = this.task.getEnv().getNrDesignController().getAllFormsInGroupWithoutBinaryData(key);
            forms.stream().forEach(form -> {
                ExamineTask childTask = new ExamineTask(this.task);
                childTask.setParaType(ParaType.FORM);
                childTask.setKey(form.getKey());
                childrenTask.add(childTask);
            });
        }
        return childrenTask;
