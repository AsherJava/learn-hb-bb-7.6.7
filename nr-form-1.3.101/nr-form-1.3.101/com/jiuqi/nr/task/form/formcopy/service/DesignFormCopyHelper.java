 ...
 *  com.jiuqi.nr.definition.internal.impl.DesignRegionSettingDefineData
...
public class DesignFormCopyHelper {
    ...
    @Autowired
    DesignBigDataService designBigDataService;
    ...
    public DesignFormDefine copyForm(DesignFormDefine srcForm) {
        ...
        if (srcForm.getFormType() == FormType.FORM_TYPE_INSERTANALYSIS && null != srcForm.getFormExtension()) {
            formExtension = srcForm.getFormExtension();
            for (String string : formExtension.keySet()) {
                newForm.addExtensions(string, formExtension.get(string));
            }
        }
        newForm.setKey(UUIDUtils.getKey());
        formExtension = srcForm.getFormExtension();
        if (formExtension != null && !formExtension.isEmpty()) {
            for (Map.Entry entry : formExtension.entrySet()) {
                newForm.addExtensions((String)entry.getKey(), entry.getValue());
            }
        }
    ...
    public DesignDataRegion copyDataRegionTo(DesignDataRegionDefine srcRegion, DesignRegionSettingDefine srcRegionSetting, String fromKey, FormCopyDeleteFormRegionDefine formCopyDeleteFormRegionDefine) {
        ...
        if (formCopyDeleteFormRegionDefine != null) {
            newDataRegion.setKey(formCopyDeleteFormRegionDefine.getKey());
        } else {
        ...
        if (null != srcRegionSetting) {
            DesignRegionSettingDefineData designRegionSettingDefineData = new DesignRegionSettingDefineData(srcRegionSetting, this.designBigDataService);
            designRegionSettingDefineData.getRegionTabSetting();
            designRegionSettingDefineData.getRowNumberSetting();
            designRegionSettingDefineData.getLastRowStyles();
            designRegionSettingDefineData.getCardRecord();
            designRegionSettingDefineData.getRegionSurvey();
