...
import com.jiuqi.nvwa.framework.parameter.syntax.ParamInfoNode;
...
implements IDynamicNodeProvider {
    ...
    public boolean isZBEnabled() {
        return this.zbEnabled;
    }

    public void setZBEnable(boolean zbEnabled) {
        this.zbEnabled = zbEnabled;
    }
    ...
    public IASTNode find(IContext context, Token token, String refName) throws DynamicNodeException {
        ...
        if (this.isZBMode(model)) {
            return this.findAsZBParameter(context, token, model);
        }
        return new ParamNode(token, this.paramEnv, model);
    }

    private boolean isZBMode(ParameterModel model) {
        if (!this.zbEnabled) {
            return false;
        }
        AbstractParameterDataSourceModel datasource = model.getDatasource();
        if (datasource instanceof CustomListDataSourceModel) {
            return ((CustomListDataSourceModel)datasource).isTreatAsZB();
        }
        return false;
    }
    ...
    public IASTNode findSpec(IContext context, Token token, String refName, String spec) throws DynamicNodeException {
        ParameterModel model = this.paramEnv.getParameterModelByName(refName);
        if (model == null || !this.isZBMode(model)) {
            return null;
        }
        String zbName = this.getZBParamValue(model);
