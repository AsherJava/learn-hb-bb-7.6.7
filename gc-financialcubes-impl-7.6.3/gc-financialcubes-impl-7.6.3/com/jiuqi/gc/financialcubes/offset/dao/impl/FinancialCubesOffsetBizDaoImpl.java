...
@Repository
public class FinancialCubesOffsetBizDaoImpl
extends BaseDataCenterDaoImpl
implements FinancialCubesOffsetBizDao {
    @Override
    public int insertTempTable(List<Map<String, Object>> resultList, Set<String> dimensionCodeSet) {
        return this.batchInsertToTempTable(resultList, dimensionCodeSet);
    }
    ...
    public void updateTempTableByPriorPeriod(FinancialCubesOffsetTaskDto offsetTaskDto, String priorPeriod, Set<String> dimensionCodeSet) {
        ...
        sql.append("             \n");
        sql.append("            %3$s \n");
        sql.append("          GROUP BY F.DATATIME,F.MDCODE,F.UNITCODE,F.OPPUNITCODE,F.MD_GCORGTYPE,  \n");
        sql.append("                   F.MD_CURRENCY,F.ORGNCURRENCY,F.MD_AUDITTRAIL,F.SUBJECTCODE  \n");
        sql.append("                   %4$s  \n");
        sql.append("        )\n");
        sql.append(" WHERE EXISTS (SELECT 1 FROM %2$s F");
        sql.append("                WHERE 1 = 1  \n");
        sql.append("                  AND F.DATATIME = '##PRIORPERIOD##'  \n");
        sql.append("                  ##UNITWHERESQL##");
        sql.append("                  AND F.MDCODE = ?\n");
        ...
        if (!CollectionUtils.isEmpty(offsetTaskDto.getSubjectCodes())) {
            subjectSql.append("AND EXISTS (SELECT 1 FROM DC_TEMP_CODE SUBJECT WHERE TEMP.SUBJECTCODE LIKE ").append(OuterDataSourceUtils.getJdbcTemplate().getIDbSqlHandler().concat(new String[]{"SUBJECT.CODE", "'%%'"}));
        }
        String formatSql = String.format(sql.toString(), "GC_FINCUBES_DIM_TEMP", FinancialCubesCommonUtil.getFinancialCubesDimTableName((FinancialCubesPeriodTypeEnum)offsetTaskDto.getPeriodTypeEnum()), existsFieldsEqualsSql, assistDimsFields, subjectSql);
    ...
    public int insertNotExistsDataByTemp(FinancialCubesOffsetTaskDto offsetTaskDto, Set<String> dimensionCodeSet) {
        ...
        sql.append(" %2$s )\n");
        sql.append(" SELECT ##UUIDFUNCTION## AS ID, ? ,?,?,TEMP.UNITCODE,TEMP.OPPUNITCODE,?,\n");
        sql.append(" TEMP.MD_CURRENCY,TEMP.ORGNCURRENCY,TEMP.MD_AUDITTRAIL, TEMP.SUBJECTCODE, \n");
        ...
        sql.append(" %3$s \n");
        sql.append("  FROM ").append("GC_FINCUBES_UPDATE_DIM_TEMP").append(" TEMP \n");
        ...
        args.add(FinancialCubesAuditTrailEnum.OFFSET.getCode() + "%");
        String formatSql = String.format(sql.toString(), FinancialCubesCommonUtil.getFinancialCubesDimTableName((FinancialCubesPeriodTypeEnum)offsetTaskDto.getPeriodTypeEnum()), insertFields, valueFields);
    ...
    public int deleteOffsetByTemp(FinancialCubesOffsetTaskDto offsetTaskDto, Set<String> dimensionCodeSet) {
        ...
        sql.append("   AND F.MD_GCORGTYPE = ? \n");
        sql.append("   %2$s ");
        sql.append("   AND NOT EXISTS (SELECT 1 FROM %3$s TEMP \n");
        sql.append("                    WHERE 1 = 1 \n");
        ...
        StringBuilder subjectSql = new StringBuilder();
        if (!CollectionUtils.isEmpty(offsetTaskDto.getSubjectCodes())) {
            subjectSql.append("AND EXISTS (SELECT 1 FROM DC_TEMP_CODE TEMP WHERE F.SUBJECTCODE LIKE ");
            subjectSql.append(OuterDataSourceUtils.getJdbcTemplate().getIDbSqlHandler().concat(new String[]{"TEMP.CODE", "'%%'"}));
        }
        String formatSql = String.format(sql.toString(), FinancialCubesCommonUtil.getFinancialCubesDimTableName((FinancialCubesPeriodTypeEnum)offsetTaskDto.getPeriodTypeEnum()), subjectSql, "GC_FINCUBES_DIM_TEMP", existsFieldsEqualsSql);
        formatSql = formatSql.replace("##UNITWHERESQL## ", unitWhereSql);
        return this.update(formatSql, args.toArray());
    }
    ...
    public int updateFinancialCubes(FinancialCubesOffsetTaskDto offsetTaskDto, Set<String> dimensionCodeSet) {
        ...
        args.add(FinancialCubesAuditTrailEnum.OFFSET.getCode() + "%");
        String formatSql = String.format(sql.toString(), FinancialCubesCommonUtil.getFinancialCubesDimTableName((FinancialCubesPeriodTypeEnum)offsetTaskDto.getPeriodTypeEnum()), "GC_FINCUBES_DIM_TEMP", existsFieldsEqualsSql, subjectSql);
        formatSql = formatSql.replace("##UNITWHERESQL## ", unitWhereSql);
        return this.update(formatSql, args.toArray());
    }
    ...
    public int insertDataToUpdateTempTable(FinancialCubesOffsetTaskDto offsetTaskDto, Set<String> dimensionCodeSet) {
        ...
        sql.append("       TEMP.SUBJECTCODE,TEMP.NC,TEMP.BF,TEMP.DEBIT,TEMP.CREDIT,TEMP.DEBITSUM,TEMP.CREDITSUM,TEMP.CF,\n");
        ...
        sql.append("                             AND TEMP.MD_CURRENCY = F.MD_CURRENCY AND TEMP.UNITCODE = F.UNITCODE\n");
        sql.append("                             AND TEMP.SUBJECTCODE = F.SUBJECTCODE AND TEMP.ORGNCURRENCY = F.ORGNCURRENCY\n");
        sql.append("                             AND TEMP.OPPUNITCODE = F.OPPUNITCODE AND F.MD_AUDITTRAIL = TEMP.MD_AUDITTRAIL\n");
        sql.append("                             %6$s) \n");
        ...
        String formatSql = String.format(sql.toString(), "GC_FINCUBES_UPDATE_DIM_TEMP", fieldStr, tempFieldStr, "GC_FINCUBES_DIM_TEMP", FinancialCubesCommonUtil.getFinancialCubesDimTableName((FinancialCubesPeriodTypeEnum)offsetTaskDto.getPeriodTypeEnum()), existsFieldsEqualsSql);
        ArrayList<String> args = new ArrayList<String>();
        args.add(FinancialCubesAuditTrailEnum.OFFSET.getCode() + "%");
        String unitWhereSql = "";
        if (!offsetTaskDto.isRebuildFlag()) {
    ...
    public void deleteTempDataByUpdateTempTable() {
        ...
        sql.append("DELETE ").append("GC_FINCUBES_DIM_TEMP");
        sql.append(" WHERE ID IN ( SELECT ID FROM ").append("GC_FINCUBES_UPDATE_DIM_TEMP").append(" )");
        this.update(sql.toString(), new Object[0]);
    }

    @Override
    public void cleanTempTable() {
        this.update("DELETE FROM GC_FINCUBES_DIM_TEMP WHERE 1 = 1", new Object[0]);
    }

    @Override
    public void cleanUpdateTempTable() {
        this.update("DELETE FROM GC_FINCUBES_UPDATE_DIM_TEMP WHERE 1 = 1", new Object[0]);
    }

    private int batchInsertToTempTable(final List<Map<String, Object>> resultList, final Set<String> dimensionCodeSet) {
        ...
        insertSql.append("INSERT INTO ").append("GC_FINCUBES_DIM_TEMP").append(" \n");
        insertSql.append("(ID,UNITCODE,OPPUNITCODE,MD_CURRENCY,ORGNCURRENCY,MD_AUDITTRAIL,SUBJECTCODE,\n");
        insertSql.append(" DEBIT,CREDIT,DEBITSUM,CREDITSUM,CF, ORGNDEBIT,ORGNCREDIT,ORGNDEBITSUM,ORGNCREDITSUM,ORGNCF \n");
