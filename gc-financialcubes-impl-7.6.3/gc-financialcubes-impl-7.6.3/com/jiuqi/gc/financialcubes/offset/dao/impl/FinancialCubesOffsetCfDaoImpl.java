...
@Repository
public class FinancialCubesOffsetCfDaoImpl
extends BaseDataCenterDaoImpl
implements FinancialCubesOffsetCfDao {
    @Override
    public List<Map<String, Object>> queryOffsetData(FinancialCubesOffsetTaskDto offsetTaskDto, Set<String> dimensionCodeSet) {
        ...
        sql.append(" SELECT ##UUIDFUNCTION## AS ID, OFFSETITEM.SUBJECTCODE AS CFITEMCODE, OFFSETITEM.UNITID AS UNITCODE, OFFSETITEM.OPPUNITID AS OPPUNITCODE, \n");
        sql.append(" OFFSETITEM.OFFSETCURR AS MD_CURRENCY, OFFSETITEM.OFFSETCURR AS ORGNCURRENCY, %2$s AS MD_AUDITTRAIL, ? AS SUBJECTCODE,\n");
        sql.append(" SUM(OFFSETITEM.OFFSET_DEBIT) AS DEBIT, SUM(OFFSETITEM.OFFSET_CREDIT) AS CREDIT, SUM(OFFSETITEM.OFFSET_DEBIT) AS DEBITSUM, SUM(OFFSETITEM.OFFSET_CREDIT) AS CREDITSUM,\n");
        sql.append(" SUM((OFFSETITEM.OFFSET_DEBIT-OFFSETITEM.OFFSET_CREDIT)) AS CF, SUM(OFFSETITEM.OFFSET_DEBIT) AS ORGNDEBIT, SUM(OFFSETITEM.OFFSET_CREDIT) AS ORGNCREDIT, \n");
        sql.append(" SUM(OFFSETITEM.OFFSET_DEBIT) AS ORGNDEBITSUM, SUM(OFFSETITEM.OFFSET_CREDIT) AS ORGNCREDITSUM, SUM((OFFSETITEM.OFFSET_DEBIT-OFFSETITEM.OFFSET_CREDIT)) AS ORGNCF \n");
        sql.append(" %3$s \n");
        ...
        sql.append("   AND S.GENERALTYPE ='" + SubjectClassEnum.CASH.getCode() + "'  \n");
    ...
    public List<Map<String, Object>> queryOffsetDataByRebuild(FinancialCubesRebuildDTO rebuildDTO, Set<String> dimensionCodeSet, String subjectTempGroupId) {
        ...
        StringBuilder sql = new StringBuilder();
        sql.append(" SELECT ##UUIDFUNCTION## AS ID, OFFSETITEM.SUBJECTCODE AS CFITEMCODE, OFFSETITEM.UNITID AS UNITCODE, OFFSETITEM.OPPUNITID AS OPPUNITCODE,\n");
        sql.append("        OFFSETITEM.OFFSETCURR AS MD_CURRENCY, OFFSETITEM.OFFSETCURR AS ORGNCURRENCY, %1$s AS MD_AUDITTRAIL, ? AS SUBJECTCODE, \n");
        sql.append("        SUM(OFFSETITEM.OFFSET_DEBIT) AS DEBIT, SUM(OFFSETITEM.OFFSET_CREDIT) AS CREDIT, SUM(OFFSETITEM.OFFSET_DEBIT) AS DEBITSUM, SUM(OFFSETITEM.OFFSET_CREDIT) AS CREDITSUM,\n");
        sql.append("        SUM((OFFSETITEM.OFFSET_DEBIT-OFFSETITEM.OFFSET_CREDIT)) AS CF, SUM(OFFSETITEM.OFFSET_DEBIT) AS ORGNDEBIT, SUM(OFFSETITEM.OFFSET_CREDIT) AS ORGNCREDIT, \n");
        sql.append("        SUM(OFFSETITEM.OFFSET_DEBIT) AS ORGNDEBITSUM, SUM(OFFSETITEM.OFFSET_CREDIT) AS ORGNCREDITSUM, SUM((OFFSETITEM.OFFSET_DEBIT-OFFSETITEM.OFFSET_CREDIT)) AS ORGNCF \n");
        sql.append("        %2$s \n");
        ...
        args.add(date);
        args.add(date);
        String formatSql = String.format(sql.toString(), srcTypeSql, selectFieldStr, subjectSql, groupFieldStr);
        formatSql = formatSql.replace("##UUIDFUNCTION##", sqlHandler.newUUID());
