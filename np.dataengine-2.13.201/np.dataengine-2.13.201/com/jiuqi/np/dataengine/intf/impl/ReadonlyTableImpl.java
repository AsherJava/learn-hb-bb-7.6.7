...
implements IReadonlyTable {
    ...
    public int groupingIdColIndex = -1;
    ...
    private final HashMap<Integer, HashMap<String, List<IDataRow>>> fuzzyMaps;
    private boolean supportTreeGroup = true;
    ...
    public void setRowFilterNode(IASTNode rowFilterNode, String filterExp) {
        this.rowFilterNode = rowFilterNode;
    ...
    public Object getRefValue(FieldDefine fieldDefine, String codeValue) {
        ColumnModelDefine column = this.cache.getDataModelDefinitionsCache().getColumnModel(fieldDefine);
        return this.getRefValue(column, codeValue);
    }

    public Object getRefValue(ColumnModelDefine fieldDefine, String codeValue) {
        if (fieldDefine.getReferColumnID() != null) {
            ColumnModelDefine refField = ReadonlyTableImpl.getRefField(this.cache, fieldDefine, fieldDefine.getReferColumnID());
            if (refField == null || refField.getID().equals(fieldDefine.getID())) {
                return codeValue;
            }
            IEntityTable entityTable = this.getRefTable(refField.getTableID());
            if (entityTable == null) {
                return codeValue;
            }
            IEntityRow entityRow = entityTable.findByCode(codeValue);
            if (entityRow != null) {
                return UUID.fromString(entityRow.getEntityKeyData());
            }
            return codeValue;
        }
        return codeValue;
    ...
    private static ColumnModelDefine getRefField(DefinitionsCache cache, ColumnModelDefine fieldDefine, String referFieldKey) {
        try {
            DataModelDefinitionsCache tableModelCache = cache.getDataModelDefinitionsCache();
            ColumnModelDefine refField = tableModelCache.findField(referFieldKey);
            if (refField == null) {
                return fieldDefine;
            }
            if (refField.getReferColumnID() != null) {
                return ReadonlyTableImpl.getRefField(cache, refField, refField.getReferColumnID());
            }
            return refField;
        }
        catch (Exception e) {
    ...
    private IEntityTable getRefTable(String tableKey) {
        return null;
    }
