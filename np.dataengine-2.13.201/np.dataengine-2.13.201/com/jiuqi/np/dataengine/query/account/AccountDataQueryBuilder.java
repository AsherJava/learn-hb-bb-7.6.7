...
import com.jiuqi.np.dataengine.query.account.AccountQuerySqlBuilder;
import com.jiuqi.np.dataengine.query.account.AccountReadonlyTableImpl;
...
extends DataQueryBuilder {
    ...
    private final BiFunction<QueryContext, List<QueryField>, Boolean> isVersionFunc = (context, fields) -> fields.stream().anyMatch(e -> e.isNeedAccountVersion());
    private final BiFunction<QueryContext, List<QueryField>, Boolean> isDataTimeFunc = (context, fields) -> fields.stream().filter(r -> !"DATATIME".equals(r.getFieldCode())).anyMatch(e -> e.getTable().isAccountRptTable());

    @Override
    protected DataTableImpl createDataTable(QueryContext qContext, CommonQueryImpl queryInfo) {
        return new AccountDataTableImpl(qContext, queryInfo.getMasterKeys(), queryInfo.getColumns().size());
    }
    ...
    public void buildQuery(QueryContext qContext, CommonQueryImpl queryInfo, boolean resultReadOnly) throws ExpressionException, SQLException, ParseException, InterpretException {
        ...
        this.table = resultReadOnly ? new AccountReadonlyTableImpl(qContext, queryInfo.getMasterKeys(), queryInfo.getColumns().size()) : this.createDataTable(qContext, queryInfo);
    ...
    protected Object nodeEvaluate(QueryContext qContext, IASTNode node) {
        GregorianCalendar fixCalendar;
        GregorianCalendar calendarValue;
        Object value = super.nodeEvaluate(qContext, node);
        if (value instanceof GregorianCalendar && (calendarValue = (GregorianCalendar)value).compareTo(fixCalendar = new GregorianCalendar(9999, 0, 1)) == 0) {
            value = null;
        }
        if ("-".equals(value)) {
            value = "";
    ...
    public void loadToDataRowReader(QueryContext qContext, IDataRowReader dataRowReader) throws UnsupportedOperationException, Exception {
        ...
        sqlBuilder.doInit(qContext);
        qContext.setQueryRowStart(0);
        boolean sqlSoftParse = !qContext.isDebug();
        sqlBuilder.setSqlSoftParse(sqlSoftParse);
        sqlBuilder.buildSql(qContext);
