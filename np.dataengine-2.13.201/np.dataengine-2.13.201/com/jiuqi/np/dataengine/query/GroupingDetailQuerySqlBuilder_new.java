...
public class GroupingDetailQuerySqlBuilder_new
extends GroupingQuerySqlBuilder {
    ...
    private void appenddimensionFields(QueryContext context) throws ParseException {
        TableModelRunInfo primarytableRunInfo = null;
        for (QueryTable table : this.queryRegion.getAllTableFields().keySet()) {
            String mainQueryTableAlias = this.getTableAlias(context, table, true);
            String subQueryTableAlias = this.getTableAlias(context, table, false);
            StringBuilder tableSql = this.tableSqls.get(table);
            ...
            if (table.equals(this.primaryTable)) {
                primarytableRunInfo = tableRunInfo;
            }
        ...
        for (ColumnModelDefine fieldDefine : primarytableRunInfo.getDimFields()) {
            StringBuilder tableSql = this.tableSqls.get(this.primaryTable);
            tableSql.append(",").append("dim").append(".").append(fieldDefine.getName()).append(" as dim_").append(fieldDefine.getName());
        }
    }
    ...
    private StringBuilder joinSubQueryTables(QueryContext context) throws ParseException, SQLException {
        ...
        StringBuilder tableSql = this.buildSubQueryByTable(context, this.primaryTable, addRowNumColumn, true);
    ...
    private StringBuilder buildSubQueryByTable(QueryContext context, QueryTable table, boolean addRowNumColumn, boolean isPrimaryTable) throws ParseException, SQLException {
        ...
        if (addRowNumColumn) {
            ...
            for (int i = 0; i < table.getTableDimensions().size(); ++i) {
                ...
                if (dimensionField == null || tableInfo.isBizOrderField(fieldName = dimensionField.getName())) continue;
                tableSql.append(tableAlias).append(".").append(fieldName).append(",");
            }
            tableSql.setLength(tableSql.length() - 1);
        ...
        HashMap<String, TableModelRunInfo> dimTableMap = null;
        if (isPrimaryTable) {
            dimTableMap = new HashMap<String, TableModelRunInfo>();
            tableSql.append(" right join ");
            tableSql.append("(select ");
            StringBuilder onPart = new StringBuilder();
            for (ColumnModelDefine fieldDefine : tableInfo.getDimFields()) {
                ColumnModelDefine refField;
                String dimTableName = null;
                String refKey = fieldDefine.getReferColumnID();
                if (refKey != null && (refField = dataDefinitionsCache.findField(refKey)) != null) {
                    TableModelDefine tableModel = dataDefinitionsCache.getTableModel(refField);
                    dimTableName = tableModel.getName();
                }
                String dimensionName = tableInfo.getDimensionName(fieldDefine.getCode());
                if (dimTableName == null && exeContext.getEnv() != null) {
                    dimTableName = dataDefinitionsCache.getDimensionProvider().getDimensionTableName(exeContext, dimensionName);
                }
                if (dimTableName == null) continue;
                TableModelRunInfo dimTableInfo = dataDefinitionsCache.getTableInfo(dimTableName);
                if (dimTableInfo != null) {
                    tableSql.append(dimTableName).append(".").append(dimTableInfo.getDimFields().get(0).getName()).append(" as ").append(fieldDefine.getName()).append(",");
                    if (onPart.length() > 0) {
                        onPart.append(" and ");
                    }
                    onPart.append("dim.").append(fieldDefine.getName()).append("=").append(tableAlias).append(".").append(fieldDefine.getName());
                    dimTableMap.put(dimensionName, dimTableInfo);
                }
                if (refKey == null || dimensionName.equals("DATATIME")) continue;
                tableSql.append(dimTableName).append(".").append("VALIDTIME");
                tableSql.append(" as ");
                tableSql.append(dimTableName).append("_").append("VALIDTIME").append(",");
            }
            tableSql.setLength(tableSql.length() - 1);
            tableSql.append(" from ");
            for (TableModelRunInfo dimTableInfo : dimTableMap.values()) {
                tableSql.append(dimTableInfo.getTableModelDefine().getName()).append(",");
            }
            tableSql.setLength(tableSql.length() - 1);
            tableSql.append(") dim");
            tableSql.append(" on ").append((CharSequence)onPart);
        }
        StringBuilder whereCondition = new StringBuilder();
        this.buildWhereConditionByTable(context, whereCondition, tableInfo, tableAlias, table, dimTableMap);
    ...
    private void getOrderByField(Integer columnIndex) throws Exception {
        ...
        if (queryField != null) {
            this.orderByClause.append("c_").append(columnIndex + 1);
    ...
    protected void buildSelectSql(QueryContext context, StringBuilder sqlBuilder, StringBuilder subQuerySql) throws Exception {
        ...
        StringBuilder filterNullCondition = this.buildFilterNullCondition(context);
        if (this.wantDetail) {
            sqlBuilder.append(" select ").append((CharSequence)this.gatherFields).append(" from (").append((CharSequence)subQuerySql).append(")");
            sqlBuilder.append((CharSequence)filterNullCondition);
            if (this.whereCondition.length() > 0) {
                sqlBuilder.append(" and ").append((CharSequence)this.whereCondition);
            }
            if (this.groupByClause != null && this.groupByClause.length() > 0) {
                this.appendGroupBySql(database, sqlBuilder, this.groupByClause, this.summarizingMethod);
            }
    ...
    private StringBuilder buildFilterNullCondition(QueryContext context) throws ParseException {
        String commonDim;
        StringBuilder filterNullCondition = new StringBuilder();
        ExecutorContext exeContext = context.getExeContext();
        String string = commonDim = exeContext.getEnv() == null ? null : exeContext.getEnv().getUnitDimesion(exeContext);
        if (commonDim == null) {
            boolean allOwned = true;
            for (int i = 0; i < this.primaryTable.getTableDimensions().size(); ++i) {
                String dimension = this.primaryTable.getTableDimensions().get(i);
                for (QueryTable talbe : this.queryRegion.getAllTableFields().keySet()) {
                    if (talbe.getTableDimensions().contains(dimension)) continue;
                    allOwned = false;
                    break;
                }
        ...
        for (QueryTable talbe : this.queryRegion.getAllTableFields().keySet()) {
            TableModelRunInfo tableRunInfo = exeContext.getCache().getDataModelDefinitionsCache().getTableInfo(talbe.getTableName());
            ColumnModelDefine dimField = tableRunInfo.getDimensionField(commonDim);
            if (dimField == null) continue;
            String tableAlias = this.getTableAlias(context, talbe, true);
            if (filterNullCondition.length() == 0) {
                filterNullCondition.append(tableAlias).append(".").append(dimField.getName());
                continue;
            }
            filterNullCondition.insert(0, context.getQueryParam().getDatabase().getDescriptor().getNVLName() + "(" + tableAlias + "." + dimField.getName() + ",");
            filterNullCondition.append(")");
        }
        filterNullCondition.insert(0, " where ");
        filterNullCondition.append(" is not null");
        return filterNullCondition;
    }
    ...
    private void joinTable(StringBuilder sql, QueryContext context, QueryTable table, boolean orderJoin, boolean noJoin) throws SQLException, ParseException {
        ...
        StringBuilder tableSql = this.buildSubQueryByTable(context, table, orderJoin, false);
        ...
        if (joinItem != null && joinItem.getJoinFields() != null && joinItem.getJoinFields().size() > 0) {
            ...
            for (int i = 0; i < table.getTableDimensions().size(); ++i) {
                ...
                if (this.primaryTable.getTableDimensions().contains(dimension)) {
                    ...
                    sql.append(primaryTableAlias).append(".").append("dim_").append(DataEngineUtil.getDimensionFieldName(primaryTableInfo, dimension));
                    sql.append("=");
    ...
    private void buildWhereConditionByTable(QueryContext qContext, StringBuilder sql, TableModelRunInfo tableInfo, String tableAlias, QueryTable queryTable, Map<String, TableModelRunInfo> dimTableMap) throws SQLException {
        int dimDataType;
        ColumnModelDefine dimensionField;
        ...
        for (i = 0; i < masterkeys.size(); ++i) {
            ...
            if (dimTableMap != null) {
                if (!dimTableMap.containsKey(keyName)) continue;
                tableAlias = "dim";
            ...
            whereNeedAnd = true;
            if (dimensionField.getReferColumnID() == null || keyName.equals("DATATIME") || dimTableMap == null) continue;
            TableModelRunInfo dimTableInfo = dimTableMap.get(keyName);
            String dimTableName = dimTableInfo.getTableModelDefine().getName();
            sql.append(" and not exists (select 1 from ").append(dimTableName).append(" o");
            sql.append(" where o.").append(dimTableInfo.getDimFields().get(0).getName()).append("=").append(tableAlias).append(".").append(dimensionFieldName);
            sql.append(" and o.").append("VALIDTIME").append(">").append("dim.").append(dimTableName).append("_").append("VALIDTIME");
            sql.append(")");
        }
