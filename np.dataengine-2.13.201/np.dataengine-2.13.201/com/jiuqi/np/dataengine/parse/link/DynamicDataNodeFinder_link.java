...
public class DynamicDataNodeFinder_link
implements IReportDynamicNodeProvider {
    private static final Logger logger = LoggerFactory.getLogger(DynamicDataNodeFinder_link.class);
    ...
    public IASTNode findSpecial(IContext context, Token token, String refName) throws DynamicNodeException {
        ...
        if (dataModelLinkFinder != null && (column = dataModelLinkFinder.findDataColumnByFieldName(qContext.getExeContext(), null, refName.toUpperCase())) == null && refName.endsWith("$")) {
    ...
    public IASTNode findRestrict(IContext context, Token token, List<String> objPath, List<IASTNode> restrictItems) throws DynamicNodeException {
        ...
        try {
            ...
            if (groupName != null) {
                columnModelDefine = qContext.getExeContext().getCache().getDataModelDefinitionsCache().parseSearchField(groupName.toUpperCase(), refName.toUpperCase(), this.defaultTableName);
            ...
            if (columnModelDefine != null) {
                ...
                DynamicDataNode dataNode = this.createNode(qContext, columnModelDefine, restrictInfo.periodModifier, restrictInfo.getDimensionRestriction(qContext, columnModelDefine));
    ...
    protected IASTNode find(QueryContext qContext, Token token, String groupName, String refName, boolean hasGroupName) throws DynamicNodeException {
        ...
        try {
            ...
            ColumnModelDefine columnModel = dataDefinitionsCache.parseSearchField(groupName.toUpperCase(), refName.toUpperCase(), this.defaultTableName);
            if (columnModel != null) {
    ...
    protected DataModelLinkColumn findDataModelLinkColumn(QueryContext qContext, String groupName, String itemName, RestrictInfo restrictInfo, IDataModelLinkFinder dataLinkFinder) {
        List<ReportInfo> relatedReportInfos;
        if (dataLinkFinder == null) {
            return null;
        }
        ...
        if (reportInfo != null) {
            ...
            if (column == null && (column = dataLinkFinder.findDataColumnByFieldName(qContext.getExeContext(), reportInfo, itemName)) != null) {
                column.getShowInfo().setZBExpression(true);
            }
